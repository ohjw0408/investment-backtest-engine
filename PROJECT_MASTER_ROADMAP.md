...
**[P3 — PHASE4 제품 기능]**
- ✅ **포트폴리오 즐겨찾기(B1) 완료(2026-06-12).** 종목+비중 1세트·로그인 전용·5탭 전부·한도 20(`get_portfolio_limit()` 단일 변경점). 검증 4층 = API 5 + jsdom 20 + Playwright 31 + 회귀 255. 상세 = log/status update 64.
- **리스크리턴도표** — 선행조건(즐겨찾기) 해소, 착수 가능. D1 TDF·D2 연금통합·A4 종목상세·C1/C2.

**[잔여 곁가지(낮음)]**
- KQ150 fdr 티커 수정(수집 실패). 데이터 파이프 갭채움+gold 외 일일 스케줄러.
- 배당금계산기 G2/절세액 미지원(별도 엔진, 후속).
- 리스크리턴도표(`리스크리턴도표_plan.md`) — 선행(즐겨찾기) ✅ 해소(2026-06-12), 착수 가능.
