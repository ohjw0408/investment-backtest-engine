...
| `기여도_plan.md` | 종목별 상승 견인/하락 방어 지분(attribution). 내자산·백테(구간드래그)·계산기(롤링분포) 3화면. | ✅ **완료·배포(2026-06-17).** `modules/attribution.py`(원화기준) + `/api/attribution/{window,rolling}` + `/api/myassets/attribution`. 14 PASS + 실시세 검증. |
