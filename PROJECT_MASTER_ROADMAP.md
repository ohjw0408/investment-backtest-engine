...
| `간편계산기_plan.md` | 가정 기반 간편 계산기 묶음(복리·배당재투자 등, 시트 대체). 롤링 엔진과 별개. | ✅ **4종 구현 완료(2026-06-10, `/simple`, JS 전용).** 로컬 검증 25+35 PASS. 배포 후 서버검증. |
...
- ✅ **간편 계산기 묶음** (`간편계산기_plan.md`) — **완료(2026-06-10).** `/simple` 4종(복리·배당재투자·인플레 생활비·실질 구매력), JS 전용, 손계산 25+jsdom 35 PASS. 잔여 = 배포 후 서버검증·브라우저 육안.
