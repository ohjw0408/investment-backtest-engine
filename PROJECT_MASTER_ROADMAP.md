...
| `divrefactoring.md` | 배당금계산기 엔진을 공용 Runner 파이프라인으로 통합 | ✅ **통합 완료(2026-06-13, 게이트 방식·오너 승인).** 이 줄은 2026-08-12 동기화에서 "미착수" stale 정정. |
...
| `알림_plan.md` | 가격 트리거(일간변동률·목표가·신고가·신저가) + 리밸런싱 밴드 알림, 인앱 수신함(🔔). Celery Beat 장중 평가. | ✅ **완료·배포(2026-06-17).** `modules/alerts/`(engine·store·runner) + `/alerts` + `/api/alerts/*` + base.html 종 + tasks.py `evaluate_alerts` beat + 토스풍 인라인 진입점(검색·종목·내자산·포폴). **저장 포폴 = 일일리밸 수익 추종 알림 + 즐겨찾기/위젯 추가.** 라이브 probe 12 PASS. ⚠️ Celery beat 재시작해야 자동발화. |
| `기여도_plan.md` | 종목별 상승 견인/하락 방어 지분(attribution). 내자산·백테(구간드래그)·계산기(롤링분포) 3화면. | ✅ 완료·배포(2026-06-17). 단 **2026-07-14에 내자산 상승견인/하락방어 카드와 `/api/myassets/attribution`은 제거**되고 "종목별 손익 카드"로 대체됨(백테·계산기 attribution은 유지). |

### 2026-06-19 이후 신설 플랜 (2026-08-12 동기화에서 추가)

| File | Role | Status |
|---|---|---|
| `디자인통일_plan.md` | 코인베이스 룩 개편 — 팔레트 토큰 + 아키타입 재구성(리스킨 아님) | ✅ 전 페이지 팔레트 + 핵심 4탭(backtest·calculator·dividend·retirement) 아키타입 이식·배포. 잔여 폼 통일은 오너 결정. |
| `모바일홈_출시준비_plan.md` | 모바일 홈 3도어·스플래시·다운로드 프로모·Play 에셋 | 부분 — 앱/푸시/AAB·오프라인 폴백은 완료, 홈 3도어 개편은 미착수. |
| `GOOGLE_PLAY_RELEASE_PLAN.md` | Play 출시 절차 | 로컬 준비 완료(업로드키·서명·AAB·아이콘·피처그래픽). **잔여 = 오너 수동**(Play Console 등록·스크린샷·비공개 테스트). |
| `LAUNCH_LEGAL_SECURITY_PLAN.md` | 약관·개인정보·보안 게이팅 | ✅ 법무 페이지·동의 플로우·계정삭제 안내·HSTS/Secure 쿠키 반영. |
| `MONETIZATION_PLAN.md` | 수익화 | ⏸ 보류(오너). |
| `출시완성도_개선_plan.md` | 출시 완성도 A~I 전수 감사(보안·데이터·인프라·백엔드·프론트·UX·성능·테스트·게이팅) | ✅ P0(백업·WAL·합성손상)·P1(CI 게이트·dep audit·로깅·XSS)·P2(F-1 3상태·E-1 공용JS·E-2 캐시버스팅·E-3 인라인JS 외부화·B-2 데이터품질·F-4 문구·F-5 a11y·F-6 디자인 크리틱) 완료·배포. 잔여 = D-1 app.py 분할(출시 후)·H-1 테스트 분류·H-2 스모크 단일명령·G-1 성능 P3(체감 시). |
| `투자대가_포트폴리오_plan.md` | SEC 13F 대가 포트폴리오(`/examples?tab=guru`, `/gurus`→리다이렉트) | ✅ 데이터·카드·홈카드·상세모달 + **시점별 NAV 사전계산(2026-07-18)**. 잔여 = P5 자동갱신 CI 키(오너). |
| `포트폴리오예시_plan.md` | 추천 자산배분 예시(`/examples`) | ✅ P0~P5 + 오너 피드백 4건 완료·**라이브 배포**(2026-06-26). |
| `포트폴리오_심화분석_plan.md` | 분석탭 분포/롤링 심화 + 비교탭 동급화 | ✅ 심화 아코디언·오차막대·기간 컨트롤·손실확률 배포. 잔여 = [[Current Situation]] 미결 이슈 3(게이트 비율식). |
| `검색_plan.md` | SEO 유입(도메인·robots·sitemap·canonical) | ✅ 인프라 완료. 잔여 = **오너 Search Console 등록**·콘텐츠/권위(시간 필요). |
| `성능최적화_plan.md` | 전 연산경로 최적화(결과 불변) | ✅ P0~P2 완료·배포(탭별 5~9×). 잔여 = P3 백로그(오너 체감 시). 골든마스터 `scripts/perf_golden.py`가 CI 게이트. |
| `ETF_BACKFILL_ARCHITECTURE_PLAN.md` (후속) | 백필 프록시 정합성 | ✅ 2026-07~08 감사로 프록시 오매핑·금리 시계열·비가격 시계열·만기매칭형 거부 가드 신설, 백필 27→339종. |

## Current Situation (2026-08-12 기준 — 이 절이 진실원천)

**제품은 라이브 서비스 상태다.** `https://moneymilestone.co.kr` (nginx + gunicorn + Celery + Redis,
push=자동배포). Android 앱 = Capacitor 래퍼로 같은 도메인 로드, 업로드 AAB·FCM 푸시까지 완성.

2026-06-19 이후 추가된 것(플랜별 소유는 아래 표):
디자인 통일(아키타입 이식) · 모바일 앱/푸시 · 도메인 컷오버(co.kr)+SEO · 출시완성도 A~I(백업·WAL·
CI 게이트·XSS·3상태·공용 JS·캐시버스팅·인라인 JS 외부화·데이터 품질 파이프라인) · 투자대가 13F(+시점별
NAV) · 포트폴리오 예시 · 포트폴리오 심화분석/비교 · 거시지표·캘린더 · ETF 카테고리 검색 · 레이아웃
안전성 + 글자배율 + 금액 콤마 · 데이터 무결성 감사(백필 오매핑·금리프록시·만기매칭 거부) · 적립식 지표
TWR 전환 · 시간봉 tz/결손 · 등락색(캔들 한정) · **가격 갱신 정지 복구(08-12)**.

**현재 블로커: 없음.** 남은 것은 (a) 오너 손이 필요한 출시 절차, (b) 새 기능 착수 결정,
(c) 아래 "알려진 미결 데이터 이슈" 뿐이다.

### 알려진 미결 데이터 이슈 (2026-08-12 실측)

1. ⚠️ **`index_daily` 프록시 시계열 35/37종이 2026-05-28~29에 멈춰 있다.** 갱신 주체가 수동
   스크립트(`index_loader_develop`·`fetch_kr_rates`·`fetch_us_credit_rates`·`build_djdiv_proxy`)뿐이라
   주기 작업이 없다. USD/KRW·KRX_GOLD만 자동. 영향 = 백필 프록시는 **상장 이전** 구간에 쓰이므로
   기존 종목엔 무해하지만, **05-29 이후 상장분을 백필하면 프록시가 모자란다**. `index_ohlc`에 없는
   코드(KS200·SCHD·DJUSDIV_PROXY·KOFR·DBAA·DGS*)는 위젯/상세 폴백에서도 stale.
   → `price_daily`는 08-12에 `refresh_stale_prices` beat로 해결했지만 **`index_daily`는 같은 장치가 없다.**
2. ⚠️ `refresh_stale_prices` 쿨다운에 들어간 14종(상폐 추정 + `310960`·`310970`처럼 야후에 일봉
   자체가 없는 저유동 ETF). 주기 재시도는 돌지만 소스에 데이터가 생기지 않으면 그대로다.
3. ⚠️ `risk_return_logic._clean_deep_points`의 심화통계 게이트가 스캐너와 같은 비율식을 써서
   만기매칭형 단독 포트폴리오면 같은 오탐 함정(2026-08-04 기록). **사용자 수치가 바뀌는 변경이라 보류.**
4. ⚠️ 오너 결정 대기: `US_SEMICONDUCTOR` 8종 / `JAPAN_TOPIX` 9종 프록시 매핑(2026-08-03 감사).

<details><summary>2026-06-18 이전 Current Situation (이력)</summary>
...
> 🟢 **2026-08-12 (세션 마감 · 전체 동기화): 종목 가격 영구 정지 복구 + 그룹 탭 종목 담기.**
> ① **가격 갱신이 실패 한 번에 영구 정지하던 버그**(오너 제보: 스페이스X). 지연 적재 경로에서
> 트레일링 gap-fill 가드가 "시도했음"을 페치 **전에** 기록해, 야후 실패 1회가 그 프로세스의 그날
> 재시도를 전부 막았다. 검색 페이지의 18종목 병렬 시세 호출이 레이트리밋을 유발해 그 기회를 태웠고,
> 아무도 안 보는 종목은 영영 복구되지 않았다 — **prod 1,031종 중 971종 정지**. 수정 = 결론 인지형
> 가드(+10분 백오프) · 페치 예외에도 DB 서빙 · **`refresh_stale_prices` beat(매일 22:00 UTC)** ·
> 무결성 스캔 종목별 정지 비율 경보. prod 복구 **971 → 14종**(잔여는 야후에 데이터 없음).
> ② **그룹 탭에서 보유 종목 담기**(오너 불편 제보) — 코드 단위 체크 1회로 일반·ISA·연금저축 행을
> 한 번에 담는다. 커밋 `118df50`·`05f96d8`. 상세 = wiki `dev/status` update 139 + `dev/bugs`.
>
> **다음 후보 (오너 결정):**
> 1. **`index_daily` 프록시 자동 갱신** — 위 "알려진 미결 데이터 이슈 1". `price_daily`만 자동 복구
>    장치를 얻었고 프록시 시계열은 2026-05-29에 멈춰 있다. 신규 상장분 백필에 직접 영향.
> 2. **Play Store 출시 마무리** — 코드/에셋은 준비 완료, 남은 건 오너 수동 절차
>    (Play Console 등록·스크린샷·비공개 테스트) + **Google OAuth Console redirect URI 등록**
>    (안 하면 로그인 死) + Search Console 등록.
> 3. **PHASE4 잔여 신규 기능** — D1 TDF · D2 연금통합 · C2 자산군비교 · B4 거래트래킹.
> 4. **레이아웃 안전성 하니스 CI 편입** — 지금 수동이라 재발이 자동 차단되지 않는다.
> 5. 모바일 홈 3도어 개편(`모바일홈_출시준비_plan.md` 미착수분).

<details><summary>이전 Next Action 이력 (2026-06-18 이하)</summary>

> 🟢 **2026-06-17~18 (세션 마감): 알림 + 기여도 기능 완료·배포.** ① **인앱 알림**(가격·신고가/신저가·리밸런싱 + 저장 포폴 수익 추종, 토스풍 종 진입점, Celery beat 평가) ② **저장 포폴 = 일일리밸 수익 지수**(알림·위젯 추종·즐겨찾기 추가) ③ **기여도 분석**(상승 견인/하락 방어, 내자산 텍스트·백테 구간드래그·계산기 롤링분포, 원화기준). 전부 라이브 배포. 검증 = attribution 14 + alerts(engine19/api39/runner10) + 라이브 probe 12. **⚠️ 미결: Celery beat 재시작해야 알림 자동발화(서버 작업) + 실 브라우저 OAuth 육안(오너).** ④ **기여도 후속(2026-06-18):** 방어력=비중 무관 포착률로 정정(SCHD<QQQM 방어 정상화)·모바일 날짜입력+MDD 드래그·표 부채꼴밑. **다음 후보 = 디자인 시스템 리팩터(오너 관심, rewrite❌·토큰+공용컴포넌트, 모바일 하단탭바) OR 모바일 앱 전환(Capacitor) OR PHASE4 잔여(D1·D2·C2).**
...
**[6] 마지막 — 인프라/UX 마감:** ~~E1 모바일~~ ✅(2026-06-11), ~~E2 코드 속도 최적화~~ ✅(`성능최적화_plan` P0~P2로 흡수·배포). **E3 `_price_cache` TTL/LRU · E4 서버 가격 보존정책 · C4 온보딩 = 잔여.**
(주의: `출시완성도_개선_plan.md`의 E-1/E-2/E-3는 **다른 항목**이다 — 공용 JS·캐시버스팅·인라인 JS 외부화. 문자만 같다.)
