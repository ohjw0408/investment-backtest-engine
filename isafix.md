...
>
> ✅ **2026-05-30 추가 해결:** 에러 팝업→배너 미관(①), T-B3 리밸런싱 목표비중 하드코딩→계정 연동(③) 모두 해결.
> ✅ **잔여 데이터 이슈 해소:** T-F5 ISA 1억 캡 결과의 배당 0은 `ETF_BACKFILL Phase 6.0 Stage A` 서버 적용으로 해소. ~~정상 배당 데이터 기준 세금 재검증 필요~~ → ✅ 재검증 완료(2026-05-31, Gate 2c 3/3 + tax_truth 64/64).
> ✅ **2026-06-13 동기화: 이 계획 미결 항목 없음 — 종결.** ISA 한도·계좌제한은 이후 다중계좌(Track G)·E2E 16건(2026-06-11)에서도 검증됨.
