...
| BUG-CALC-40Y | **투자계산기 장기(30·40년) 시뮬 실패** — 가상데이터 체크해도 "가상 데이터 생성 불가: 실제 데이터가 너무 적습니다" 에러. 20년은 정상. (오너 발견, 2026-06-04) | **증상:** QQQ+GLD+SCHD 포트, `use_synthetic=True`인데 40년·30년 실패, 20년 작동. **에러 출처 확정:** `calculator_logic.py:268`(멀티) / 대응 단일경로 — `prepare_scenario_data`가 **`n_cases == 0`** 반환 시 발생(`"가상 데이터 생성 불가: {tickers}의 실제 데이터가 너무 적습니다 (최소 1년 필요)..."`). 합성 경로(`allow_synthetic=True`)는 `DataPreparer.prepare`→`_calc_rolling_cases`로 윈도우 수 산출. **로컬 재현 안 됨(핵심 단서):** 로컬 DB는 QQQ 1971~2026·GLD 1971~2020·SCHD 1928~2026까지 **이미 백필**돼 40년 `prepare_scenario_data`가 n_cases=60 정상 반환(에러 없음). **`price_daily.db`는 gitignore라 서버 DB가 로컬과 다름** — 서버는 lazy backfill이라 QQQ/GLD/SCHD history가 얕을 가능성(실제 상장 GLD 2004·SCHD 2011). 40/30년 윈도우엔 합성 백필 필요한데 서버에서 그 생성이 실패해 `n_cases=0` → 에러. 20년은 실데이터로 충분 → 작동(오너 "20부터 된다"와 정합). ⚠️ **추가 발견:** 로컬 GLD 실데이터가 **2020-12-30에서 끝남**(2021~2026 누락, stale 다운로드) — 별도 데이터 갱신 필요. | `calculator_logic.py`(268·612 n_cases==0 분기), `modules/retirement/data_preparer.py`, `modules/retirement/synthetic_price_generator.py`, `modules/retirement/ticker_stats_cache.py`, `data/price_cache/price_daily.db`(서버) | ⚠️ **미해결 — 진단 중.** C3와 **완전 무관**(별개 트랙). 다음 단계: **서버 DB 실제 상태 확인 필요**(SSH로 서버 `price_daily.db`의 QQQ/GLD/SCHD `MIN/MAX(date)·COUNT` 조회) → 서버 history가 얕으면 합성 생성이 왜 실패하는지(`stats is None`? `_target_start` 캡? backfill 실패?) 추적. GLD stale(2020 종료)도 갱신. (Claude) |
