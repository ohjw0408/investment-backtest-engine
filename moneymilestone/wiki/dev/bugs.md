...
| BUG-PENSION-1 | 단일계좌 은퇴 인출 연금소득세 1500만 초과 처리 = 하이브리드(저율+초과분 16.5%) | `pension_monthly_after_tax`가 1500만 이하 저율 + 초과분만 16.5%로 계산. 현행법(선택분리과세)은 1500만 초과 시 **전액** 16.5%라 과소과세 가능 | `modules/tax/base_tax.py` | ⏸️ 보류(낮음) — G5-C용 `pension_separate_tax_annual`(전액 16.5%) 신규 추가함. 단일계좌 인출 통일은 별도 검토(결과 바뀜) |
...
| BUG-TAX-3 | **은퇴 시뮬 이중과세** — 적립 종료 시 전액 청산세(연금 5.5%·위탁 15.4/22%) 부과 후 인출단계서 또 과세 | `AccumulationAnalyzer`/`TaxableSimulationRunner`/`MultiAccountSimulationLoop`이 `tax_enabled`면 적립 끝에 무조건 `apply_liquidation_tax`. 은퇴는 적립종료값을 인출단계로 넘기는데 그게 이미 세후 → 인출세 또 부과(연금=명백한 이중과세, 위탁=비현실적 일괄청산). 청산세는 **독립 투자계산기에서만** 맞음 | `modules/simulation/taxable_runner.py`, `modules/simulation/multi_account_loop.py`, `modules/retirement/accumulation_analyzer.py`, `modules/retirement/multi_account_analyzer.py`, `retirement_logic.py`, `app.py` | ✅ 수정 (2026-06-03, 오너결정): `apply_final_liquidation` 플래그 신설(기본 True=계산기·백테 불변). **은퇴 적립 전 경로(단일 `run_retirement_logic`+`app.py:retirement_run`, 멀티 `_run_multi_account_retirement_logic`) = False → 무청산 gross 인계.** 적립기 중간세(배당·리밸)는 유지. 연금 이중과세 해소. ⚠️위탁 적립차익은 스칼라 인계라 인출단계까지 과세 누락 잔존 → 취득가 포지션 인계는 G5-C. L11 `test_l11_no_final_liquidation_gross_handoff`(거치 위탁 tax ON==OFF). calculator/backtest 회귀 91 PASS(불변) |
