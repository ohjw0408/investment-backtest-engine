...
> 2026-08-03 업데이트: **데이터 무결성 3건**(오너 제보). ①`0046A0` 무결성 알림 = `US_TREASURY` 매핑 누락으로 **금리(DGS10, %)가 가격으로** 백필됨(prod 15,233행, 하루 +41.9%). 단기 3종 DGS3MO 명시 + `US_TREASURY_10Y` 카테고리 추가 + **`_is_rate_series()` 가드로 이 버그 종류 원천 차단**. ②"환헤지 ETF만 과거 데이터 없음"은 **오진** — 진짜 원인은 `tr_index`가 지연 백필을 안 타는 것이고, prod 백필 커버리지가 **1,075종 중 27종(2.5%)**뿐이라 비헤지도 동일 실패. 읽기 직전 `_ensure_history()` 추가로 자기치유(콜드 1928년부터, 0.8~2.4초). ③비교 항목 상한 **무음** 12 → 20 + 토스트, 서버(15/10)도 20으로 통일.
> 2026-08-03 업데이트 2: **후속 감사 — ETF 자동분류 오염 발견**(BUG-ETF-CATEGORY-CONTAMINATION). `KOSPI200` 110종에 섹터·커버드콜·팩터·**미국 ETF**가, `SP500` 40종에 서학개미·동일가중·TOP10이, `KR_TREASURY` 33종 중 국채는 7종뿐. 카테고리 매핑을 신뢰한 게 전제 오류 → **종목명 2차 검증 가드**(`_name_mismatches_index`) 신설. 백필 후보 **471 → 371종**(119 차단 + 신규매핑 18 편입). 신규: `USD`→USD/KRW(11종, 1964년부터·이중환산 없음 실증), `US_CORPORATE` 한글명 분류기(하이일드·TIPS 스킵, 5/8), `US_BOND_AGGREGATE`·`US_MONEY_MARKET`·`KR_TREASURY_5Y`. 타겟 회귀 **110 PASS**.
> 2026-08-03 업데이트 3: **배포·prod 실행 완료.** 0046A0 복구(오염 15,233행 → DGS3MO carry 10,832행, 변동성비 1.05x·스캐너 `corrupt:False`), 백필 스윕(317종 백필/실패0/351초 → **백필완료 27종 → 339종**), 환헤지 S&P500 9종 전부 **1928-01-03부터** 생성 확인. 스윕이 **TPX.F가 TOPIX가 아님**을 드러냄(값 -7.53~29.60, 52%가 0 이하 → 인버스 ETF에 하루 +465.3%) → `JAPAN_TOPIX` 매핑 제거 + 비가격 시계열 런타임 가드(비율 판정 — `CL=F` 2020-04-20 마이너스 유가는 실제 역사라 보존) + prod 5종 정리.
> **잔여(오너 결정)**: ①⚠️ **`US_SEMICONDUCTOR` 8종 매핑 보류** — `RISE 미국반도체NYSE`·`글로벌HBM`·`아시아exChina`가 각각 다른 지수라 ^SOX 적용 가능 여부 확인 필요 ②⚠️ **`JAPAN_TOPIX` 9종** — 올바른 TOPIX 가격 시계열 확보 or ^N225 대체 여부(다른 지수) ③⚠️ **만기매칭형 회사채 ETF 듀레이션** — `26-04`·`만기자동연장` 류에 고정 2.0은 과대(실데이터 std 0.0003 vs 일반채권 0.002). 스캐너 알림 20종의 원인이나 **손상은 아님**. 고치려면 실측 보정 듀레이션을 건드려 백테 결과가 바뀜

> 2026-08-04 업데이트: **매일 오던 "합성 백필 손상 20종" 알림 = 스캐너 오탐**(BUG-SCAN-FROZEN-DENOMINATOR). 20종 전부 만기매칭형 회사채 ETF — 만기가 다가와 **실데이터 std가 0.0003~0.0014(사실상 정지)**라 정상 합성(std 0.0031 = 일반 채권 ETF 수준)도 비율 2.57~9.69로 무조건 걸렸다. 최대 일변동 6.9%도 `CORPAA3Y` **1998-01-05 -3.45%p = IMF 실제 역사**. 수정 = 분모 하한 `REAL_STD_FLOOR=0.0018`(통과선 연 7% → 가짜 역사 불가, SHY 구손상은 여전히 8.9배로 검출). 검증 = 신규 `tests/test_backfill_corruption_scan.py` 5 PASS + **prod 전수 326종 구·신 동시 스캔 20 → 0, 신규 오탐 0**.
> 2026-08-04 업데이트 2: **위 ③ 만기매칭 듀레이션 = 오너 결정 "백필 자체를 금지"**(BUG-TARGET-MATURITY-BACKFILL). 2026-04에 만기가 오는 펀드의 1995년은 존재하지 않고, 듀레이션이 매일 줄어 고정값 모델이 성립하지 않는다 → 값 보정이 아니라 거부. `is_target_maturity()`(YY-MM·만기자동연장·자동연장·존속기한·BulletShares·iBonds, 숫자쌍은 KR만) + `is_bond` 무관 차단 + `_PERMANENT_SKIP`. **대상 26종은 상장일부터만 표시.** 검증 = 타겟 회귀 **84 PASS**.

> 2026-08-03 업데이트 4: **종목상세 배당 그래프**(오너 요청). 가격 차트와 **겹치지 않는** 별도 차트 — 막대(주당 배당금)+선(배당수익률) 이중축, 연간/분기 토글. 착수 전 발견: **배당 표가 백필 생성분을 실제처럼 보여주고 있었다**(449180은 393건 중 380건, 1928년부터 — 상장은 2022-12). 가격 차트는 volume=0을 이미 빼는데 배당만 그대로였다. → 백엔드 `est` 플래그(첫 실거래일 이전=추정), 표에 `추정` 배지, **그래프 기본은 실측만 + `상장 전 추정 포함` 체크박스**. 착시 방지=진행중(올해)·부분 구간(상장 연도) 반투명 처리. 검증 **48 PASS/0 FAIL**(라이트/다크).
> ⚠️ 무관한 기존 실패: `tests/test_symbol_browser.js`가 `candleChart.timeScale()`에서 죽음 — HEAD에서 재현 확인(내 변경 무관, 미수정).
...
> 🔄 **2026-08-12 전체 동기화:** `PROJECT_MASTER_ROADMAP.md`가 06-18에 멈춰 있던 것(커밋 354건 드리프트)을 다시 진실원천으로 복구. stale 6건 정정(divrefactoring 완료·포폴예시 배포됨·거시 C3 완료·출시완성도 P0~P2 완료·PHASE4 E2 흡수완료·기여도 카드 제거 사실). 신규 발견 = [[bugs]] **BUG-INDEX-DAILY-FROZEN**(`index_daily` 프록시 35/37종이 05-29 정지, 주기 갱신 장치 없음 — 오너 판단 대기). 상세=log 2026-08-12 sync. (Claude)

> ✅ **2026-08-16 업데이트 142 (투자대가 백테스트 시점별 재현 + 인접 틈 3건 — 141의 잔여 4건 전부 종결):** 오너 "전부 시작해줘". **①백테 후견편향 제거**: 대가 "분석하기"가 최신 분기 상위 30을 전 구간에 소급하던 것(버핏 2016 구간에 AAPL 25% — 실제로는 2016 하반기에야 산 종목)을 **공시일마다 그때 보유로 갈아끼우는** 방식으로 교체. 실측 차이 **CAGR 21.5%→15.3%(6.2%p가 전부 편향)**. 해법 = `modules/rebalance/scheduled.py: ScheduledRebalance` — 루프가 이미 `should_rebalance`/`generate_orders` 구조라 **엔진 무수정**, `target_weights`를 제자리 갱신해 `SimulationConfig`가 공유 → 적립·배당 스윕도 시점별을 따름. 세그먼트 소스 = `nav.weight_schedule()`로 **비교탭 NAV와 동일**(한 역사). 가격 없는 종목은 **세그먼트별** 제외+재정규화(NaN 차단), 제외 몫은 `covered_ratio`로 UI 노출. **종목·비중 편집 시 즉시 해제**(고정 비중 강등 + 배너·조건바 동기), 다계좌는 `guru_pit:null`로 경고 표기. **②오버레이 무경고 폴백**: `app.py` index_series가 가격 기준일 때 NAV를 버리고 현재 비중 혼합으로 갈아타던 것 → 항상 NAV 서빙 + `basis_mismatch` 경고(+`lookahead` 플래그). **③TOP_N 불일치**: `store.get_guru`가 `nav.TOP_N` 단일 소스로 `in_sim` 부여, 모달 표에 **시뮬** 배지+각주. **④P5 분기 자동화**: `guru-resync-13f.yml`(마감 익일+1주 뒤 지각 회수 → 리빌드→커밋→배포→`refresh_guru_nav`) + **안전망 2겹**(키 없으면 시작 거부, `scripts/guru_db_stats.py`가 filings·대가 감소/매핑률 5%p↑ 하락 시 커밋 차단). ⚠️**오너 잔여 = repo secret `OPENFIGI_API_KEY` 등록**(없으면 매 분기 워크플로 실패). 검증 = 신규 `test_guru_pit_backtest.py` **7**(후견편향 회귀 가드 포함) + `test_guru_pit_browser.js` **17**(라이트/다크·콘솔0) + 기존 6 + **perf_golden 4/4 결과 불변**. 상세=[[../log]] 2026-08-16. (Claude)

> ✅ **2026-08-16 업데이트 141 (투자대가 13F seed가 한 분기 밀려 있던 문제 + 시점별 비중 구조 점검):** 오너 제보 "DB에 최신 정보 반영 안 됐다". 실측 = EDGAR에 2026-06-30(Q2)이 8/13~8/14 올라왔는데 `guru_holdings.db`는 10명 전원 **2026-03-31**. 갱신 경로가 수동 리빌드뿐이라(P5 `guru-resync-13f.yml` 미착수) 공시가 떠도 가져오는 주체가 없었다. **조치** = `scripts/build_guru_db.py` 전체 리빌드(`2f3fb49`) → push → 배포 → prod probe. filings 471→**479**, holdings 9,538→**9,719**, 미매핑 26.2→26.0%, 커버 72.9→73.1%. Q2 반영 8명 / **Ackman = Q2 미제출**(EDGAR에 없음) · **Burry = 2025-09-30 이후 무공시(10.5개월)** — 둘 다 stale 2년 컷 미만이라 목록 유지(오너 기준 유지). 검증 = Buffett Q2 AAPL 22.04·AXP 17.14·KO 10.86·GOOGL 9.41·BAC 9.20 실제 13F 일치, prod `tasks.refresh_guru_nav()` 수동 트리거로 10명 30,325행·곡선 2026-08-14까지 연장. **같이 파악한 구조 = 시점별 비중이 절반만 쓰인다** — "비교 담기"→`/risk-return`만 `guru_nav`(공시일 리밸, 분기 상위10 재정규화, 45일 지연 반영)를 타고, **"분석하기"→`/backtest`는 최신 분기 상위30을 그대로 꽂아 후견편향**(백테 엔진에 guru 인지 코드 0줄). ⏸**미해결 틈 4건(오너 결정 대기)**: ①백테 핸드오프 시점별화 ②`app.py:3452` 가격 기준일 때 NAV 버리고 현재 비중으로 **무경고 폴백** ③TOP_N 10 vs 30 불일치 ④P5 분기 자동화(11월 Q3 재발 확정). 상세=[[../log]] 2026-08-16. (Claude)

> ✅ **2026-08-15 업데이트 140 (배당락 알림이 6주간 0건 — 실적·배당을 종목 알림으로 이관):** 오너 제보 "배당락일 알림을 켰는데 받은 기억이 없다". prod 실측 = **진짜 안 왔다** — user 1 캘린더 알림 12건(07-01~08-13) 전부 경제지표/통화정책, 배당·실적 0건. 러너를 prod에서 그대로 돌리니 `ALERT CODES=['KRX_GOLD']` 한 종목뿐이고 그마저 배당 비대상 필터에서 탈락 = **발화 확률 0**. 뿌리 = 대상 종목 선택이 **전역 `excluded` 코드 집합**인데 체크박스는 내 자산·포폴·관심목록마다 같은 코드를 중복 렌더 → 한 그룹에서 끄면 다른 그룹의 같은 종목까지 사망(prod DB에 SPY·GLD 12중복, `PF:1` 비티커 혼입). 거기다 본문이 앞 3건만 담는데 조립 순서상 배당이 항상 꼴찌라 "외 N건"으로 잘렸다. **조치(오너 지시)**: 종목 소스 선택 UI 폐기 + 실적·배당락을 **종목 알림 › 실적·배당 탭**의 룰로 이관(`modules/alerts/symbol_event_runner.py`, beat 23:05 UTC). 캘린더 알림은 거시(econ·policy) 전용. 배당 룰은 **계좌별·종목별 예상 배당금** 동봉(보유수량×주당배당, ISA/연금 비과세·KR 15.4%·US 15% 세후 원화). ⚠️**지급일은 뺐다** — yfinance 실측상 지급일이 오는 건 미국 개별주 다음 1회뿐이고 ETF·한국 종목은 404(오너 보유는 전부 ETF)라 배당락일 기준 예상액임을 UI·본문에 명시. 곁가지 = `dividend_events`가 예측 플래그를 `predicted`(항상 False)로 읽던 것 `projected`로 정정, `last_triggered_at`을 KST 날짜로 기록해 23:05 UTC 재실행 중복 차단. 검증 = 신규 `test_symbol_event_runner.py` 16 + `test_calendar_alert_runner.py` 8 + `test_alerts_api.py` 39→50 + `test_alert_runner.py` 10 + **Playwright `test_alerts_event_tab_browser.js` 27/27**(라이트/다크 스샷·콘솔 0). (Claude)

> ✅ **2026-08-12 업데이트 139 (종목 가격 영구 정지 + 그룹 탭 종목 담기):** 오너 제보 2건. **①SPCX(스페이스X) 가격이 상장 며칠치에서 두 달째 멈춤** — prod 실측 `price_daily` 5행(06-12~06-18). 뿌리 = `get_price` 트레일링 gap-fill 가드가 `_gapfill_trail_day[code]=end_date`를 **페치 결과와 무관하게 페치 전에** 찍은 것. 야후 실패 한 번이 그 프로세스의 그날 재시도를 전부 차단했고, 검색 페이지의 `/api/watchlist/quotes` 18종목 **병렬** 호출이 레이트리밋을 유발해 그 유일한 기회를 태웠다(08-10 접속로그: 14:57 quotes 실패 → 14:58 `/api/symbol/SPCX` 947바이트 stale). 게다가 페치 예외가 `get_price` 밖으로 전파돼 **DB에 데이터가 있어도** 시세가 통째로 비었고, `_watchlist_quote`는 `except: pass`라 로그도 없었다. **SPCX 단독 문제 아님 — prod 1,031종 중 971종 정지**(전체 `MAX(date)`만 보던 무결성 스캔이 두 달간 눈 멈). 수정 3층 = ①`_trail_should_try`/`_trail_mark`로 "결론이 났을 때만" 하루 잠금(빈 결과는 DB 최종일 간격 ≤5일=정상 / 초과=실패 → 10분 백오프 재시도) ②페치 try/except로 DB 보유분 서빙 + 실패 로그 ③`refresh_stale_prices` beat(매일 22:00 UTC) = 지연 적재의 **유일한 능동 복구 경로**(연속 3회 실패 = 7일 쿨다운, `price_refresh_state`) + 무결성 스캔에 종목별 정지 비율 30% 경보. 회귀 `tests/test_price_loader_stale_refresh.py` 4종. **②그룹 지정이 너무 번거로움** — `holdings`가 `(code, account_type)` 행이라 같은 종목이 일반·ISA·연금저축에 나뉘면 종목 탭 수정 모달을 계좌 수만큼 반복해야 했다. 그룹 탭에 구성 종목 칩(`×`로 빼기)·`+ 종목 담기`·미분류 배너 신설, 담기 모달은 **코드 단위 체크박스**(체크 1회 = 모든 계좌 행 반영, 검색·필터·일괄 선택). API `POST /api/myassets/holdings/group`. 검증 = `tests/test_myassets_groups_browser.js` **15/15 PASS**(실서버·실DB·라이트/다크/모바일 육안·콘솔 0). **prod 배포·검증 완료**(`118df50`): 수동 복구 6배치로 **정지 971종 → 14종**(15,340행 추가). 남은 14종은 야후에 데이터가 없는 것(상폐 추정 + `310960`·`310970`처럼 거래가 거의 없어 일봉 자체가 없는 ETF) — 쿨다운 등록됨. `AXP`·`TLT`·`VTI`·`MSTY`·`SKYT` 시세 복구 확인, beat 등록 확인. 상세=[[bugs]] BUG-PRICE-STALE-FOREVER·BUG-GROUP-ASSIGN-UX. (Claude)

> ✅ **2026-07-22 업데이트 138 (앱 뒤로가기가 홈에서 죽는 버그 — 로컬 검증 완료):** 오너 보고 — 안드로이드 앱에서 뒤로가기로 홈까지는 가지지만 홈에서 아무리 눌러도 종료가 안 됨. **원인 = 종료 코드가 없었다.** `@capacitor/app` `AppPlugin.java:53-56`은 JS `backButton` 리스너가 없을 때 `if (canGoBack) goBack();` **else 없음** — 홈은 앱 시작 URL이라 `canGoBack=false` → 무동작. `Activity.finish()` 경로는 `App.exitApp()`뿐인데 아무도 호출하지 않았음(히스토리 오염 문제 아님). 수정 = `static/js/base_app.js`에 앱 전용 `backButton` 리스너: ①열린 오버레이(사이드바→벨 드롭다운→`.modal-overlay.show`) 닫고 소비 ②홈이 아니면 `history.back()`(히스토리 없으면 `/`로 — 푸시 딥링크 진입 대비) ③홈이면 두 번 눌러 종료(1회차 토스트, 2초 내 2회차 `exitApp()`). 리스너 등록만으로 기본 동작은 자동 비활성. **APK 재빌드 불필요**(원격 로드 래퍼, push만으로 반영). 검증 = Playwright에서 `window.Capacitor` 스텁 후 실제 콜백 발화, 7케이스 PASS(비홈→이동·홈1회 토스트·2회차 exit·2.3초 후 만료·드로어만 닫힘·모달만 닫힘·일반 웹 미등록), pageerror 0. **실기기 확인 미완 — 오너 확인 필요.** (Claude)

> ✅ **2026-07-22 업데이트 137 (내 자산 온보딩 UX + 모달 입력창 시인성 — 로컬 검증 완료, 미배포):** 오너 제보 2건, 기능 변경 없는 UI 개선. **①신규 가입자가 "현황"에서 막힘** — 종목 추가 진입점이 "종목" 탭·보유 종목 카드에만 있어 첫 화면인 현황에는 없었음. 자산 추이 빈 영역 `#maHistoryEmpty`를 안내 한 줄 → 아이콘+제목+설명+`+ 첫 종목 추가` CTA 빈 상태로 교체. 보유는 있는데 이력만 없는 경우와 섞이지 않도록 `syncHistoryEmptyMode()`가 `holdings.length`로 `.has-holdings` 토글(있으면 CTA 대신 "시세 이력이 아직 없습니다"). 보유 목록 빈 상태(`renderHoldings`)에도 같은 버튼. **②모달 입력창이 빈 공간처럼 보임** — `.calc-input`은 테두리가 없고 `.input-group` 래퍼가 테두리·배경·포커스링을 담당하는 구조인데 **종목 검색·매수일·그룹 select·그룹명** 4개가 래퍼 없이 맨몸이었음(수량·평단가만 래퍼 보유). 전부 `.input-group`으로 감싸고 종목 검색에 돋보기 아이콘 추가(`mi('search')` 신규). 자동완성 드롭다운은 기존 relative 컨테이너의 형제 유지 → 위치 영향 없음. **검증**: 로컬 서버 + Playwright 430×900 라이트/다크 — 현황 CTA 노출 → 모달 → "삼성" 검색 → 드롭다운 6건 → 선택 → 수량·평단 입력 → 저장 → 보유 목록·차트 렌더, 추가 후 CTA 사라짐까지 실클릭 확인, 콘솔 에러 0. 변경=`templates/myassets.html`·`static/js/myassets_page.js`. **다음=오너 확인 후 push(자동배포).** (Claude)

> 🚨✅ **2026-07-21 업데이트 136 (내 자산 0원 장애 — 긴급 수정·prod 검증 완료):** 플레이스토어 정식버전 설치 후 앱·웹 모두 보유자산 0원(배당금만 정상). **데이터 손실 아님** — prod users.db 보유 8행 전부 무사. 뿌리 = `/api/myassets/data`·`/api/portfolio/history`가 요청 안에서 KRX Open API를 동기 호출하는데 `krx_client` 타임아웃 30초 하드코딩 × KOSPI/KOSDAQ 2회 순차 = 최대 60초 > gunicorn worker `timeout=30` → WORKER TIMEOUT SIGKILL → 500. 07-21 KRX 실제 무응답(3연속 30.2s read timeout 실측)이라 상시 발화. 배당 API는 KRX를 안 타서 200 = 증상 비대칭의 정체. 프런트가 `res.ok`를 안 봐서 500을 "보유 없음"으로 렌더 → 0원. Redis 캐시는 성공한 fetch만 채우므로 장애 중 영영 cold(죽음의 고리). 수정(76a0901) = ①요청경로 KRX 타임아웃 `(3s, 6s)` ②서킷브레이커 `krx_api:down` Redis TTL 600s ③최후 폴백 `price_daily`/`index_daily` 최신 종가(0원 대신 낡은 값) ④두 라우트 시세 예외 흡수 ⑤프런트 `res.ok` 가드. 덤으로 설정›계정 푸시 토글이 기기 알림 권한을 반영하도록 `window.mmPushPermission()` 신설(재설치 후 "켜짐"인데 권한 없어 다시 켤 수 없던 문제). **검증(KRX 죽은 실전 조건)**: prod gunicorn 경유 실HTTP cold 13.2s 200 총액 95,777,660원 / 브레이커 후 0.0s 200, 로컬 KRX+yfinance 동시 사망 시 DB 폴백 동작, Playwright 라이트/다크 콘솔 0. 상세=[[bugs]] BUG-MYASSETS-ZERO-KRX-STALL. (Claude)

> ✅ **2026-07-19 업데이트 135 (ETF 카테고리 검색 — 전 종목 분류 + 자연어 + 상세검색 탭):** 오너 요청 "미국 단기채 etf 검색 시 미국·한국 상장 둘 다"를 NH증권급 카테고리 검색으로 구현. ①분류 데이터: `symbols`에 8컬럼 신설(asset_class·region·bond_type·bond_dur·eq_style·eq_size·sector·cls_src), KR 1,157 전 종목=이름규칙(`modules/etf_classifier.py`), US 5,528=yfinance 모닝스타 category 매핑+이름규칙 폴백(`scripts/fetch_us_etf_categories.py` 재개형 수집→`data/meta/us_etf_categories.csv` git 추적, 레이트리밋 백오프 필수 — 3워커 병렬은 401 크럼 오류 대량). US 레버리지 부호 이름규칙 산출(UltraPro Short=-3 등). ②자연어: `modules/etf_facets.py` 별칭사전+그리디 토큰분해 — "미국 단기채"→region=US∧bond∧{short,ultrashort} 양시장 동시 히트(통합검색 최상단, 잔여토큰은 이름 LIKE). ③UI: `/search`에 통합/ETF 상세검색 모드 탭 — 패싯 패널(상장시장·자산군·투자지역·만기·채권종류·스타일·섹터·레버리지·환헤지, 채권/주식 조건행, 검색어 없이 브라우즈), 카드 부제=한국어 패싯("채권 · 미국 · 단기 · 국채"). ④CI: resync-symbols.yml에 신규분 카테고리 수집+분류 재실행 편입. 검증: API 24 PASS + Playwright 23/23(라이트/다크/모바일 육안·콘솔 0). (Claude)

> ✅ **2026-07-14 업데이트 134 (내자산 종목별 손익 + 홈 최대기여 한줄):** 기간 선택 시 종목별 등락%·기여금액이 안 보이던 것 해결. `_compute_portfolio_history`에 종목별 `series`+`names` 추가(종목 diff 합=총액 diff, 현재 수량 고정 가정), 내자산 히어로 아래 `perStockCard` 신설(오늘/기간 토글·기간칩 연동, 기여금액 내림차순, 수동가격 — 표시, 가림 시 %만 노출), 상승견인/하락방어 카드·`/api/myassets/attribution` 웹·모바일 모두 제거(백테·계산기 attribution은 유지), 홈 포폴 카드에 기간 최대기여 종목 한 줄(`#pfTopMover`→/myassets). Playwright 14/14 PASS(합계 일치 ±10원·모바일 탭 노출/숨김·콘솔 0)+라이트/다크 육안. (Claude)

> ✅ **2026-07-14 업데이트 133 (오너 보고 버그 4건 일괄 종결 — 지수 포인트 표기·검색 신선도·푸시 진단):** ①지수 단위: 코스피 "₩6,900" → "6,900" 포인트 — 통화코드 `PT` 신설(`market_alias.is_index_point()`), 검색·홈위젯·상세·차트툴팁·알림본문 전 표시층 적용, EEM/ACWI는 실ETF라 $ 유지 (7a693a1). ②검색 stale 가격: `_search_attach_prices`가 price_daily(클릭 시만 백필) 박제값만 읽던 것 → 신규 `_search_index_quotes()`로 index_ohlc(30분 beat)→index_daily→price_daily 3단 조회 + `CANDLE_INDEX_CODES` 12→20종 확장(^KQ11·^SOX·^RUT·^STOXX50E·^HSCE·^NSEI·000300.SS·TPX.F — 이 8종은 검색뿐 아니라 홈위젯·캔들·알림 live_quote까지 5/29 시드 박제였음) + 금리 별칭 `RATE`(%) 표기. ③전 종목 자동갱신: 검색 렌더 직후 보이는 페이지(≤18종목)만 `/api/watchlist/quotes` 자동 호출, 방어 = per-code Redis 15분 캐시 + rate limit 60/min 신설 + 실패 negative cache 5분 + 실패 시 기존값 유지. ④푸시 미도착: prod 전수 조사(env·키·토큰·동의·로그·실전송 sent=1 → 오너 폰 도착) 결과 **서버 무죄** — 07-13 건은 단말/타이밍. push_sender 무음 제거(키 no-op·죽은토큰 삭제·발화당 감사 로그) → 재발 시 journalctl 1줄 판별. 검증: 타겟 테스트 3종 PASS(home_widgets 지수통화 기대값 KRW→PT 동기화 포함), Playwright 라이트/다크(코스피 6,807 박제 해소·삼성전자 카드 ₩285,000→₩254,500 라이브 갱신 실확인·콘솔 0), prod 배포 후 refresh_index_ohlc 20종 수동 적재 + 라이브 API 검증 완료 (96b72d1). (Claude)

> ✅ **2026-07-07 업데이트 132 (포폴 정밀 비교 — 기본선택 제거 + 거시지표 벤치마크):** `/risk-return` 정밀 비교에서 내 포트폴리오 기본 전체선택 → 기본 선택 없음(`risk_return_page.js` `rrSelected` 빈 Set). 거시지표(지수형만 — 부동산·M2·물가 등, 금리·% 제외)를 벤치마크로 추가 가능하게: `risk_return_logic._load_macro_series`가 macro_observations 레벨을 (close, div=0)으로 로드→`compute_comparison`·`compute_risk_return` 자동 적용, 프론트 검색 드롭다운에 종목과 함께 노출(거시지표 즉시 렌더). Playwright 검증(Task1 미선택·Task2 산점도/표 통합·라이트/다크·콘솔0), compare route 테스트 5 PASS.

> ✅ **2026-07-07 업데이트 131 (거시지표 "부동산" 카테고리 + 한국 주택가격지수 32종):** `/macro`에 "부동산" 카테고리 신설. 한국부동산원 유형별 주택매매가격지수(`901Y093`+`901Y113` 스플라이스, 2003.11~)로 서울·전국·수도권·지방·17시도 × 아파트/주택종합/연립다세대/단독주택 세분, 아파트 실거래지수(`901Y089`, 2006~)로 서울 5생활권역까지 추가(총 32종). 기존 미 케이스실러·주택착공·건축허가·KB전국주택도 부동산으로 이동. 변경=`modules/macro_loader.py`만(`_ecos_splice`/`fetch_ecos_splice` 체인링크 + `CATEGORIES` 삽입). 로컬 Playwright 검증 통과(칩·카드·상세차트·라이트/다크·콘솔0). ⚠️배포 후 `python -m modules.macro_loader --backfill`로 prod 백필 1회 권장.

> ✅ **2026-07-02 업데이트 130 (모바일 푸시 — heads-up/소리/진동용 HIGH 알림 채널):** 오너가 푸시가 폰 알림함에는 뜨지만 유튜브/인스타처럼 상단 배너, 소리, 진동, 화면 꺼짐 상태에서의 주의 환기가 약해 기대한 알림과 다르다고 보고. 확인: FCM notification은 정상 도착했지만 Android가 Google 기본 fallback 채널 `fcm_fallback_notification_channel`(`Miscellaneous`, importance=3 DEFAULT)을 사용하고 있었다. Android 8+에서는 FCM payload priority보다 앱이 미리 만든 notification channel importance가 실제 heads-up/소리/진동 수준을 좌우하므로 DEFAULT 채널로는 제한적일 수 있다. 수정: Android native `MainActivity`에서 신규 `money_alerts_high_v1` 채널 생성(IMPORTANCE_HIGH, 기본 알림음, 진동 패턴, lockscreen public, lights ON). Manifest에 FCM 기본 채널 meta-data를 추가하고 `VIBRATE` 권한도 선언. 서버 `push_sender`는 FCM HTTP v1 payload에 `android.priority=HIGH`, `android.notification.channel_id=money_alerts_high_v1`, `notification_priority=PRIORITY_HIGH`, default sound/vibration/light, visibility PUBLIC을 포함한다. 포그라운드 로컬 알림도 같은 `channelId`를 사용. 검증: `py_compile push_sender.py` OK, `tests/test_push_consent.py` 15 PASS, `gradlew assembleDebug` BUILD SUCCESSFUL, 새 APK 실기기 설치 성공. 실기기 `dumpsys notification`에서 `money_alerts_high_v1` 채널이 `mImportance=4`, `mSound=content://settings/system/notification_sound`, `mVibrationPattern=[0,250,150,250]`, `mVibrationEnabled=true`로 생성됨을 확인. 변경=`mobile/android/app/src/main/java/com/moneymilestone/app/MainActivity.java`, `mobile/android/app/src/main/AndroidManifest.xml`, `mobile/android/app/src/main/res/values/strings.xml`, `modules/alerts/push_sender.py`, `templates/base.html`. (Codex)
...
> ✅ **2026-07-01 업데이트 127 (알림 — 증시 캘린더 안내 강화 + 모바일 메뉴 스크롤):** 오너가 `/alerts`의 증시 캘린더 알림이 “경제지표·통화정책·실적발표·배당락”만 보여 비직관적이고, 저장 후 알림함에 바로 뜨지 않아 잘 저장됐는지 알기 어렵고, “매일 아침 8시”가 한국/미국 장 기준으로 무슨 의미인지 모호하다고 지적. 또한 모바일 햄버거 메뉴가 스크롤되지 않는다고 보고. 수정: `templates/alerts.html`의 증시 캘린더 알림 카드에 “저장 직후 알림 내역은 바로 생기지 않고 다음 08:00 KST 실행 때 오늘 일정이 있으면 생성됨” 안내를 추가하고, 한국 주식은 장 시작 전·미국 주식은 그날 밤 정규장 시작 전 알림이라는 설명을 명시. 네 가지 일정 종류에는 CPI·FOMC·선택 종목 실적/배당락 같은 보조 설명을 붙이고, 저장 상태 문구와 현재/저장된 선택 요약(`경제지표 N개`, 종목 일정 대상, 08:00 KST 발화 조건)을 카드 안에 계속 표시. 경제지표 기본 ON 상태에서 세부 지표가 비어 보이지 않도록 저장값이 비어 있으면 전체 지표를 선택해 보여주며, 저장 시 release_id 숫자 배열이 서버에 정상 persisted 되는 것까지 확인. 모바일 사이드바는 `100dvh` 높이, 내부 `overflow-y:auto`, iOS 터치 스크롤, 하단 safe-area 여백을 적용해 긴 메뉴를 드로어 안에서 스크롤 가능하게 수정. CSS 캐시 버전 `20260701nav2`로 갱신. 검증: 로컬 Playwright DOM 검사 PASS(캘린더 안내·4개 설명형 라벨·FOMC 보조문구·경제지표 8/8 선택·저장 POST 200·저장 상태/요약 08:00 KST 문구·서버 persisted `econ_ids=[10,46,50,53,54,9,13,192]`·모바일 390x640 드로어 `scrollHeight 903 > clientHeight 640`, scrollTop 이동, 콘솔 에러 0), `git diff --check` OK. 변경=`templates/alerts.html`, `static/css/style.css`, `templates/base.html`. (Codex)
...
> ✅ **2026-06-30 업데이트 124 (백테스트 — SCHD 배당성장률 부분연도 스파이크 제거):** 오너가 백테스트 배당 성장률 그래프에서 SCHD 2012년 값이 크게 튀는 문제를 보고. 원인 확인: SCHD는 2011년 10월 상장이라 2011년 배당은 12월 1회뿐이고, 2012년 full-year 배당과 단순 전년 대비를 하면 +563.9% 착시가 발생. 백테스트 프론트는 첫 해/저베이스/미완료 연도를 필터링하지 않고 `annual_dividends`를 바로 전년 대비로 계산했다. 수정: `renderDividendGrowthChart()` 앞에 `btCleanDividendGrowthRows()` 추가해 시작/종료 부분연도, 중앙값 30% 미만 저베이스 선행연도, 연속 full-year 체인 이전의 고립 연도를 제외하고, 성장률 막대도 연속 연도끼리만 계산. 검증: 실제 SCHD 백테스트 엔진 출력(raw 2010~2026)으로 Playwright DOM 검증 PASS(cleaned 2012~2025, chart labels 2013~2025, maxAbs 33%, 2012 스파이크 없음, 콘솔 에러 0), JS 문법 체크 OK, `git diff --check` OK. 변경=`templates/backtest.html`. (Codex)

> ✅ **2026-06-30 업데이트 123 (포트폴리오 비교 — 지표별 심화 오차막대 표본 길이 선택):** 오너가 지표별 심화 비교에서 장기 이력 종목은 여러 위기/호황을 겪어 오차막대 꼬리가 길고, 신생 ETF는 표본이 짧아 안정적으로 보이는 착시가 있다고 지적. 수정: `/risk-return` 지표별 심화 비교의 5개 오차막대 그래프(수익률·변동성·MDD·배당·배당성장률)에 표본 길이 선택 버튼 추가(`최대`, `자동`, `5/10/15/20/25/30년`). 기본값은 `최대`로 두어 장기 데이터를 최대한 보존하고, `자동`은 해당 지표에서 가장 짧은 유효 표본에 맞춰 각 항목의 최근 N년만 사용하도록 구현. 카드 상단에 `자동`과 `최대`의 장단점 설명을 별도 안내 블록으로 표시. 검증: JS 문법 체크 OK, `git diff --check` OK, 로컬 Playwright DOM 검증 PASS(컨트롤 5개, 기본 `최대` active, `10년` 선택 시 표본수≤10, `자동` 선택 시 수익률 표본 8년으로 전 항목 통일, MDD만 5년 선택 가능, 차트 bar 유지, 콘솔 에러 0). 변경=`templates/risk_return.html`. (Codex)

> ✅ **2026-06-30 업데이트 122 (포트폴리오 비교 — 도표 로딩 상태/등장 애니메이션):** 오너가 `/risk-return` 정밀 비교 실행 시 리스크-리턴 도표와 종합 비교(레이더/다각형)가 늦게 출력되는데 로딩/애니메이션이 없어 갑자기 생기는 느낌이라고 피드백. 수정: 정밀 비교 시작 직후 `rrCompareChartsLoading` 로딩 패널을 먼저 표시하고, API 응답 완료 후 로딩을 숨긴 뒤 `rrCompareCharts`에 `rr-chart-reveal` 페이드업 애니메이션을 적용. 비교 중 버튼은 disabled 유지, 오류/빈 데이터 시 로딩을 정상 해제. 검증: 로컬 Playwright DOM 검증 PASS(초기 로딩 숨김, 비교 중 로딩 표시·차트 숨김·버튼 잠김, 완료 후 로딩 숨김·차트 표시·산점도/레이더 인스턴스 생성·콘솔 에러 0), JS 문법 체크 OK, `git diff --check` OK. 변경=`templates/risk_return.html`. (Codex)

> ✅ **2026-06-30 업데이트 121 (포트폴리오 비교 — 추세/상관 기간 동기화 + 도표 재배치):** 오너 요청 반영. `/risk-return`에서 정밀 비교 결과의 **리스크-리턴 도표**와 **종합 비교(레이더/다각형)**를 정밀 비교 카드 내부에서 빼서 **상관계수 계산기 바로 아래**에 표시하도록 재배치. 추세 겹쳐보기와 상관계수 계산기의 날짜·1/3/5/10년·전체·사건구간 입력을 `rrOv` 단일 상태로 통합해, 어느 한쪽에서 바꾸면 다른 쪽도 즉시 같은 값/active 표시로 따라가게 수정. 사건확대 버튼도 3년/5년 버튼처럼 선택 시 파란 active 상태를 유지. 검증: 로컬 JS 파싱 OK, `git diff --check` OK, Playwright DOM 검증 PASS(금융위기 active 양쪽 동기화, 3년 버튼 양쪽 동기화, 사건 active 해제, 정밀 비교 후 도표가 상관계수 아래·정밀 비교 입력 위에 표시, 산점도/레이더 렌더, 콘솔 에러 0). 변경=`templates/risk_return.html`. (Codex)
...
> ⏳ **2026-06-26 업데이트 114 (심화분석 P2 — 분석탭 롤링/분포/낙폭/실질, 미배포):** `/backtest` 결과 아래 "심화 분석 — 언제 시작했든" 섹션 신설. **전체기간·거치식·배당재투자 TR 인덱스** 기반(결정#7, 백테 설정과 별개). **① 기간별 손실확률 표**(horizon 1/3/5/10/15/20y, 손실확률·중앙·최악·최고·표본) **② 오차막대**(box-whisker 커스텀 플러그인, 박스=p25~75/수염=p0~100/중앙값) **③ 롤링 CAGR 다선**(범례 토글) **④ 전체기간 낙폭+회복 통계** **⑥ 명목/실질 토글**(물가율 input, CAGR 디플레이트). ⑤ 배당성장률=기존 차트 충족. **합성 표기**(syn_frac>0.5 회색+ⓘ, syn_overall>0.6 골드배너). 신규 `modules/tr_index.py`(Flask-free 공용 TR빌더, app `_portfolio_index_series` 코어 미러·NULL홀가드) + `backtest_logic.compute_rolling_analysis`(단일+멀티계좌 자본가중). ⚠️**데이터=전체기간+합성 회색(오너결정)** — SPY/TLT 등 합성 1928~ → 헤드라인 MDD -85%(1932 합성)·전행 회색(실데이터한정 대안 기각). 검증=골든 `test_tr_index.py` 4 + gate/d4 회귀 16(metrics 불변) + Playwright 라이트/다크·실질토글·콘솔0. **다음=P3 비교탭 동급화.** 상세→[[log]]. (Claude)

> ⏳ **2026-06-26 업데이트 113 (포트폴리오 예시 — 카드 클릭 상세 모달 통합 + 오색 멀티에셋, 미배포):** 오너 3건. **① 투자대가 상세 페이지 제거→카드 클릭 인라인 모달**: `guru_detail.html`·`gurus.html` 삭제, `/gurus`·`/gurus/<slug>`→`/examples?tab=guru` **301 리다이렉트**(홈프로모·북마크 보존). `examples` 라우트가 `get_guru(slug,25)` 전체상세를 카드 data-*에 동봉. **② 미국·한국 예시 카드도 클릭→상세 모달**: 단일 `#exModal`(examples.js) — 예시=구성바+범례+설명, 대가=13F면책+공시메타+보유25행표. 모달 하단 액션3종=활성카드 대상 기존 핸드오프 재사용, 닫기 X·백드롭·Esc, `?tab=guru` 딥링크. **③ 오색 멀티에셋(짬뽕) 신규**(US·KR defensive 최상단, 배당·성장·금·채권·현금 올인원, 합100·risk5). 검증=Playwright 라이트/다크 — US/KR 오색카드·예시모달·대가모달(25행)·비교바·X/Esc·딥링크 PASS, **콘솔에러0**. ⚠️잔여=`guru_fav.js`+`/api/gurus/<slug>/portfolio`·`/backtest` 고아(보존, 오너 정리판단). 변경=`app.py`·`portfolio_examples.json`·`examples.html`·`examples.js`. **다음=오너 push.** 상세→[[log]]. (Claude)

> ✅/⏸ **2026-06-25 업데이트 112 (투자대가 13F 포트폴리오 P1·P2·P4 + 웹홈 정리):** 신규 시장탭 **"투자대가의 포트폴리오"**(`/gurus`) — SEC 13F 공시 기반 대가 10명(Buffett~Burry, 낙관↔비관 stance순). **P1 데이터**(c991bcc): `modules/gurus/`(EDGAR 13F파서·OpenFIGI CUSIP→티커) + `data/meta/guru_holdings.db` seed + 파서 테스트 PASS, Buffett AAPL22% 실제일치, 2년↑미공시 stale 자동제외. **P2 페이지**(0cd178c): 목록(모노그램·스펙트럼·한계배너·공시일·미니바)+상세(보유테이블), 시장메뉴 추가. **P4 홈카드**(a6d4c93): 시장지수 위 진입 프로모(웹·앱). 전부 Playwright 라이트/다크/모바일 검증·콘솔에러0. **잔여 P3**(상세 백테연결, 버튼 현재 disabled "준비중")·**P5**(분기 자동갱신+새공시 알림, ⚠️prod/CI에 `OPENFIGI_API_KEY` 시크릿 등록 필요·로컬.env완료). 별건: 웹홈 macro카드 확대·히어로 웹노출(로고주황)·CTA제거·거시탭 데스크톱노출. (Claude)

> ✅/📋 **2026-06-25 업데이트 111 (OAuth 계정선택 + 출시준비 플랜 + mobile/ 소스추적):** ① **OAuth `prompt=select_account`**(982f103 배포): 로그인마다 계정선택창 → 다른 계정 전환 가능(전엔 단일계정 자동선택돼 전환불가). ② **출시준비 플랜 신설** `모바일홈_출시준비_plan.md`: 모바일 비로그인 홈=**압축 3도어**(자산관리·리밸런싱알림·계산시뮬, 로그인카드 제거, 데스크톱 불변)/첫실행 스와이프 튜토리얼4장/앱 다운로드 프로모(웹전용 `html:not(.is-app)`, 데스크톱QR·모바일웹배너)/스플래시 웹 오버레이 SVG애니/Play에셋(Playwright 스크린샷 자동생성+설명문). 정체성=이정표 여정앱·자산배분 페르소나. **앱/웹 분기=`is-app` 클라클래스(서버 UA구분 불가).** ③ **mobile/ Capacitor 소스 git 추적시작**(bc294ad, 94파일), google-services.json은 .gitignore(공개레포 노출방지). **다음 착수=홈 3도어** (작업순서·튜토리얼범위·에셋위치 오너 결정대기). (Claude)

> ⏳ **2026-06-25 업데이트 110 (앱 로그아웃 안 풀리는 버그 픽스 — 테마토글 시 로그인 부활, 미배포):** 오너 신뢰박살 버그. **뿌리=stateless 서명쿠키**: `/auth/logout`이 빈 세션→Flask가 쿠키 *삭제*(만료) 전송하는데 **Android WebView가 삭제를 무시**→옛 서명쿠키 살아남고 테마토글 `location.reload()`가 재전송→재인증→부활(브라우저는 삭제 먹어 무증상=앱전용). **수정 A(최소)**: 로그아웃서 `session['_lo']=1` 더미 넣어 user_id없는 새 세션으로 **'덮어쓰기'** 강제(WebView는 삭제는 씹어도 쓰기는 먹음)+no-store, 스키마변경0. 검증=로컬 HTTP 로그아웃 Set-Cookie 새값(덮어쓰기 확정)·`/api/me` logged_in:false·브라우저 회귀0. ⚠️실기기 chrome://inspect 최종확인→안되면 B(users.token_version 서버무효화) 승격. **서버측 수정=앱 재빌드0, 배포만으로 반영.** 변경=app.py(logout). 홈정비=오너 보류. 상세→[[log]]. (Claude)

> ⏳ **2026-06-24 업데이트 109 (리밸 밴드 계정저장 + 알림 모바일이동 + 알림 오버플로우, 미배포):** 오너 4건. **①리밸 경고밴드 계정저장**(전엔 로컬변수→리셋): `user_settings.rebal_band`, settings POST 키병합(hide_amounts 클로버 방지)·data 응답 반환, 프론트 loadAll 복원+슬라이더 onchange/프리셋 saveRebalBand. **②그룹 미설정 리밸=정상**(오류/빈칸 아닌 유도 안내문, 보고만). **③알림 설정 버튼 모바일=풀페이지 `/alerts` 이동**(바텀시트 부실→오너), 데스크톱은 모달 유지(`openAlertSettings` _isMobile 분기). **④알림 페이지 모바일 가로 오버플로우 픽스**(긴 종목명/코드 360px서 218px 재현): `alerts.html` overflow-wrap:anywhere+pill flex-shrink+field min-width/select max-width → 218→0. 변경=`app.py`·`myassets.html`·`alerts.html`, Playwright 검증(밴드 리로드유지·알림이동·오버플로우0·6탭회귀0·에러0). **다음=오너 push.** 상세 → [[log]]. (Claude)

> ⏳ **2026-06-24 업데이트 108 (내 자산 모바일 6탭 재구성 — 도미노식, 모바일/앱 전용, 미배포):** 오너 "모바일서 배당·캘린더·비중 한 화면에 너무 많다 → 도미노 투자관리앱처럼 탭 분리". **모바일(≤768px)·앱만, 데스크톱 웹은 그대로**(오너 결정). 6탭=**현황**(금액가리기+히어로·추이+기여도)·**종목**(보유 hold-card 펼침수정)·**비중**(자산구성 도넛+요약)·**배당**(차트+세전세후+캘린더)·**리밸**(리밸런싱+추가매수 합침)·**그룹**. 원리=`myassets.html` 단일파일 반응형 두 얼굴: 카드 9곳 `data-mtab`+모바일 탭바(`.ma-mtabs`/`mShowTab`), **인라인 display 직접 제어**(class+`!important`는 `#id`에 짐), 데스크톱은 `_isMobile()` 분기로 기존 `showTab`/4탭/긴스크롤 100% 보존+`showTab→mShowTab` 위임. **버그=숨김탭 차트 캔버스0**(비중 도넛·배당) → 탭 노출 시 `renderSummary()`/`renderDivYear()` 재렌더(`_mReady` short-circuit으로 TDZ 회피). 검증=Playwright 390 라이트/다크 6탭 실클릭(자기카드만·overflow0)+인터랙션(펼침수정·세후·슬라이더·추가매수·모달불투명)+데스크톱 회귀0+콘솔에러0. **다음=오너 확인 후 push(자동배포)+라이브 검증.** 상세 → [[log]]. (Claude)

> ✅ **2026-06-24 업데이트 107 (내 자산 모바일 클린 개편 — 한 줄 카드 + 오늘/기간 토글 + 매수일):** 도미노·더리치 벤치. **1차 시도(기존 테이블에 오늘±·기간셀렉터 욱여넣기)는 모바일에서 더 지저분 → 오너 지적·revert 후 재설계.** ① **보유 종목 한 줄 카드 + 탭 펼침**(`.hold-card`: 평소 `종목\|평가액 / 비중·수익` 한 줄, 탭하면 계좌·수량·현재가·평단·매수일·보유일+수정/현재가/삭제/그룹/상세). 구 9열 테이블 폐기, 페이지 5053→3422px. ② **오늘/기간 토글**(`_metricMode`): 히어로 메트릭 1개만 — 오늘=오늘±(칩숨김)/기간=누적±(칩노출), 행 수익도 토글 따라감(`updateHeroMetric` 단일화). ③ **칩 1주/1개월/3개월/6개월/1년/3년, "전체" 제거**(3년이 데이터상한). 기존 `_compute_portfolio_history` 3년 보유역산 재사용. ④ **오늘±** 백엔드 `_prev_close_krw_map`(price_daily/index_daily 직전종가, US×환율, 수동가 제외). ⑤ **매수일** `holdings.buy_date`+모달 input+보유N일, `quickSetGroup` 보존. 변경=`auth_manager.py`·`app.py`·`myassets.html`(웹전용). 검증=로컬 8종목 Playwright 라이트/다크 모바일·콘솔에러0·페이지높이 3422px. **다음=라이브 검증(오너)**. 상세 → [[log]]. (Claude)

> ✅ **2026-06-23 업데이트 106 (앱 푸시알림 끝→끝 + 하단탭 6개 개편 + 알림UX·모바일 픽스):** 모바일 앱(Capacitor) 집중 세션. **① 앱 푸시(FCM) 끝→끝 완성·실기기 검증**: device_tokens+push_sender(google-auth HTTP v1, 키없음 graceful)+alert_runner 훅+register/unregister/status/disable 엔드포인트, 모바일 @capacitor/push-notifications+local-notifications(포그라운드 시스템알림 재현), 웹 인앱 토스트(웹푸시 미채택), 동의문구(광고아님·철회가능), 설정›계정 철회 토글, 서버키 prod /root/secrets/fcm.json+워커 드롭인env. USB 실기기 백/포그라운드 OS알림 확인. **② 하단탭 6개 개편**(홈·분석·시장·내자산·검색·설정, 알림→상단종): 분석허브(/tools 도구카드 그룹) + 시장(/market 큐레이션 대시보드=내지수+핵심거시+링크) 신설. **③ 알림 "새 알림 만들기" 분리카드**(드롭다운 묻힘 해결). **④ 모바일 픽스**: 동의모달/토스트 탭바 z-index 가림, 햄버거 드로어 하단 패딩, 시장 가로 오버플로우(minmax grid), 거시 한·미비교·겹쳐보기 검색창밑 카드 노출+딥링크. 커밋 69008c7·912425e·ead5c60·b696887·833284d·565df58·adad986·8410f32·49c50f0·6476379 전부 배포. **다음=Play 출시 잔여(백버튼·아이콘1024·법무·서명) or 계산기 기능중심 리네임**. 상세 → [[log]]·[[project-push-notifications]]·[[project-mobile-app-capacitor]]. (Claude)

> ✅ **2026-06-21 업데이트 105 (간편 계산기 확장 + 배당 역산 성능/버그):** ① 배당 재투자 그래프 평가액→**월 배당금(명목/실질)**. ② **배당 목표 역산 탭 신규**(간편판 등위선): 기본가정 + (초기금×월적립) 2축 동적 칸(＋추가/×삭제, 원 단위) → 목표 달성 년수 초기금별 등위선+표, 사용법 안내. ③ 배당 역산 **속도 최적화**(월변환 전체월별 캐시·통계/MVN피팅 캐시: 79→27s, 퍼센타일 5%이내·오너허용. SimulationLoop 미세최적화는 효과0 입증·롤백). ④ **k=1 MVN 크래시 픽스**(단일종목 corrcoef 0-d→fill_diagonal, 배당·인출·가구 3곳 가드). ⑤ "초기금 무관" 신고=버그아님(프론트 기본 range 0~2천만 작아서, 백엔드 정상). ⑥ **`VECTORIZATION_PLAN.md`**(멀티 포함 벡터화 단계 P0~P4, numpy 경로축+Numba, C비채택). 커밋 80b0b80·c6cfcb9·9046b7c·741a77c·a24a5f5·94c9edc 배포. **다음=벡터화 착수(오너 케이스 MC/실측 확인 후) or 라이브검증**. 상세 → [[log]]. (Claude)

> ✅ **2026-06-21 업데이트 104 (ENGINE — 몬테카를로(MVN) 플랜 종결, P1~P4 전부):** 오너 "나머지 싹 다". `MONTECARLO_PLAN.md` 완료. **P1 은퇴 인출**(기완료) + **P2 배당**(단일 `_run_mvn_div_cases` — 종목별 mu/sigma+상관 다변량-t 풀호라이즌+분기배당, 구 단일종목 GBM 대체. probe SCHD60/QQQ40: 구 975/994/1014만(분포없음)→MVN 440/928/1687만 spread 32배) + **멀티계좌**(인출 `_build_household_mvn` 상관 cholesky+분기배당·구 무상관GBM 폴백 보존, 배당 계좌별 합산. probe 상관 offdiag 0.023·배당 SCHD3.27%/QQQ0.76%) + **P3 검증**(계산기·축적=AccumulationAnalyzer MVN ✅; **백테=단일기간 1회 시뮬, 분포 안 만듦→MVN 불요**) + **P4 검토**(drift cap 0.0065=8.1%/yr 보수적 적정, 표본수 인출200/SIM60 충분, 현 설정 유지) + **P1 버그픽스**(`_run_mvn_cases` loader 가드, full pytest 미실행으로 깨진 test_wd_synthetic_fallback fake 시그니처 맞춤). **검증**: MC+배당 타겟 27 passed. 커밋 8057c67·1e29804 배포. **다음=라이브 검증(오너)**. 상세 → [[log]]·MONTECARLO_PLAN.md. (Claude)

> ✅ **2026-06-21 업데이트 103 (UX — 종목검색 전면 개편 + 설정 통합/재디자인):** 오너 후속 다건. **검색**: ① **서버사이드 페이지네이션+필터** — `/api/search`에 page/per/cats 파라미터, paged면 전체매칭(universe 1e7=무제한)서 서버필터→페이지 슬라이스, **보이는 18개만 가격조회**(`page`없으면 기존 list 반환=nav·계산기 호환). 프론트 `spFetch(page)`. "s"=9059개·504페이지, 국내필터 61개. ② **페이지네이션 UI** 18개/페이지·10단위그룹(« 처음/‹ 이전묶음/1~10/› 다음묶음/» 끝). ③ **랭킹 수정**: `results[:20]` 하드코딩 슬라이스 제거(긴 ETF이름 잘림), 코드(티커)prefix>이름prefix+짧은코드 우선(sp→SPY4위·spy→1위), 단어시작 티어로 ETF surfacing(삼성→KODEX삼성그룹). ④ **버그**: 검색바 폭변동(grid-column 1/-1 전폭+900), 빈칸 도배(spSeq in-flight 가드), 다중선택 필터(국내주식/국내ETF/해외주식/해외ETF/크립토). **설정**: ⑤ **세금 설정 탭 제거→설정에 편입**(세금폼 전체 이식, `/tax-settings`→`/settings` 302, ID·`/api/settings/tax` 보존). ⑥ 세그먼트 nav(세금/홈화면/캘린더). ⑦ **캘린더 상위↔하위 동기**(소스 체크→하위종목 동기, 하위변경→소스 indeterminate). ⑧ **가로 재디자인**(세금 카드 2~3열 그리드, 캘린더 raw체크박스→set-chk 칩토글, 세로밴드 탈피). + 내자산 종목클릭→상세. **검증**: test_client(paged/cats/legacy)·Playwright(폭·페이지이동·국내필터·캘린더동기·세금폼·콘솔0)·스샷. 커밋 555a181→4f5d1aa 배포. **다음=라이브 새로고침 실검증(오너)**. ⚠️로컬: 죽지않는 python이 :5000 점유→`Get-NetTCPConnection -LocalPort 5000`로 PID 죽일 것. 상세 → [[log]]. (Claude)
...
> ✅ **2026-06-19 업데이트 100 (UX — calculator 입력화면 "조잡함" 정리: 고급옵션 접기 + 삐져나옴 봉쇄 + 종목줄 backtest결):** 오너 라이브 피드백 = "입력화면이 조잡·불편, 누르다 보면 칸이 삐져나옴". 4건 처리(`templates/calculator.html`+`static/js/calculator.js`+`static/js/multi_account_ui.js`). **A 점진적 공개**: 첫 화면 = 종목·금액·기간·리밸·배당만, **거래수수료·세금·가상데이터를 「고급 옵션 ▾」 접기 카드로 격리**(기본 접힘, `calcToggleAdvanced`/`advBody.open`, 세금/수수료 ON 복원 시 자동 펼침). **B 삐져나옴 원천봉쇄**: 입력 grid/flex child에 `min-width:0`/`box-sizing` + 계좌카드 금액 grid `1fr 1fr`→`minmax(0,1fr)`(multi_account_ui, 4탭 공통 개선 — min-width:auto 오버플로우 정석픽스). **C 종목 줄**: 옛 마크업(찌그러진 슬라이더)→backtest 결(`.ticker-badge`+`.ticker-name`+`.ticker-weight-input`+`.ticker-weight-slider`+`.ticker-remove-btn`). **D**: 옵션 그룹 구분선·간격 정리. 캐시 `?v=20260619ds2`(calculator.js·multi_account_ui.js). **검증**(로컬+Playwright 실클릭): 고급옵션 기본 접힘(수수료/세금 숨김)·펼침 동작·종목줄 신마크업·**삐져나옴 0**(데스크톱 1280 + 모바일 390, 세금ON+수수료ON+계좌2 최대부하서 `scrollWidth==clientWidth`)·라이트/다크·콘솔0·dialog0. **✅ 배포(d13729b)·라이브 검증 PASS**(moneymilestone.duckdns.org: 접힘 기본·종목줄 신마크업·펼침·데스크톱+모바일390 오버플로우0·실시뮬 p50 ₩5,639만·콘솔0·dialog0). **다음=dividend_target·retirement 동일 이식.** 상세 → [[디자인통일_plan]]. (Claude)

> ✅ **2026-06-19 업데이트 99 (UX — 투자 계산기(calculator) 디자인을 포트폴리오 분석(backtest) 탭에 통일):** 오너 지시 = "calculator 디자인을 포트폴리오 분석 탭과 일치하게, 수수료 설정·화면 레이아웃 등 거의 모든 디자인적 느낌을 그대로 이식(결과·데이터 로직 제외)". `templates/calculator.html` 전면 재구성 + `static/js/calculator.js` 배선. **레이아웃**: 좌입력/우결과 2단 → backtest 결 **전폭 중앙(`.bt-page`) + 입력뷰↔결과뷰 전환**(`calcShowInput/calcShowResults/calcEditConditions`, 실행 시 결과가 대체, 상단 ←조건수정/재실행). 입력 카드 = `.bt-fcard`(ds-canvas·hairline·shadow·dark-el), bt-input-grid 영역배치(종목 전폭 / 투자금액·투자옵션 2열 / 세금 전폭). **수수료**: 인라인 select(키움/삼성/토스 하드코딩) → backtest와 **동일한 `.bt-switch` ON/OFF + `.bt-fee-panel` + broker_fee_presets.json(8개사 9옵션) + `.bt-fee-market` 시장세그(국내주식/ETF/미국)** 공유. **세금토글** ds 토큰화. **실행버튼** `.bt-run` pill+nicon(zap). **결과**: dist-cards 3개 → `.bt-hero`(중간값 대형 모노숫자 + 비관/낙관 sub) + `.bt-cond` 조건요약 바. 이모지 전면 nicon(calc/search/zap/refresh/plus/repeat/trend-down/warn/compare/link/image). 레거시토큰→ds. alert→mmToast(7곳). 차트색(롤링 green/red→up/down, 부채꼴 blue→brand) ds 바인딩(`_ccCss/_ccRgba`). 기존 결과 카드(롤링·부채꼴·기여도·히스토그램10)·모든 JS 참조 ID·계산 로직·multi_account/favorites/limit_guard 배선 **100% 보존**. 캐시 `?v=20260619ds`. **검증**(로컬서버+mint_session+Playwright 실클릭, ⚠️Redis 다운으로 실시뮬 불가 → 가짜 결과 주입으로 결과뷰 렌더 확인): 입력뷰(5 bt-fcard)·종목검색(20건)·2종목 비중100%·밴드 토글·수수료 ON+프리셋9+미국주식 0.25%+meta갱신·세금 ON+계좌1·결과주입→뷰전환·히어로 ₩3억(color ds-ink #0a0b0d 확인)·조건요약·롤링·히스토그램10·뒤로가기·라이트/다크·**콘솔에러0·네이티브dialog0**. **✅ 배포(5ccaf93)·라이브 실시뮬 검증 PASS** — moneymilestone.duckdns.org/calculator에서 SPY/TLT 50:50·5년·수수료 ON 실제 시뮬 실행: 히어로 중간값 ₩5,639만(비관 4,365만/낙관 8,251만)·조건요약(수수료 국내주식 0.015% 포함)·롤링·부채꼴·기여도·히스토그램10·**총수수료 요약**·입력↔결과 전환·라이트/다크·콘솔에러0·네이티브dialog0. **다음 = dividend_target·retirement(폼 무거운군) 동일 이식.** 상세 → [[디자인통일_plan]]. (Claude)

> ✅ **2026-06-19 업데이트 98 (UX/DATA — backtest 수수료 설정 정리 + 증권사 수수료 프리셋 + 내 자산 전체자산 히어로):** 오너 추가 요청 반영. **거래수수료**를 세금설정 토글과 같은 ON/OFF 스위치 결로 바꾸고, 수수료 패널을 `증권사 프리셋 → 시장 구분(국내주식/국내 ETF/미국주식) → 수수료율` 단일 컬럼으로 재정리해 좁은 우측 카드에서도 넘치지 않게 고정. 주요 증권사 8곳(키움/미래에셋/한국투자/삼성/KB/신한/토스/NH나무) 대표 온라인 국내·해외 수수료율을 `static/data/broker_fee_presets.json`으로 저장하고 backtest 및 다중계좌 카드 수수료 프리셋이 이 JSON을 공유하도록 배선. **내 자산**은 금액가리기와 보유종목 사이에 홈 카드 결의 `전체 자산 + 기간별 자산추이 그래프 + 기간 수익률`을 통합 배치. 검증: JSON parse, JS syntax, Python compile, Flask test-client `/myassets` 로그인 DOM 순서, static JSON 200, backtest 데스크톱/모바일 브라우저 overflow 0, 수수료 프리셋 시장 전환, 콘솔에러 0. (Codex)

> ✅ **2026-06-19 업데이트 97 (HOTFIX — backtest 입력 레이아웃 오버플로우/빈 공간 수습):** 직전 #96 입력 grid 변경 후 오너 스샷에서 `월 적립금` 입력이 카드 밖으로 삐져나오고 좌측에 큰 빈 공간이 생기는 회귀 확인. 원인 1: `.main-content`가 grid인데 `.bt-page`가 `grid-column: 1 / -1`을 받지 않아 첫 컬럼 폭(로컬 699px)에 갇힘. 원인 2: `bt-grid2`가 `1fr 1fr` + grid item 기본 `min-width:auto`라 금액 input intrinsic width가 카드 밖으로 밀림. 수정: `.bt-page` 전폭 grid span + `width:100%; box-sizing:border-box`, 입력 grid는 `assets assets / money options / synthetic options / tax tax`로 재배치, 금액 grid는 `minmax(0,1fr)`와 `min-width:0` 방어. 검증: 1280/2048/390px Playwright 스샷, DOM 좌표 overflow 0, 모바일 `scrollWidth==clientWidth`, 콘솔에러 0, `tests/test_price_loader_spikes.py` 2 PASS. (Codex)

> ✅ **2026-06-19 업데이트 96 (BUGFIX/UX — backtest 입력 블록 정리 + 세션 한정 결과 복원 + SPY/TLT 가격 이상치 급등 방어):** 오너 피드백 반영. **입력 초기조건 화면**을 `grid-template-areas`로 재정렬해 종목 구성은 좌측 큰 블록, 기간·투자금/투자옵션/가상 데이터는 우측 스택, 세금은 하단 전폭으로 고정하고 실행 블록도 동일 카드 결로 정리. **결과 기억 기능**은 `localStorage`→`sessionStorage`로 낮춰 같은 탭 이동/새로고침 중에는 유지하되 브라우저·탭 재시작 후에는 복원되지 않게 변경, 기존 `mm_task_backtest`/`mm_result_backtest` localStorage 키는 진입 시 제거. **SPY 50%/TLT 50% 가치추이 급등** 원인은 `price_daily`의 SPY 2026-06-17 단일일 오염값(`close=346500`)이 전후일 750달러대와 섞인 것. `PriceLoader.get_price()`에서 전후일이 같은 스케일인데 하루만 25배 이상 튀었다 즉시 복귀하는 고립 이상치를 FX 적용 전 제거. 검증: `tests/test_price_loader_spikes.py` 2 PASS, 직접 `run_backtest_logic` none/band 정상, 서버 `/api/backtest/run` band end≈5,369만·max≈5,398만, 브라우저 라이트/다크+모바일 스샷·검색 실클릭·콘솔에러 0, stale localStorage 제거 Playwright PASS. (Codex)
...
## 2026-07-09 겹쳐보기 종목 동결 수정 + 백테 가치 차트 UX

- **추세 겹쳐보기 `SYM:` 종목 데이터 동결 수정**: `_portfolio_index_series`는 price_daily를 직독하고 `ensure_full_history`(최초 1회 백필 후 플래그 no-op)만 있어서, 백테스트 등 다른 플로우가 안 건드리는 종목(^IXIC·^GSPC 같은 지수)은 최초 사용일에 데이터가 동결됐다(prod 2026-04-01, 로컬 ^GSPC 2024-12-30 재현). 백필 루프에 트레일링 갱신 추가 — 플래그된 종목의 DB 마지막 날짜가 오늘보다 과거면 `get_price(code, db_max, 오늘)` 호출(기존 P2-3 코드별 하루 1회 dedupe가 중복 차단), 새 행 들어오면 `_TS_TKCACHE` 무효화. 지수뿐 아니라 저장 포폴(PF:)·예시(EX:) 안의 방치 종목 동결도 함께 해소.
- **백테 가치 추이 차트**: ① y축 `beginAtZero`(오너 요청 — 데이터 최소값 맞춤이 아니라 0부터 전체 스케일). ② 드래그 줌 복귀 수단 부재 해소 — 가치·낙폭 카드 제목에 `↺ 전체 기간` 버튼(줌 시에만 노출, `btZoomResetBtns`) + 캔버스 더블클릭 리셋. 기존 `btAttrReset` 재사용.
- 검증: 동결 재현 후 트레일링 갱신(2024-12-30→2026-07-08, API 끝→끝) + Playwright 12 PASS(y축 0·버튼 노출/숨김·리셋·더블클릭·낙폭·다크·jsErr 0) + `tests/test_portfolio_compare_route.py` 5 PASS.

## 2026-07-09 포트폴리오 비교 — 부동산 거시지표 추가 가능화
...
## 2026-07-21 심화 롤링 가짜 구성 수정 ✅ / 공유 기능 부분 수정 🔶

**① 심화 롤링(26751fe) ✅ 코드 완료 · ⚠️prod 검증 미완**
- `build_portfolio_tr_index`가 전 구성종목 데이터 존재일부터 시작하도록 절단. 결측 종목 비중
  재정규화로 "50/25/25인데 실제론 TLT 100%" 구간이 통계에 섞이던 문제 제거.
- 오너 결정: 심화표를 **사용자 선택 기간에 맞추는 안(A)은 미채택**. 심화표는 계속 전체 가용기간
  기준 → 항목별 기간 불일치는 알려진 잔여 한계. **재제안 전 오너 확인**.
- 다음: 배포 후 prod에서 `^NDX/GLD/TLT` 심화 시작일 2004-11-18 · 1년 손실확률 24.6%→12.1% 확인.

**② 공유(링크복사·이미지저장) 🔶 절반**
- backtest 죽은 셀렉터(`.bt-layout`→`#btResultContent`) 수정·푸시(5b31f09), prod 실클릭 미검증.
- 링크 복사 413 = **nginx `client_max_body_size` 미설정(기본 1MB)** → **오너가 직접 실행해야 함**
  (Claude 권한 차단). 명령 = [[../log]] 2026-07-21.
- risk-return 공유는 `{% if user %}` 뒤라 비로그인 prod에서 미검증 → 로컬 + mint_session 필요.

**③ 미착수 (오너 지시)**: DB 결측 티커가 조용히 빠지고 비중 재정규화되는 문제를 사용자에게 노출.
  상세 = [[bugs]] BUG-TR-MISSING-TICKER-SILENT.

상세 = [[../log]] 2026-07-21.

## 2026-08-05 적립식 백테스트 지표 오염 수정 ✅

- 포트폴리오 분석(백테스트) 적립식에서 연간수익률·MDD·Sharpe가 **잔고 기준**이라 납입금 증가를
  수익으로 계산하던 버그 수정 → TWR(시간가중) 지수 기준으로 전환. CAGR은 적립식이면 MWR(금액가중).
- 총 납입금 근사식 → 실제 `cash_flow` 합계. 단일/멀티계좌 경로 `_perf_block`으로 통합.
- 상세 = `dev/bugs.md` 2026-08-05 항목, `log.md` 2026-08-05.

> 2026-08-05 업데이트 2: **등락색 한국식 전환 완료**(상승=적 `#e0342c`, 하락=청 `#1565d8`
> / 다크 `#ff6b6b`·`#4d9bff`). 단순 flip이 아니라 **방향색(`--up`/`--down`)과 상태색
> (`--ok`/`--danger`) 분리**가 핵심 — 기존엔 에러·삭제·취소 버튼이 `--down`, 성공 토스트·
> 절세액이 `--up`이라 그대로 뒤집으면 에러가 파랑, 성공이 빨강이 됐다. 레거시 `--green*`/
> `--red*` 별칭은 이제 `--ok`/`--danger`를 가리킨다. 캔들·거래량·스파크라인·차트 폴백 상수
> 등 하드코딩 잔재도 전수 교체. 검증 = 신규 `tests/test_direction_colors.js` **204 PASS**,
...
> ⚠️ 2026-08-06 **범위 축소(오너 재지시)**: 한국식 등락색은 **캔들차트에만** 남기고 나머지는
> 원복. `--up`/`--down` = 녹/적(전환 이전 값)으로 되돌림. 한국식 적/청은 이제 캔들 시리즈 +
> 그 아래 거래량 히스토그램 하드코딩 상수(`symbol_page.js`·`macro.js`)뿐 —
> **토큰이 아니라 의도적 예외**다. 방향색/상태색 토큰 분리는 그대로 유지(값만 원복).
> 상세 = `log.md` 2026-08-06.
