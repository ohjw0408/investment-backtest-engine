...
> ✅ **2026-06-14 업데이트 77 (저장 포트폴리오 상세 — 수량입력→비중·추이·배당 + 내자산 배당일정 월별칸·홍보문구 강조):** 오너 요청 3건. ① "내 포트폴리오" 저장 카드 클릭 → **내자산 같은 상세 페이지**(`/myportfolios/<id>`, `portfolio_detail.html`). 종목별 **수량 입력** → 비중 파이(종목별)·자산 추이(1M/3M/1Y/전체)·배당금(세전후·원외화·연도탭·막대드릴·**월별 네모칸**·일정 리스트). ② 내자산 배당 CTA(백테/계산기 유도) 너무 작음 → `#divCta` **강조 박스**(0.92rem·연파랑 배경/테두리·💡, 튀지 않게). ③ 내자산 배당 일정 = **12개월 네모칸**(월 합계 표시) 클릭 시 그 달 종목별 펼침 표(막대 클릭 외 새 진입점). **백엔드(app.py)** = `_compute_portfolio_history(valid)` 헬퍼 추출(기존 `/api/portfolio/history` 재사용) + 신규 `/api/portfolio/item/<id>`·`/compute`(현재가+추이)·`/dividends-preview`(내자산 `build_dividend_chart` 동일 엔진). 수량 = `saved_portfolios.tickers_json`의 `quantity` 필드 **영구 저장**(스키마 무변경), `auth_manager.get_portfolio` 추가·`portfolio_save` quantity 보존. 검증 = venv test_client+세션주입: item/상세페이지/수량 저장 라운드트립(3→5 영구)·compute(price>0·hist keys)·dividends-preview(years·events) 전부 200 PASS, 기존 history 회귀 200 PASS, JS `node --check` 3파일 OK. ⚠️ **실브라우저(Playwright) 검증 미수행 — 로그인 E2E 필요. 미배포(커밋 대기).** **▶ 다음 = 실브라우저 검증·배포 후 PHASE4 잔여(D1·D2·C1·C2·B4) — 오너 결정.**
