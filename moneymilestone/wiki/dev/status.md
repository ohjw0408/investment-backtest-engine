...
> ✅ **2026-06-15 업데이트 85 (거시경제 지표 탭 — 신규 기능, Step 1·2·4/6, 배포·라이브검증 완료, 0df9740):** 신규 플랜 `거시지표_캘린더_plan.md`(거시지표 탭 + 증시 캘린더, 알림 제외=리밸런싱 알림 때 같이). **데이터:** `modules/macro_loader.py` — SERIES 레지스트리 **90종**(미 FRED 66 + 한 ECOS 24) + `fetch_fred`(공식 API 키)·`fetch_ecos`(StatisticSearch)·`backfill`·`validate`·`backfill_if_empty`(배포 멱등 훅) + 신규 테이블 `macro_series`/`macro_observations`(index_master.db). 전 코드 라이브 검증 90/90 적재. **KOSIS 불필요 확정**(한국 고용·산업생산·선행지수·주택가격·BSI·CSI 전부 ECOS), 제외=GOLDAMGBD228NLBM(폐지)·USALOLITONOSTSAM(stale). **UI:** `/macro`(국가토글 US/KR/비교 + 카테고리섹션 + 카드그리드, 카드=값·등락·SVG스파크, 클릭→상세 전체시계열 Chart.js+기간토글, **한미비교=단위자동: %·%p 원값 / 그외 시작=100 정규화**) + `templates/macro.html`·`static/js/macro.js`·`/api/macro/{overview,series/<code>,compare}`·nav/사이드바("🌐 거시지표"). **배포 안전:** deploy.yml에 `--ensure`(서버 테이블 비었을 때만 최초 백필, 키 서버보유, `||true`) + 읽기경로 ensure_schema(빈테이블 크래시 방지). 검증=라이브 엔드포인트 4종 + Playwright(US 67·KR 23카드·상세/비교 캔버스·12쌍·콘솔에러 0). **오너 피드백 후속:** 전체 히스토리(1990캡 제거, 출범부터 394,593행, 서버 자동 재백필), PC 풀폭(`grid-column:1/-1` 6칸/줄), 지표 검색창, **임의 겹쳐보기**(🔬 — 거시90+종목 N≤6 오버레이, 공통시작=100 정규화/2개 2축, `/api/macro/multi`). **▶ 잔여: Step3 beat 자동갱신·Step5 지표설명문(desc 빈값)·Step6 캘린더(실적+발표일).**
