...
> ⏳ **2026-06-24 업데이트 107 (내 자산 오늘±·기간 누적± 셀렉터 + 매수일 — 미배포):** 도미노·더리치 벤치마크. **① 오늘±**(히어로+보유행, 현재가 vs 직전 거래일 종가; 백엔드 `_prev_close_krw_map` = `_search_attach_prices` SQL 재사용, `myassets_data`에 `prev_close`, 수동가격 제외). **② 기간 누적± 셀렉터**(히어로 칩 1주/1개월/3개월/6개월/1년/3년/전체 — 기존 `setHistoryPeriod`+`_compute_portfolio_history` 3년 보유역산 재사용, 한 셀렉터가 차트+누적±₩/% 동시구동; **스냅샷 미구현**=스토리지0·삭제흔적0; 5년은 데이터 3년한계로 보류). **③ 매수일**(`holdings.buy_date` 컬럼+ALTER 마이그레, `upsert_holding`/API/모달 input/보유N일 표시; `quickSetGroup`도 buy_date 보존). 변경=`auth_manager.py`·`app.py`·`myassets.html`(웹전용, Capacitor 자동반영). 검증=로컬 시드 Playwright 라이트/다크 모바일·콘솔에러0. **다음=오너 확인 후 push 배포 + 라이브검증**. 상세 → [[log]]. (Claude)
