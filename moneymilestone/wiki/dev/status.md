...
| 🔴 **금융소득 종합과세 완전 구현** | 엔진 수학 완료. **상세 설계 완료**(Phase 2f + trackG 2-4, 오너 결정 4 반영). 갭 = ① **KR_FOREIGN 실현차익+배당 합산 종합과세**(현 청산 15.4% 분리=핵심) ② `_ytd_income` 주입 ③ 자동산출 ④ 분할매도 슬라이더 전탭 배선 ⑤ 연도별 종합과세 대상 트래킹 → (Track G) 금종세자 ISA 풍차 중단·무한유지·리라우팅 | **다음 작업(구현 대기)** | `금융소득 종합과세 구현해줘` |
