...
> **잔여**: ①**prod 정리 미실행** — 배포 후 `scripts/fix_rate_proxy_backfill.py --apply`(오염=0046A0 1종) ②prod 백필 일괄 스윕은 오너 결정 대기 — 지금은 요청 시 자기치유라 필수는 아님 ③⚠️ **`US_SEMICONDUCTOR` 8종 매핑 보류** — 추종 지수가 종목마다 달라 ^SOX 적용 가능 여부 오너 확인 필요
