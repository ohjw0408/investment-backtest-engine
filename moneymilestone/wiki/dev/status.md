...
> ✅ **2026-06-19 업데이트 100 (UX — calculator 입력화면 "조잡함" 정리: 고급옵션 접기 + 삐져나옴 봉쇄 + 종목줄 backtest결):** 오너 라이브 피드백 = "입력화면이 조잡·불편, 누르다 보면 칸이 삐져나옴". 4건 처리(`templates/calculator.html`+`static/js/calculator.js`+`static/js/multi_account_ui.js`). **A 점진적 공개**: 첫 화면 = 종목·금액·기간·리밸·배당만, **거래수수료·세금·가상데이터를 「고급 옵션 ▾」 접기 카드로 격리**(기본 접힘, `calcToggleAdvanced`/`advBody.open`, 세금/수수료 ON 복원 시 자동 펼침). **B 삐져나옴 원천봉쇄**: 입력 grid/flex child에 `min-width:0`/`box-sizing` + 계좌카드 금액 grid `1fr 1fr`→`minmax(0,1fr)`(multi_account_ui, 4탭 공통 개선 — min-width:auto 오버플로우 정석픽스). **C 종목 줄**: 옛 마크업(찌그러진 슬라이더)→backtest 결(`.ticker-badge`+`.ticker-name`+`.ticker-weight-input`+`.ticker-weight-slider`+`.ticker-remove-btn`). **D**: 옵션 그룹 구분선·간격 정리. 캐시 `?v=20260619ds2`(calculator.js·multi_account_ui.js). **검증**(로컬+Playwright 실클릭): 고급옵션 기본 접힘(수수료/세금 숨김)·펼침 동작·종목줄 신마크업·**삐져나옴 0**(데스크톱 1280 + 모바일 390, 세금ON+수수료ON+계좌2 최대부하서 `scrollWidth==clientWidth`)·라이트/다크·콘솔0·dialog0. **다음=커밋·푸시·라이브 확인.** 상세 → [[디자인통일_plan]]. (Claude)
