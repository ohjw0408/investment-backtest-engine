...
> 🟡 **진행중 (2026-06-15): 증시 캘린더 정책일정/한국지표.** ✅ **Tier 1 완료** = US FOMC + KR 금통위 결정일 큐레이션(`policy_meetings.json`). ⏳ **Tier 2 = 한국 경제지표 발표일** — PoC 결과: 깨끗한 API 없음. KOSIS 공표일정=月 단위만(부족), **kostat.go.kr "주요통계 공표일정"에 정확 日 존재하나 월별 보도자료 HTML 스크래핑만 가능**(유지보수 부담). 스크래퍼 구현 여부 = **오너 결정 대기**.
