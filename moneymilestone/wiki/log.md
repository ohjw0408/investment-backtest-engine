...
리밸런싱하는 경우**가 어느 탭에도 없어 불편하다. → 추가매수 탭에 모드 스위치 2개 추가.

- `구매만`(기본, 기존 동작 그대로): 매도 없이 목표 대비 부족한 그룹에만 신규자금 배분.
- `구매 + 리밸런싱`(신규): 신규자금 투입 **후 모든 그룹이 목표 비중에 정확히 도달**하도록 계산.
  초과 그룹은 매도까지 제안한다.

**계산**(`static/js/myassets_page.js` `calcPurchase`)
```
newTotal = Σ그룹평가액 + 투입금액
delta_i  = newTotal * w_i − 그룹평가액_i      // 음수 = 매도
Σ delta_i = 투입금액                          // 항상 성립
```
기존 코드가 이미 `newTotal`을 쓰고 있었고 `Math.max(0, …)` 클램프만 매도를 잘라내고 있었다.
rebal 모드는 그 클램프를 풀고, 목표가치를 `allocExact`로 **원 단위 정확 배분**해 잔돈을 없앤다
(그룹 평가액도 먼저 `Math.round` — 안 하면 순투입이 입력액과 1원 어긋난다).

**오너 결정**: 매도 기준 = **목표 정확히 맞춤**. (대안이던 "경고밴드 안이면 매도 안 함"은 미채택 —
거래·세금은 줄지만 목표엔 근사치로만 도달.) 매도에 세금·거래비용이 붙을 수 있다는 경고는 결과
하단 안내문에 넣었다.

**UI** `templates/myassets.html` — `.pur-modes` 칩 2개(`.rb-preset` 톤 재사용), 모드별 안내문
교체(`#purIntroText`), 결과에 매도/매수 라벨 + `매도 합계·매수 합계·순투입` 3행 요약.
매도는 `--down`, 매수는 `--up`/그룹색 [[decision-direction-colors-candle-only]].

**검증**
- `tests/test_purchase_mode_dom.js` (신규, jsdom) **21/21** — 두 모드 금액·라벨, 홀수 금액 라운딩
  (Σ = 입력액), 모드 전환, 가드. `구매만` 결과가 변경 전과 동일한지도 회귀로 고정.
- `tests/test_purchase_mode_browser.js` (신규, Playwright) **25/25** — 실데이터로 매도행 등장/소멸,
  순투입 = 입력액, 매수합−매도합 = 순투입, 칩 재계산 시 모드 유지, 다크 전환, 콘솔 에러 0.
- 스샷 육안: 데스크톱 라이트/다크 + 모바일 390px(가로 오버플로 없음).

⚠️ 작업 중 발견한 **기존 버그**(이번 변경과 무관, 미수정): `savePrivacySetting()`이 금액숨김 토글
시 `calcPurchase()`를 부르지 않아 추가매수 탭이 열려 있으면 마스킹이 갱신되지 않는다.
→ [[bugs]] BUG-HIDEAMT-PURCHASE-STALE.
...
**가장 큰 드리프트: `PROJECT_MASTER_ROADMAP.md`가 2026-06-18에 멈춰 있었다.** 그 사이 커밋 **354건**.
로드맵을 다시 진실원천으로 만들었다 — 06-18 이전 서술은 `<details>`로 접어 **이력**으로 표시하고,
현재 상태(라이브 서비스 + 앱)·2026-06-19 이후 신설 플랜 12종·다음 후보를 새로 작성.

**stale 정정 (실제와 어긋나 있던 것)**
- `divrefactoring.md` = "💡 미착수" → **실제 2026-06-13 통합 완료**(로드맵·wiki index 양쪽).
- `포트폴리오예시_plan` = "⏸ 미배포(push 대기)" → **라이브 배포됨**(prod `/examples` 200 확인).
- `거시지표_캘린더_plan` = "⏳ C3 포폴통합" → **C3 완료**(전체 종결).
- `출시완성도_개선_plan` = "📝 신규·오너 리뷰 대기" → **P0·P1·P2 완료·배포**(잔여를 플랜 머리에 명시).
- `PHASE4_PLAN` E2(코드 속도 최적화) = 미착수 표기 → **`성능최적화_plan` P0~P2로 흡수·완료**.
  같은 파일에 **문자만 같은 다른 항목 주의 표기** 추가(`출시완성도`의 E-1/E-2/E-3와 혼동 방지).
- `기여도_plan` 산출물 중 **내자산 상승견인/하락방어 카드는 2026-07-14에 제거**(종목별 손익으로 대체)
  — 로드맵이 "완료"로만 적어두고 제거 사실을 안 담고 있었다.

**동기화 중 새로 발견한 것 → [[bugs]] BUG-INDEX-DAILY-FROZEN**
`index_daily` 프록시 시계열 **35/37종이 2026-05-28~29에 정지**. 채우는 게 수동 스크립트뿐이고
주기 작업은 USD/KRW·KRX_GOLD만 있다. 오늘 `price_daily`는 `refresh_stale_prices` beat를 얻었지만
**`index_daily`엔 같은 장치가 없다**. 2026-08-04 로그의 "`CORPAA3Y` 2개월 stale, 확인 필요"가
이 문제의 일부였다. 영향은 상장 이전 구간용 프록시라 기존 종목엔 무해하나, **05-29 이후 상장분
백필은 프록시가 모자란다**. 오너 판단 대기 — 다음 후보 1순위로 로드맵에 기록.

_작성: Claude_

## [2026-08-12] bugfix | 종목 가격이 영구히 멈추는 버그(SPCX) + 그룹 탭에서 종목 담기

### ① 가격 갱신 정지 — 원인은 "실패했는데 성공으로 기록한 하루 잠금"

오너 제보: 스페이스X(**SPCX**)가 상장 직후 며칠 데이터만 있고 갱신이 안 됨.
prod 실측 = `price_daily` **5행(2026-06-12~06-18)**, 오늘은 08-12. 두 달 정지.

가격 적재는 전부 **지연(lazy)** — 누가 그 종목을 볼 때 `get_price`가 트레일링 gap-fill을 한다.
P2-3에서 낭비를 줄이려고 넣은 가드가 `_gapfill_trail_day[code] = end_date`를 **페치 전에**
찍었다. 야후가 실패하든 성공하든 "오늘은 시도했음"으로 기록 → 그 워커 프로세스에서
그날 내내 재시도 없음. 검색 페이지가 `/api/watchlist/quotes`로 최대 18종목을 **병렬**
요청하면서 레이트리밋을 맞고, 그 실패가 그날의 유일한 기회를 태워버린 것.
(08-10 접속 로그가 그대로 보여준다: 14:57 quotes 실패 → 14:58 `/api/symbol/SPCX` = 947바이트 stale.)

거기에 두 번째 결함이 겹쳤다 — **페치 예외가 요청 전체를 죽였다.** DB에 멀쩡한 데이터가
있어도 트레일링 페치가 터지면 `get_price`가 예외를 그대로 올려 `_watchlist_quote`가
빈 시세를 negative-cache. 게다가 그 실패는 `except Exception: pass`라 로그에 흔적이 없었다.

**SPCX만의 문제가 아니었다.** prod 1,031종 중 **971종**이 정지(최종일 < 08-01):
07-10·07-13·07-17·07-31에 무더기로 몰려 있다. 전체 `MAX(date)`만 보던 무결성 스캔은
인기 종목 하나만 살아 있어도 통과라 이걸 두 달간 못 잡았다.

**수정 3층**
1. `_trail_should_try`/`_trail_mark` — "시도했다"가 아니라 **"결론이 났다"** 일 때만 하루를 잠근다.
   빈 결과의 의미를 갈랐다: DB 최종일이 5일 이내면 정상(새 거래일 없음, 하루 잠금 유지 = 기존
   절약 그대로), 몇 주씩 벌어져 있으면 실패로 보고 **10분 백오프 후 재시도**.
2. `get_price` 페치 try/except — 실패해도 **DB 보유분은 서빙**하고 실패는 로그로 남긴다.
   `_watchlist_quote`의 조용한 삼킴에도 로그 추가.
3. `PriceLoader.refresh_stale_prices()` + `tasks.refresh_stale_prices` (beat 매일 22:00 UTC) —
   지연 적재의 **유일한 능동 복구 경로**. 정지 종목을 찾아 재페치하고, 상폐 등으로 연속 3회
   실패하면 `price_refresh_state`에 기록해 7일 쿨다운(영구 포기는 안 함).
   `data_integrity_scan`에 **종목별 정지 비율 30%** 경보 추가(전체 MAX만 보던 눈먼 구간 제거).

회귀 `tests/test_price_loader_stale_refresh.py` 4종 — 실패 후 백오프 재시도 / 최신 종목은
하루 1회 유지 / 예외에도 DB 서빙 / 복구+쿨다운.

### ② 그룹 탭에서 보유 종목 담기 (오너 불편 제보)

기존엔 그룹 지정이 **종목 탭 → 수정 모달 → 그룹 선택**뿐이었다. `holdings` 행이
`(code, account_type)` 단위라 같은 종목이 일반·ISA·연금저축에 나뉘어 있으면 **행 수만큼 반복**해야 했다.

그룹 탭에 담기 경로를 신설: 그룹 카드 하단에 **구성 종목 칩**(계좌별 수량 표시, `×`로 즉시 빼기)
+ `+ 종목 담기` 버튼, 상단에 **미분류 N종목 배너**(→ 미분류 필터로 바로 진입).
담기 모달은 **코드 단위 체크박스** — 한 번 체크하면 그 종목의 **모든 계좌 행**이 함께 담긴다.
그룹 선택·검색·필터(전체/미분류만/이 그룹)·일괄 선택 지원.
API `POST /api/myassets/holdings/group` (`codes`=담기, `clear_codes`=이 그룹에서만 빼기) +
`auth_manager.assign_holdings_to_group`(그룹 소유권 검증).
...
오너 재지시: "캔들차트 색만 남기고 나머지는 원래대로. 검색 등에서 코스피·S&P·개별종목
캔들만 빨강/파랑."

전날([2026-08-05] 항목) 전면 전환했던 것을 **캔들차트 예외 하나로 축소**했다.

**되돌림 (상승=녹 / 하락=적)**
- 토큰 값: `--up:#05b169`·`--up-tint:#e6f7ef`·`--down:#cf202f`·`--down-tint:#fdeaec`
  (다크 `#27c281`/`#0f2a20`·`#ff5a5f`/`#2e1518`) — 전환 이전 값 그대로
- 스파크라인(`charts.js` 홈·검색·시장 / `macro.js` 거시 카드), 종목 상세 **라인**차트,
  차트 폴백 상수(`backtest_page.js`·`calculator.js`·`tax_switch.js`), 공유카드(`share.html`)

**유지 (상승=적 / 하락=청)** — 하드코딩 상수, 주석으로 "의도적 예외" 명시
- `symbol_page.js` 캔들 시리즈 + 그 아래 거래량 히스토그램
- `macro.js` 캔들 시리즈 + 거래량

**방향색/상태색 토큰 분리(`--up`/`--down` vs `--ok`/`--danger`)는 유지했다.** 값이 원래
녹/적으로 돌아가 화면은 전환 이전과 동일해지고, 되돌리면 템플릿 24개를 의미 없이 다시
건드려야 한다. 레거시 `--green*`/`--red*` → `--ok`/`--danger` 별칭도 그대로.

**검증**(로컬 렌더, 기대색만 갱신하고 스위트는 재사용)
- `tests/test_direction_colors.js` **204 PASS / 0 FAIL**
- `tests/probe_direction_dom.js` 실렌더 **42건 불일치 0**
- `tests/probe_result_view_colors.js` **28 PASS / 0 FAIL**
- 육안 스샷 라이트/다크: 종목 캔들=적/청 + 거래량도 적/청, 종목 라인=녹/적,
  홈·거시 스파크라인=녹/적, 헤더 등락률=녹/적. `shot_direction_interact` 콘솔 에러 0
- 회귀 `tests/test_layout_safety.js` **164/164 PASS**

캔들은 canvas라 DOM 프로브가 못 잡는다 → 캔들 색 확인은 스샷 육안이 유일한 게이트다.

**배포·prod 검증 완료**: `200af46` 푸시 → 배포 확인(prod `style.css`에 `--up:#05b169`) 후
라이브 재실행 — `test_direction_colors.js` **208 PASS / 0 FAIL**,
`probe_direction_dom.js` **22건 불일치 0**(비로그인이라 표기 수가 로컬보다 적음),
prod 스샷 육안으로 캔들·거래량 적/청 + 헤더 등락률 녹 확인.
