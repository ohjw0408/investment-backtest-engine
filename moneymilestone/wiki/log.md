...
- **후속3(오너 "전체화면에도 달력·사건 UI 다 넣고, 가로화면 안내 문구")**: 전체화면 진입 시 `rrOvCtrl`(달력·기간·사건·모드·기준)를 wrap 안으로 DOM 이동(원위치 기억→종료 시 복원). CSS grid 3행(fullbar/ctrl/canvas), ctrl `max-height:30vh`(가로 40vh) 스크롤. 세로+터치+전체화면에서만 뜨는 회전 안내 `📱↻ 가로로 돌리면 더 편해요`(가로면 `@media(orientation:landscape)`로 자동 숨김). 재검증 17 PASS(컨트롤 이식·달력2·사건4·기간·회전안내·종료복원·라이트/다크·jsErr 0).
