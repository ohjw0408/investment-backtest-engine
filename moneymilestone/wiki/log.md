...
## [2026-08-06] BUGFIX | 1시간봉 타임존 + 시간봉 영구 결손 (오너 제보)

오너 제보: "코스피 1시간봉이 7/13 다음에 7/27이 나온다. 값은 정확하냐?"
→ 실제로 **서로 다른 버그 2개**였다.

### 버그 A — 시간봉 x축이 UTC로 찍힘 (표기 버그)

`price_hourly`는 `yf.download(interval="1h")` 인덱스를 `strftime`으로 저장하는데,
이 인덱스는 **tz-aware UTC**다. tz가 문자열에서 탈락하면서 DB에는 UTC 벽시계가 남는다
(코스피 = `00:00~05:00`, SPY = `13:30~20:30`).

프론트는 그 문자열을 `new Date("2026-07-13T00:00")`로 파싱 → **브라우저 로컬(KST)** 로
해석하고, Lightweight Charts는 `UTCTimestamp`를 **UTC로 렌더**한다. 오차가 두 번 겹쳐
코스피는 `KST − 18시간`으로 찍혔다. 즉 **09:00 봉이 전날 15:00에 그려져 모든 봉이
하루씩 밀렸다.** 미국 종목은 날짜는 안 밀리고 시각만 −5시간.

거시지표(`/macro`)도 같은 증상이되 원인 위치가 달랐다. `macro_loader`는
`Ticker.history`를 쓰는데 이쪽 인덱스는 **거래소 현지시각**이라 저장값은 맞고,
프론트 파싱만 틀려 −9시간이었다.

**수정 — API 계약 통일: `date` = 거래소 현지 벽시계.**

| | 저장 | 서빙 |
|---|---|---|
| `price_loader` (`/api/symbol/*/intraday`) | UTC 고정(`tz_convert("UTC")` 명시) | `_intraday_tz()`로 거래소 tz 변환 + `tz` 필드 |
| `macro_loader` (`/api/macro/intraday/*`) | 거래소 현지(Ticker.history 그대로) | 그대로 |

프론트(`symbol_page.js`·`macro.js`)는 `Date.parse(s + 'Z')` — 벽시계 숫자를 UTC로
고정해 LWC 축에 그대로 찍히게 한다. DST가 있는 시장 때문에 변환은 **봉 단위**로 한다
(EST 09:30 / EDT 09:30 둘 다 회귀 테스트에 있음).

부수 수정: `get_intraday_data`의 조회 경계가 서버 로컬 날짜였는데 컬럼이 UTC라
`utcnow()` 기준으로 맞췄다.

### 버그 B — 방문 공백기가 영구 구멍으로 남음 (오너가 본 "7/13 → 7/27")

`range=max` 경로가 `has_deep`(30일 이전 row 존재 여부)만 보고 분기했다. 한 번 730일
백필이 끝나면 `has_deep`는 **영원히 참**이라 그 뒤로는 `period="7d"` 갱신만 돈다.
사용자가 2주 만에 다시 들어오면 그 사이 구간을 아무도 안 받아오고, 다음 방문에도
복구 트리거가 없어 **구멍이 영구화**된다. 실제 로컬 DB에서 확인:

- `^KS11` 2026-06-23 → 2026-07-28 (35일 결손)
- `SPY` 2026-06-12 → 2026-07-24 (42일 결손)

**수정**: `_has_intraday_hole()` — 결손이면 730d 재조회. 판정은 신호 2개의 OR다.

1. **일봉 대조** — `price_daily`에 있는 거래일이 시간봉에 없으면 결손. 연휴를 임계로
   추측할 필요가 없어 정확하다. 단 일봉이 stale하면 아무것도 못 잡는다.
2. **간격 백스톱** — 연속 시간봉 날짜 간격이 `INTRADAY_MAX_CLOSURE_DAYS=8`을 넘으면 결손.
   8일 = 관측된 실제 최장 휴장(2025 추석+개천절+한글날, 10/02→10/10). 1이 눈 먼 구간용.

처음엔 임계 하나(10일)로 갔다가 **prod 실측에서 정확히 10일짜리 진짜 구멍**
(`2026-06-26 → 07-06`)이 잡히지 않는 걸 보고 갈랐다. 야후가 하루치만 빠뜨리는 경우가
있어 1번은 2일까지 허용한다(아니면 방문마다 730일 재조회가 돈다). 무한 재조회 방지로
코드별 6시간 레이트리밋(`_intraday_fetch_ok(code + ":deep", 21600)`).

로컬에 prod와 같은 구멍(07-14~07-27)을 지워 재현 → 감지 True → 재조회 → 7월 거래일 전부
복구 → 감지 False 확인. 남은 5일 이상 공백은 전부 실제 휴장(추석·설·연휴)이었다.

### 검증

- `tests/test_symbol_api.py` 13 PASS — tz 회귀 4케이스(KST·EDT·EST) + 결손감지 3케이스, 무네트워크
- Playwright: `/symbol/^KS11` 라인 1일/1주 + 캔들 1시간~1년, `/macro` S&P500 1시간,
  라이트·다크, 콘솔에러 0. 축이 코스피 `09:00~14:00`, SPY·S&P500 `09:30~15:30`으로 찍힘

### 값 자체는 정확한가 (오너 질문)

봉 값(OHLCV)은 야후 원본 그대로다. 다만 **인베스팅닷컴과 봉 모양이 다를 수 있다**:
야후 KRX 1시간봉은 하루 **6봉(09:00~14:00)** 이라 마지막 14:00 봉이
**14:00~15:30(90분, 종가 단일가 포함)** 을 통째로 담는다. 15:00대를 별도 봉으로 쪼개는
제공사와는 마지막 봉의 고저·몸통이 구조적으로 다르다. 시세 소스도 서로 달라
개장 초 오차가 있다.
...
20종 전부 **만기매칭형 회사채 ETF**(`TIGER 26-04 회사채`·`ACE 2년만기자동연장`류)다. prod 실측:

| 항목 | 값 |
|---|---|
| 합성 std | 0.00305~0.00425 (일반 채권 ETF 0.002~0.0037과 같은 수준) |
| 실데이터 std | **0.00031~0.00135** (만기가 다가와 듀레이션이 0으로 수렴 = 사실상 정지) |
| 비율 | 2.57~9.69 (임계 2.5) |
| 최대 일변동 | **6.9%** (임계 50%) |

비율 판정은 "합성이 이 종목답지 않게 요동치나"를 보려는 것인데, **분모가 0에 가까우면
정상 합성도 무조건 배수를 넘는다.** 20종의 6.9%조차 확인해보니 `CORPAA3Y`가
**1998-01-05에 -3.45%p**(IMF 직후 회사채금리 31.11%→) 움직인 것 × 듀레이션 2.0 = 실제 역사다.

**매일 같은 20종을 알리는 알림은 알림이 아니다** — B-2 무결성 알림 전체가 무시되기 시작한다.

### 수정 = 분모 하한

`ratio = 합성std / max(실데이터std, REAL_STD_FLOOR=0.0018)`.

하한을 왜 저 값으로 두는지가 핵심이다. 통과선이 `0.0018 × 2.5 = 0.0045`(연 7% 변동성)이므로
**그 아래로는 애초에 가짜 역사를 만들 수 없다**. 반대로 이 게이트가 원래 잡으라고 만들어진
SHY 구손상(합성 std 0.016)은 하한을 써도 **8.9배**로 그대로 검출된다. 임계값을 올리는
(=검출력을 파는) 방식이 아니라, 무의미한 분모만 막는 방식이다.

곁가지: 실데이터 std가 정확히 0이면 판정 자체를 스킵하던 것도 하한 덕에 판정 가능해졌고,
출력에 `r_std`·`s_std` 칼럼을 넣어 다음에 알림이 오면 비율만 보고 헤매지 않게 했다.

**검증**: 신규 `tests/test_backfill_corruption_scan.py` 5 PASS(정지 실데이터+정상 합성=OK /
같은 분모라도 SHY급 합성=CORRUPT / 정상 주식 / 단일일 60% 점프 / close<=0) + **prod DB 전수
326종을 구·신 스캐너로 동시 실행: 손상 20 → 0, 새로 잡힌 종목 0**(32초).
만기매칭 가드는 `test_backfill_rate_proxy.py`에 18케이스 추가(만기형 9 차단 / 상시형·영문
숫자쌍 8 통과 / kr_etf_list 전수 — 주식형 카테고리 오탐 0). 두 파일 합쳐 **84 PASS**.

### 그리고 진짜 문제 — 만기매칭형은 백필 대상이 아니었다 (오너 결정)

스캐너를 고쳐도 데이터는 그대로다. **만기매칭형에 듀레이션 2.0 고정은 과대**다 —
실데이터가 함의하는 듀레이션은 0.2~0.9인데 모델은 2.0을 써서 합성 역사가 실제 펀드보다
3~5배 요동친다. 오너 결정: **듀레이션을 손보지 말고 백필 자체를 거부**.

근거는 듀레이션 값이 아니라 **상품 구조**다. `TIGER 26-04 회사채`는 2026-04에 청산되고
`ACE 2월만기자동연장`은 만기마다 갈아탄다. **2026-04에 만기가 오는 펀드의 1995년은 존재하지
않고**, 듀레이션도 만기까지 남은 기간에 따라 매일 줄어 **고정값 모델 자체가 성립하지 않는다**.
값을 0.8로 낮춰도 같은 종류의 허구다 — 프로젝트 원칙(틀린 역사보다 없는 역사)대로 거부한다.

- `bond_model.is_target_maturity()` — `YY-MM`(26-04·32-10·53-09)·`만기자동연장`·`자동연장`·
  `존속기한` + 미국 타겟만기(`BulletShares`·`iBonds`). 숫자쌍 규칙은 **한국 ETF 작명 규칙**이라
  KR에만 적용한다(영문 `Global X NASDAQ 100 Collar 95-110`이 오탐).
- 가드는 **`is_bond` 판정과 무관하게** 이름만으로 건다 — `BNK 26-06 특수채`처럼 분류가
  equity로 오배정된 종목도 같은 이유로 백필하면 안 된다. `target_maturity_skip` +
  price_loader `_PERMANENT_SKIP` 등재(매 요청 재시도 방지).
- prod 정리는 `scripts/fix_rate_proxy_backfill.py`에 `find_target_maturity()` 추가로 흡수
  (기존 purge→재생성 흐름 재사용: 삭제 후 재백필하면 가드가 거부해 무데이터로 남는다).
- 대가: 이 종목들은 **상장일부터만** 표시된다. 26종이 대상(회사채 20 + 국고채·특수채·금융채 6).

### 남은 것

- `risk_return_logic._clean_deep_points`의 심화통계 게이트도 같은 비율식(`_DEEP_SYNTH_VOL_RATIO`)을
  쓴다. 포트폴리오 지수 단위라 이번 20종처럼 분모가 정지하는 경우는 드물지만, 만기매칭형
  단독 포트폴리오면 같은 함정이 있다. **사용자에게 보이는 수치가 바뀌는 변경이라 이번 범위에서 제외.**
- 곁가지 관찰: `index_daily`의 `CORPAA3Y` 최신일이 **2026-06-01**(2개월 stale). 백필은 상장 전
  구간만 쓰므로 이번 건과 무관하지만 확인 필요.

_작성: Claude_

## [2026-08-03] FEATURE | 종목상세 배당 그래프 (표 → 표 + 그래프)

오너 요청: 종목·ETF 검색 시 배당금이 표로만 보여서 그래프로 보고 싶다. 가격 차트와
겹칠지, 따로 뺄지, 고르게 할지 고민 중.

**겹치지 않기로 했다.** 근거 셋:
- **스케일**: `TIGER 미국배당다우존스` 주가 ~11,000원 vs 분기배당 ~150원. 한 축이면 배당은
  0에 붙은 직선이고, 이중축으로 맞추면 두 선의 관계가 **축 스케일이 만든 착시**가 된다.
- **기간 선택기와 충돌**: 라인차트는 1개월~전체를 고르는데 배당은 분기 이벤트다. 1개월
  뷰에선 막대가 0~1개.
- **구현 2배**: 라인=Chart.js, 캔들=LightweightCharts. 겹치려면 양쪽에 각각 만들어야 한다.

분기 raw를 선으로 그리는 것도 뺐다. 진짜 궁금한 건 "배당이 늘고 있나"인데 분기 노이즈가
그걸 가린다. 정확한 날짜·금액은 지금 표가 이미 잘 하고 있다.

### 만든 것

배당 섹션 안에 **막대(주당 배당금) + 선(배당수익률) 이중축**. 이 조합은 이중축을 써도 되는
드문 경우다 — 수익률 = 배당/주가라 둘의 관계가 정의상 실재한다. "배당은 늘었는데 주가가 더
올라 수익률은 떨어졌다"가 한눈에 보인다. **연간/분기 토글**(월배당 ETF는 분기가 궁금할 수 있다).

### 착수 전에 발견한 것 — 표가 이미 가짜 배당을 보여주고 있었다

`KODEX 미국S&P500(H)`의 배당 393건 중 **380건(97%)이 백필 생성분**이고, 표엔 **1928-03-30**부터
찍혀 있었다. 이 ETF 상장일은 2022-12-02다. `가격 차트는 volume=0 행을 이미 빼는데
배당 표만 그대로 보여주고 있었다.` 오늘 스윕으로 317종에 이게 퍼졌다.

표일 땐 그냥 숫자 목록이라 덜 눈에 띄는데, **그래프로 그리면 "이 ETF는 1928년부터 배당을
꾸준히 늘려왔다"는 설득력 있는 그림**이 된다. 그래서 그래프보다 이걸 먼저 처리했다.

- 백엔드가 배당마다 `est` 플래그를 준다. 판정은 **첫 실거래일 이전이면 추정** — 백필은
  상장일 이전 구간에만 주입하므로 정확하고, provenance 테이블 없이도 된다.
- 표에도 `추정` 배지 + 흐린 글씨.
- **그래프 기본값은 실측만.** 처음엔 전체를 그렸는데, 1964~2022가 추정이라 60년치가 화면을
  지배하고 수익률 선은 오른쪽 끝 점 3개로 뭉개졌다 — 내가 경고했던 그 그림이 그대로 나왔다.
  가격 차트도 합성을 빼므로 기본값을 맞추고, **`상장 전 추정 포함` 체크박스**로 옵트인.

### 착시 방지 2종

- **진행중(올해)** — 아직 다 지급되지 않아 막대가 낮다. 구분 안 하면 "배당 삭감"으로 읽힌다.
- **부분 구간(상장 연도)** — 458730은 2023-06 상장이라 2023 막대가 반년치뿐이다. 같은 종류의
  착시라 같이 처리했다(첫 버킷이 기간 중간에 시작하면 표시).

둘 다 반투명 + 점선. 단 **막대가 30개를 넘으면 테두리를 뺀다** — 추정 포함 시 62개가 되는데
1.5px 테두리가 4px 막대를 덮어 오히려 실측처럼 진해 보였다.

검증: 신규 `tests/test_symbol_dividend_chart_browser.js` 라이트/다크 **48 PASS/0 FAIL**
(이중축·연간분기 토글·추정 토글 왕복·범례 조건부·표 배지·진행중 막대·가격차트 무변경·콘솔에러 0),
스샷 육안 확인. `test_symbol_api.py` 8 PASS.

⚠️ 무관한 기존 실패 1건 확인: `tests/test_symbol_browser.js`가 `candleChart.timeScale()`에서
죽는다. 변경분을 stash 하고 HEAD에서 재현해 **내 변경과 무관한 기존 버그**임을 확인했다.

💡 함정 메모: 이 프로젝트 dev 서버는 `use_reloader=False`라 코드 수정이 자동 반영되지 않는다.
게다가 이번엔 `__pycache__`가 stale이라 **재시작해도** 구버전이 돌았다. 백엔드를 고쳤는데
API 응답이 그대로면 `__pycache__` 삭제 후 재시작할 것.

_작성: Claude_

## [2026-08-03] BUGFIX | 금리를 가격으로 쓴 백필 + 미백필 종목 "데이터 없음" + 비교 무음 상한

오너 제보 3건. 셋 다 원인이 제보 내용과 달랐다.

### ① 0046A0 무결성 이상 — 금리(%)가 가격으로 둔갑

prod에 **1964년부터 15,233행**의 가짜 역사가 있었다. 1981년 17,951원 → 2020-03-09 1,063원 →
다음날 1,509원(**하루 +41.9%**). 3개월 만기 초단기 국채 ETF가 이렇게 움직일 수 없다.

뿌리는 **한 줄짜리 매핑 누락**이다. `_BOND_CATEGORY_CONFIG`에 `US_TREASURY` 키가 없어
`bond_config()`가 None을 반환 → `is_bond=False` → 채권 모델(`build_bond_price_series`)을
건너뛴 채 `INDEX_MAP["US_TREASURY"] = "DGS10"`이 그대로 프록시가 됐다. **DGS10은 10년물
금리(%)지 가격이 아니다.** 그래서 1981년 15.84% ~ 2020년 0.52%인 금리곡선이 그대로 가격으로
스케일됐다. 듀레이션도 틀렸다(3개월물에 10년물).

- 단기 3종(0046A0·329750·440650)을 `_BOND_ETF_CONFIG`에 **DGS3MO**로 명시
- `US_TREASURY_10Y`(5종)는 만기가 특정되므로 카테고리 매핑 추가(DGS10, dur 7.5)
- `US_TREASURY`는 3개월물~30년물이 한 카테고리에 섞여 있어 **카테고리 단위 듀레이션이 존재할 수
  없다** → 의도적으로 매핑하지 않고 미등록 코드는 거부시킨다
- **재발방지(핵심)** = `_is_rate_series()` 가드. is_bond=False인데 프록시가 금리 시계열이면
  `rate_proxy_without_bond_model`로 백필을 **거부**한다. 매핑 하나를 빠뜨려도 이 버그 종류가
  다시는 데이터로 남지 않는다. 접두 규칙(DGS*/KTB*)이라 신규 금리 추가도 자동 커버.
- 곁가지로 `_PERMANENT_SKIP`이 실제로 작동하지 않던 것도 고쳤다 — status에 상세가 붙는데
  (`"no_index_map (US_MIXED)"`) 집합 **동등**비교라 매칭이 안 돼 매 요청 재시도하고 있었다.
- 오염 범위는 provenance로 정확히 특정 가능: `price_daily_source`에서 `source_code`가 금리인데
  `confidence='B'`(=채권모델 미적용). **prod 전수 = 0046A0 1종뿐**. 나머지 금리 프록시 백필은
  전부 `confidence='C'`(정상).

⏳ **prod 정리 미실행** — 배포 후 `scripts/fix_rate_proxy_backfill.py --apply` 필요.

### ② "환헤지 ETF만 과거 데이터가 안 나온다" → 환헤지는 원인이 아니었다

오너는 환헤지 S&P500 ETF만 실패한다고 봤지만, **비헤지인 `KODEX 미국나스닥100`도 똑같이
실패**했다. 진짜 변수는 헤지 여부가 아니라 **백필 커버리지**다 — prod의 kr_etf_list
**1,075종 중 백필된 건 27종(2.5%)**뿐이었다. 헤지 S&P500 중 448290 하나만 우연히 데이터가
있어서 "헤지 vs 비헤지"처럼 보였을 뿐이다.

경로 원인은 `modules/tr_index.py`가 설계상 `ensure_full_history`(지연 백필)를 **의도적으로
빼두고** price_daily를 raw SQL로만 읽는 것. 한 번도 요청된 적 없는 종목은 0행이라 조용히
`[]`가 나오고 화면엔 "데이터가 없습니다"로 떴다.

읽기 직전에 `_ensure_history()`를 넣어 자기치유하게 했다. 검증: 로컬 콜드 상태에서
269540·448290 → **1928-01-03부터**, 379810 → 1971-02-05부터(^NDX 시작), 0.8~2.4초.
**환헤지 백필 로직 자체는 원래 정상이었다** — 헤지는 FX를 안 곱하므로 오히려 비헤지(1964년
USD/KRW 시작에 막힘)보다 과거로 더 간다.

### ③ 비교 10개 넘으면 "작동 안 함" → 무음 상한

`rrAddBench()`가 `length >= 12`에서 **아무 알림 없이 return**. 기본 벤치마크 5개가 미리 차
있어 실제로 넣을 수 있는 건 7개뿐이었고, 초과하면 클릭이 그냥 씹혀 "버튼이 죽은" 것처럼 보였다.
서버는 별도로 `[:15]`(compare)·`[:10]`(risk-return)로 조용히 잘라 프런트와도 어긋나 있었다.

상한을 **20으로 통일**(프런트 상수 + 서버 양쪽)하고 초과 시 토스트를 띄운다. 20개 혼합
(개별종목+ETF+지수) 실측 **9.0초** — nginx 120s에 여유. 검증: 라이트/다크 **17 PASS/0 FAIL**.

### ④ 후속 감사 — 분류 자체가 오염돼 있었다

①을 고치고 나서 오너가 물었다: "그럼 한국 ETF들이 매핑이 안 돼 있다는 거 아니냐, 27종이
전부냐?" 전수를 재보니 **27종은 상한이 아니라 그때까지 실행된 수**였고, 매핑 기준
**백필 가능은 471종**이었다(444종이 매핑은 있는데 실행만 안 된 상태 = ②가 원인).

그런데 그 471종을 종목명으로 훑어보니 **분류가 오염돼 있었다.**

- `KOSPI200` 110종에 섹터(`TIGER 200 헬스케어`·`200 IT`·`200 금융` 등 11종), 커버드콜 6종,
  팩터(`KODEX 성장주`·`가치주`·`모멘텀주`·`멀티팩터`), 그리고 **미국 ETF**
  (`ACE 미국WideMoat동일가중`)까지 들어 있었다.
- `SP500` 40종엔 `KODEX 미국서학개미`·`TIGER 미국S&P500동일가중`·`RISE 버크셔포트폴리오TOP10`·
  `KODEX 미국S&P500산업재`.
- `KR_TREASURY` 33종 중 **실제 국채는 7종뿐** — 나머지는 회사채·통안채·혼합.
- `US_CORPORATE` 8종엔 하이일드 2종과 물가연동국채(TIPS) 1종.

`index` 컬럼은 `chore: ETF 분류 갱신 (자동)`으로 만들어지는 **자동 분류**라 대분류만 맞고
상품 성격을 구분하지 못한다. 이대로 백필하면 KOSPI200 역사가 헬스케어 ETF의 과거로,
S&P500 역사가 서학개미 ETF의 과거로 붙는다 — **2026-07-21 QQQ=^GSPC 사고와 같은 종류**가
대규모로 재현되는 셈이다.

1,075종을 손으로 큐레이션하는 건 유지가 안 되므로, **카테고리를 신뢰하지 않고 종목명으로 한 번
더 거르는 가드**(`_name_mismatches_index`)를 넣었다. 세 층:

1. **구조 불일치** — 커버드콜·위클리·데일리·동일가중·중소형·혼합·롱숏·팩터(성장주/가치주/
   모멘텀/퀄리티/로우볼/최소변동성/밸류)·TOP N 집중. 어느 카테고리에서도 지수 추종이 아니다.
2. **광의지수 카테고리에서만** 섹터·테마 토큰(IT·금융·헬스케어·반도체·서학개미·ESG·배당…)
   실격. `DJ_US_DIVIDEND`의 "배당"이나 `PHLX_SEMICONDUCTOR`의 "반도체"처럼 카테고리 정의
   자체인 경우를 죽이지 않기 위해 **카테고리 상대적**으로 판정한다.
3. **시장 불일치** — 한국 광의지수 버킷인데 이름이 미국/글로벌/차이나면 실격.

레버리지·인버스·TR·(H)·선물은 정상 추종이라 통과시킨다(meta.leverage/hedge가 처리).

**결과: 백필 후보 471종 → 371종.** 119종을 차단하고 신규 매핑 18종을 편입했다. 줄어든 게 아니라
**믿을 수 있는 것만 남긴 것** — 이전 471종 중 119종은 틀린 역사를 만들 종목이었다.

### ⑤ 누락 매핑 추가

- **`USD` 11종 → `USD/KRW`** — 1964년부터 17,478행이 있는데 미국달러선물 ETF들이 못 쓰고 있었다.
  `market=CURRENCY`라 `fx_applied=False` → 환율 이중적용 없음(테스트로 고정). 실측 검증:
  261240(1x) 1964년부터 15,116행, 합성/실데이터 변동성비 0.74x, 가격범위 7.7배
  (1964년 255원 → 1998년 IMF 1,960원과 일치). 139660(인버스)도 비율 0.99x로 정상.
- **`US_CORPORATE`** — 카테고리 일괄 매핑 대신 한글명 분류기(`classify_kr_listed_us_bond_etf`).
  회사채/투자등급은 DBAA(단기 2.7·장기 13.0·기타 7.0), **하이일드·TIPS는 안전 스킵**. 5/8종 통과.
- **`US_BOND_AGGREGATE`**(DGS10·4.4, AGG와 동일), **`US_MONEY_MARKET`**(DGS3MO·carry, BIL과 동일),
  **`KR_TREASURY_5Y`**(KTB3Y·4.5 — KTB5Y 시계열이 없어 KR_BOND_AGGREGATE와 같은 방식).
- ⚠️ **`US_SEMICONDUCTOR` 8종은 보류.** `RISE 미국반도체NYSE`·`PLUS 글로벌HBM반도체`·
  `KODEX 아시아AI반도체exChina`가 각각 다른 지수를 추종해 ^SOX가 맞는지 불명. 추측해서 매핑하면
  이번에 고친 것과 같은 사고가 된다 — **오너 확인 필요**.

### ⑥ 배포 + prod 실행 결과

배포 후 prod에서 정리·스윕 실행.

- **0046A0 복구**: 오염 15,233행 삭제 → DGS3MO carry 모델로 10,832행 재생성(1982-01-04 =
  DGS3MO 시작). 변동성비 5배 이상 → **1.05배**, 하루 최대 41.9% → 19.8%(1997 IMF 환율,
  unhedged라 FX 포함), 무결성 스캐너 **`corrupt: False`**. 2020-03 구간이
  8,208 → 8,252 → 8,322(+0.5%)로 정상화됐고 실데이터 경계도 9,937 → 9,970으로 매끄럽다.
- **백필 스윕**: 325종 중 317종 백필, 실패 0, 351초. **백필 완료 27종 → 339종.**
  오너가 제보한 환헤지 S&P500 9종은 전부 **1928-01-03부터**, 신규 매핑 USD 선물은
  1964-05-04부터 생성됐다.

### ⑦ 스윕이 드러낸 것 — TPX.F는 TOPIX가 아니었다

스윕 후 무결성 스캐너를 전수로 돌리자 25종이 잡혔다. 두 갈래였다.

**(a) JAPAN_TOPIX 5종 = 진짜 버그.** `INDEX_MAP["JAPAN_TOPIX"] = "TPX.F"` 인데 index_master의
TPX.F는 값이 **-7.53 ~ 29.60**, 4,925행 중 **2,551행(52%)이 0 이하**다. TOPIX 지수는 2,700pt
근처니 이건 **가격 시계열이 아니다**(선물 베이시스류로 보인다). 0을 넘나들어 pct_change가
하루 -2,380%까지 튀고, 인버스 ETF(205720 ACE 일본TOPIX인버스)에 곱해져 **하루 +465.3%**
짜리 역사가 만들어졌다. ①의 DGS10 사건과 정확히 같은 종류다.

- `JAPAN_TOPIX` 매핑 제거. ^N225로 갈아끼우는 건 **다른 지수**라 오너 판단 필요 —
  올바른 TOPIX 시계열을 확보하기 전까진 백필 없음이 안전하다.
- 방어선으로 런타임 가드 추가: 비채권 백필에서 0 이하 행이 `max(5, 전체의 1%)`를 넘으면 거부.
  **비율로 판정하는 이유**는 `CL=F`가 0 이하 행을 딱 1개 갖기 때문이다 — 2020-04-20
  마이너스 유가로, **실제 역사**다. 절대값으로 막으면 원유 백필이 통째로 죽는다.
  그 하루만 빼고 나머지는 살린다.
- prod 정리: 5종 합성행 삭제(실데이터는 보존), 매핑이 없으니 재생성도 안 됨 = 안전.

**(b) KR_CORPORATE 19종 = 손상 아님.** 전부 만기매칭형 회사채 ETF(`26-04`·`27-12`·
`만기자동연장`)인데, 이들의 **실데이터 std가 0.0003~0.0009**로 사실상 정지 상태다
(만기가 다가올수록 듀레이션이 0으로 수렴). 일반 채권 ETF는 0.002~0.0037이다.
합성 std 0.003은 **일반 채권 ETF와 같은 정상 수준**이고 최대 일변동도 6.9%(임계 50%)다.
분모가 0에 가까워서 비율만 튄 것이다.

⚠️ 다만 **2026-04에 만기가 오는 펀드에 듀레이션 2.0을 고정 적용하는 건 과대**다.
고치려면 `stage_b_full_verify`로 실측 보정한 듀레이션 값을 건드려야 하고 백테 결과가
바뀌므로 **오너 판단 대기**. 현재 스캐너 알림은 20종(스윕 전 11종 → 스윕이 9종 추가).

_작성: Claude_

---

## [2026-07-23] FEATURE | 금액 입력란 콤마 표시 (mmMoneyInput)

오너 요청: 평단가·초기투자금 같은 금액 칸에 타이핑할 때 화면에 `1,980,000` 처럼 보이게. 저장·계산 값은 그대로.

- **간단한 변경이 아니다.** `<input type="number">` 는 콤마를 담지 못한다(명세상 값이 유효한 부동소수점 문자열이어야 하고, 콤마가 들어가면 브라우저가 값을 빈 문자열로 취급). 그래서 `type="text"` 로 바꿔야 하는데, 그 순간 그 칸을 읽는 코드가 `"1,980,000"` 을 받고 **`parseFloat` 이 `1` 을 반환**한다. 예외도 없고 계산도 정상적으로 돌아간다 — 결과만 조용히 틀린다. 이 앱은 세금·백테스트·은퇴 시뮬레이션이라 가장 위험한 종류의 버그다.
- **읽기 지점이 400곳 가까이 흩어져 있어** 하나씩 고치면 반드시 빠뜨린다. 그래서 **해당 엘리먼트에만 `value` 접근자를 덮어썼다** — 화면에는 콤마가 보이되 `el.value` 로 읽으면 언제나 콤마 없는 숫자가 나온다. **기존 읽기/쓰기 코드는 한 줄도 바꾸지 않았고**, 앞으로 추가될 코드도 자동으로 안전하다. 쓰기(`el.value = h.avg_price`)도 setter 가 받아 자동 포맷하므로 기존 종목 수정 화면도 콤마가 보인다.
- **전환 가능 여부를 먼저 확인**: `valueAsNumber`·네이티브 `<form>` 전송·`stepUp()`·`checkValidity()` 사용처가 앱 전체에 **0건**. 셋 다 `type=number` 에 묶인 기능이라 하나라도 쓰였으면 이 방식이 깨졌다.
- **대상 27개(원 단위만)**. 전체 숫자 입력란 73개 중 수익률·물가상승률·수수료율 22개, 기간(년) 11개, 나이(세) 6개, 수량(주)·스텝 개수는 제외 — "7.5%"·"30년"·"만 58세"에 콤마가 낄 일이 없다.
- ⚠️ **ISA 전환(`tsCurrentValue`·`tsCostBasis`) 2개는 의도적으로 제외**. `/tax-switch` 는 종목을 넣어도 계산이 자동으로 돌지 않아(POST 0건, 진행바 40초 대기에도 무응답) 골든 마스터로 결과 보존을 검증할 수 없었다. **검증할 수 없으면 바꾸지 않는다**는 원칙으로 남겨뒀다. 그 페이지 계산을 자동화할 수 있게 되면 함께 처리할 것.
- **안전장치 = `tests/golden_number_inputs.js`** (신규). 변경 **전에** 각 계산기에 종목을 심고 큰 금액(19,800,000)을 넣어 실행한 뒤 ①서버로 나가는 POST 본문 ②화면에 렌더된 숫자를 기록하고, 변경 후 대조한다. POST 본문을 쓰는 이유는 시세 같은 외부 변동에 오염되지 않으면서 "입력값이 그대로 서버에 갔는가"를 직접 보기 때문. 비결정 값은 baseline 을 2회 돌려 두 번 다 같은 것만 골든으로 채택해 자동 제거.
- **하니스를 만들며 잡은 결함 3개**: ①종목이 0개면 계산 버튼이 아무 일도 안 해 POST 0건 — 계산을 안 돌리는 골든 마스터는 아무것도 보증하지 못한다. 페이지별 종목 추가 함수를 호출해 시드. ②비중을 60/40 으로 직접 조작했더니 오히려 상태가 깨져 백테스트가 실행되지 않았다(첫 종목이 자동 100%라 조작 불필요). ③readback 을 `.value` 원문으로 기록해서, 콤마를 켜면 `"19,800,000"` 이 되어 **설계상 무조건 실패**하는 무의미한 가드였다 → 콤마 제거한 숫자로 기록하도록 수정.
- **검증**: 골든 마스터 전 항목 일치(POST 본문 4페이지 + 렌더 숫자 6페이지) · 실제 키보드 입력(순차 타이핑·중간 삽입·백스페이스·전체삭제·콤마 붙여넣기 전부 캐럿 정상) · 레이아웃 안전성 164/164(스피너 사라짐에 따른 폭 변화 확인) · 라이트/다크 185 PASS/1 FAIL(기존 BUG-NAVLINKS-1280).


## [2026-07-22] HARDENING | 레이아웃 안전성 전수 정비 — 전 22페이지 x 폭 8종 x 글자배율 4단

오너 요청: "엄마 폰만이 아니라 플립·폴드·태블릿·노트 등 모든 환경에서 안 깨지게 해달라."

- **원인은 개별 버그가 아니라 패턴 4개였다.** 실패 468건을 추적하니 ①flex/grid 자식의 `min-width:auto`(=내용보다 작아지지 않음) ②`flex-wrap` 누락 ③줄바꿈 불가 토큰(금액·URL) ④안 접히는 고정 여백, 이 넷의 복제였다.
- **전역 안전망**(`style.css`): `*{min-width:0}` — 468건 중 대부분이 이것 하나로 해소. `body{overflow-wrap:break-word}` — 계정삭제 페이지에서 긴 URL이 +326px 뚫던 문제. `table{max-width:100%}`.
- **기계적 전수 치환**: grid `1fr`→`minmax(0,1fr)` **63건**(`1fr`은 `minmax(auto,1fr)`이라 전역 규칙이 못 잡는 유일한 예외) · 고정 `min-width:Npx`→`min(Npx,100%)` 10건(100px 이상만, 아이콘 크기는 제외) · 중앙정렬 래퍼 10개에 `width:100%`.
- **`width:100%` 가 왜 필요한가**: `max-width:N; margin:0 auto` 만 있는 래퍼는 grid 아이템일 때 auto 마진이 stretch 를 꺼서 **fit-content** 가 되고, 내부 min-content 를 그대로 물려받아 화면 밖으로 밀려난다. simple·taxswitch·myportfolios·legal 4종이 전부 이 케이스였다.
- **`mmFitText` 개선**: 즉시 부모의 clientWidth 를 재면 안 된다. 부모가 이미 카드 밖으로 밀려나 있으면 부모 폭이 카드보다 넓게 나와 "맞는다"고 오판한다(내 자산 히어로에서 실측 343px vs 카드 236px). **실제로 자르는 조상까지의 남은 거리**를 재도록 변경.
- **신규 하니스 `tests/test_layout_safety.js`** — 22페이지 x 뷰포트 8종(260~1280) x 배율 4단 x 로그인·비로그인. 워커 6개 병렬 + **모서리 우선 2단계**(가혹한 조합만 먼저 → 걸린 페이지만 전체 매트릭스). 1,600 조합 → 실측 220~380 조합, 33분 → 8분.
- **결과 468 → 181 → 77 → 2 → 0.** 최종 **156/156 PASS**. 기존 `test_responsive_dark.js` 185 PASS/1 FAIL(= 아래 기존 버그, 이 작업과 무관함을 stash 로 확인).
- **오탐 잡느라 검출기를 세 번 고쳤다 — 재사용 시 필수**: ①`scrollWidth` 기준은 장식용 `::after`(right:-50px)·`text-overflow:ellipsis`·네이티브 `<select>` 를 전부 버그로 셌다(64건) → **텍스트 노드 Range 사각형** 기준으로 교체 ②세로 검사가 접힌 아코디언(`max-height:0`)을 잘림으로 셌다(1,100건 초과) → 클립 박스 높이 8px 미만 제외 ③`-webkit-line-clamp` 말줄임도 제외.
- **작업 중 자체 발견·복구한 실수**: `min-width` 일괄 치환 정규식이 `@media (min-width: 769px)` 까지 잡아 `min(769px, 100%)` 로 바꿔놨다. 유효하지 않은 미디어 특성이라 그대로 뒀으면 해당 미디어쿼리가 통째로 죽는다. 전수 확인 후 2건 복구. **선언과 미디어 특성은 문법이 같아 정규식으로 구분되지 않는다 — 일괄 치환 시 반드시 `@media` 문맥을 확인할 것.**
- 육안 검토 시트(가혹 조건 44컷) 오너 전달. ⚠️ **자동 검사는 "잘렸나"만 판정한다. "안 잘렸지만 찌그러졌나"는 사람이 봐야 한다.**
- ⚠️ 잔여 한계: 실기기 검증 없음(어머니 폰 1대 제외) · 화면 세로 비율 미검사 · 폴드 힌지 미검사 · CI 미편입(수동 실행).

## [2026-07-22] BUG(기존) | 1280px 에서 `.nav-links` 가 숨겨지지 않음

`test_responsive_dark.js` 의 `1280px 상단 링크 숨김` 이 FAIL. `@media (max-width:1400px){.nav-links{display:none}}` 가 있는데 1280px 에서 `display:flex` 로 렌더된다(폭 377px). 레이아웃 안전성 작업과 무관함을 확인했다 — 변경분을 stash 하고 HEAD 에서 재현해 동일 실패. **미수정, 원인 미조사.**


## [2026-07-22] BUGFIX | 시스템 글자배율에서 홈 총자산 잘림 + 글자배율 회귀 하니스 (BUG-FONTSCALE-CLIP)

오너 보고: "내 폰(S22 Ultra)은 멀쩡한데 어머니 폰(갤럭시 A)에서 홈 총자산 숫자가 밖으로 잘린다. 해상도 차이인가? 모든 기기에 자동으로 맞춰지게 할 수 없나."

- **해상도가 아니었다.** 두 기기 모두 1080x2340. 어머니 폰 스크린샷 중 한 장이 `설정›디스플레이›글자 크기와 스타일` 화면이었고 슬라이더가 9칸 중 4번째(기본보다 위)였다. Android WebView는 시스템 글자배율을 **textZoom으로 CSS 폰트에 곱한다** — CSS 미디어쿼리로는 감지도 대응도 불가능한 축이다.
- **왜 잘렸나(진짜 뿌리)**: 글자가 커진 것만으로는 부족하다. `.portfolio-header`가 flex인데 자식 `<div>`의 기본 `min-width:auto`("내용보다 작아지지 않음")가 축소를 막아 max-content 폭을 유지했고, 금액 `₩75,808,745`는 줄바꿈이 불가능한 한 덩어리라 wrap으로도 해소되지 않아 카드(`overflow:hidden`) 밖으로 나가 잘렸다. **좁은 폭에서는 배율 없이도 이미 깨지던 버그**를, 큰 글자배율이 일반 폭(384~412px)에서 터뜨린 것.
- **수정 원칙 — 배율과 싸우지 않는다.** 사용자가 일부러 키운 접근성 설정이므로 강제로 되돌리면(예: `setTextZoom(100)`) 오히려 못 읽는다. 대신 **레이아웃이 큰 글자를 흡수**하게 했다: `min-width:0`으로 flex 축소 허용, `.guru-promo-top` wrap, ≤1024px 장식 여백 압축(로고 margin 32→12px, 검색창 padding 20→8px). 그래도 남는 "줄바꿈 불가 금액"만 신규 공용 유틸 `mmFitText()`가 **넘칠 때만** 16px 하한까지 축소한다. 안 넘치면 손대지 않으므로 히어로 40px는 그대로다.
- **네이티브 무관**: 원격 로드형 Capacitor 앱이라 전부 서버 push로 반영. **APK 재빌드 불필요.**
- **신규 하니스 `tests/test_font_scale_responsive.js`** — 뷰포트 8종(플립 커버 260 · 폴드 바깥 320 · 360/384/412 · 폴드 안쪽 673 · 태블릿 800/1280) × 글자배율 4단(1.0~1.5) 매트릭스. **18 FAIL → 82 PASS / 0 FAIL.**
- **하니스를 만들며 잡은 함정 3개(재사용 시 주의)**:
  1. **배율 시뮬은 2패스여야 한다.** 1패스로 부모에 폰트를 박은 뒤 자식의 computed를 읽으면 상속 사슬을 따라 배율이 **복리**로 곱해진다(40→52→67.6). 실제 textZoom은 복리가 아니다.
  2. **배율은 인라인이 아니라 스타일시트로 주입해야 한다.** 인라인으로 박으면 `mmFitText`가 자연 크기 재측정을 위해 인라인을 지우는 순간 배율까지 사라져, 실제보다 작게 재고 "없는 버그"를 보고한다.
  3. **`scrollWidth` 기반 잘림 탐지는 오탐투성이.** `.guru-promo-card::after` 장식 글로우(`right:-50px`)가 정확히 +50px로, `text-overflow:ellipsis`와 네이티브 `<select>`도 잡혔다. **텍스트 노드의 Range 사각형이 자르는 조상 밖으로 나가는지**로 바꾸니 오탐 64건→1건.
- **기존 스위트가 이 버그를 놓친 이유**: `test_responsive_dark.js`는 `documentElement.scrollWidth`만 본다. 카드가 `overflow:hidden`이라 문서 폭은 안 넘치고, 잘림은 카드 **안에서** 일어났다.
- 검증: 매트릭스 82/82 + 라이트/다크 실클릭(금액보기 토글·기간 1주/1개월/1년) 콘솔에러 0 + 실렌더 육안. 도중 **clamp(22px,8vw,40px)를 넣었다가 일반 폰 정상배율에서 40→30.7px로 작아지는 디자인 회귀를 스스로 발견하고 40px로 되돌림** — `mmFitText`가 이미 "넘칠 때만" 처리하므로 clamp는 불필요하고 해로웠다.
- ⚠️ **잔여: 실기기(어머니 폰) 확인 미완.** 배포 후 확인 필요.
- 무관한 기존 실패 1건 발견(고치지 않음): `test_responsive_dark.js`의 `1280px 상단 링크 숨김`이 FAIL. `@media (max-width:1400px){.nav-links{display:none}}`가 있는데 1280에서 `display:flex`로 렌더된다. 내 변경을 stash하고 HEAD에서 재현해 **기존 버그로 확인**.
...
**①알림 이름**: 원인 = `alert_runner._build_symbol_ctx`가 `"name": code` — alert_rules에 이름 컬럼 없고 제목이 전부 코드. 수정 = `_display_name()` 신설(발화 시 resolve, 모듈 캐시): market_alias `MARKET_CODE_META`(지수·환율·금리) → `symbol_master.db` name(주식/ETF) → 폴백 코드. `^KS11→코스피 지수`, `005380→현대차`, `379780→RISE 미국S&P500` 확인. 룰 저장 방식 무변경(마이그레이션 0).

**②무결성 — prod 직접 검사, 3종목 판정 전부 다름**:
- **379780(RISE 미국S&P500) = 진짜 오염**: 상장전 합성백필(^GSPC×USD/KRW) 안에 **1964-05-04 ×255.77 절벽**(0.175→44.88). 뿌리 = `index_master` USD/KRW가 1964-05-04(255.77원) 시작인데 `backfill_engine.get_rate()`가 FX 이전 날짜에 **1.0 폴백** → 이전 구간만 환율 미적용. 수정 = fx_applied 시 index_series를 **FX 시작일부터 절단**(환산 불가 구간 백필 금지). prod 재백필로 복구.
- **INDV(Indivior) = 피드 쓰레기**: 2022-11-23·25 close 3.13(vol=0) 2행 — 주변 실가 19.6~21, 야후 얇은 ADR 시기 불량 행. prod 2행 DELETE로 복구.
- **005380(현대차) = 오탐**: vol=0 128행은 KR 휴장일 복제행(전일 종가 복제). 스캐너가 갭7일 산재 복제행 쌍(2000-05-30→06-06)을 합성 +42%로 오판 — 실제 그 주 닷컴기 실변동. 수정 = `scan_code`에서 **전일 종가와 동일한 vol=0 행 제거 후 판정**(복제행=정보0). 데이터 무변경.
- 무결성 알림 매일 오던 이유 = beat 10:30 스캔이 손상 잔존 시 매일 재발화(dedup 없음, 의도된 동작) — 데이터 복구로 자연 종료.
- 검증: `test_alert_runner`(10 PASS)·`test_alert_engine`+`test_internal_integrity_alerts`(21 passed) + 스캐너 인메모리 시뮬(휴장복제 케이스 None·×255 절벽 케이스 corrupt 유지) + prod 재백필 후 scan_all 클린 확인(배포 후 수행).
- 부수 발견: 379780 실데이터 06-18 정지(yf는 정상 — price_daily 온디맨드 특성, 아무도 안 건드림). 검색 "-" 문제(마스터 16,425 vs price_daily 425코드)와 함께 별도 논의 예정.
...
- prod 확인: `scan_backfill_corruption.py` 전체 스캔 62종목 중 손상 2종(`379780`,`INDV`)뿐, SHY/IEF/TLT/LQD 전부 OK(volx~1.0, close<=0 없음). 수술이 prod에 돌았음 확인. 잔존 2종은 동적 손상 게이트(`_clean_deep_points`, 2.5배 변동성 컷)가 실시간 방어.
- 수정: `_item_deep`의 `_annual_from_points`·`rolling.horizon_table` **`actual_only=True→False`**(백엔드) + `risk_return_page.js` 박스행 필터에서 **`!(a.syn_frac>0)` 제거**(프론트, `!a.partial`만 유지). 손상 게이트·글리치 가드(|연수익|>300%)는 유지 = 안전망.
- 결과: 심화 박스플롯·손실확률이 각 항목 **전체 백필 이력** 사용. SPY 99년(1928~)·TLT 50년·SHY 45년. 실제 과거 극값 노출(GLD 1979 +135%·QQQ 1999 +100%). Y축 -76~162%, 3000% 폭주 없음.
- ⚠️ **배당 심화는 그대로 실데이터만**(`_annual_dividends`의 actual 마스크 유지). 이유: 상장 전엔 그 ETF가 배당을 실제로 안 냄 — 프록시 지수 배당을 "그 ETF 배당"으로 보여주면 조작. 백필 가리기가 아니라 올바른 방법론. (오너가 배당도 원하면 별건으로.)
- ⚠️ TLT 백필 한 해 82% = bond_model 재생성 추정치(손상 아님, 게이트·가드 통과). 장기채 82%는 공격적 — 별도 bond_model 튜닝 후보.
- 검증: 백엔드(항목별 연도수·max|ret| 정상), Playwright(박스플롯 백필 이력·축 정상·손실확률표·콘솔0), `test_portfolio_compare_route` 5 PASS.
...
- **데이터 소스(ECOS, 코드 실호출 검증)**:
  - 조사지수(한국부동산원 유형별 주택매매가격지수) = `901Y093`(구기준 2021.6, 2003.11~2025.03) + `901Y113`(신기준 2025.3, 2021.06~현재) **체인링크 스플라이스** → 2003.11~현재. ITEM=[유형, 지역]. 유형 H69A 종합/H69B 아파트/H69C 연립다세대/H69D 단독주택.
  - 실거래지수 = `901Y089` 아파트 실거래가격지수(2006~, 1차원=지역). 서울 5개 생활권역(도심/동북/동남/서북/서남) 포함.
  - ⚠️ 조사 vs 실거래는 방법론이 달라(호가·거래 종합 vs 신고 실거래) **별도 시리즈**로 둠. 한 줄로 안 이어붙임.
- **추가 32종**: 아파트 매매지수 × 20지역(전국·수도권·지방+17시도), 주택종합/연립다세대/단독주택 × 전국·서울(6), 아파트 실거래 × 서울+5권역(6). 기존 이동(카테고리만): 美 케이스실러·주택착공·건축허가 + KB 전국주택 → 부동산.
- **코드**(`modules/macro_loader.py`만): `CATEGORIES`에 "부동산" 삽입. `_ecos_splice()` 빌더 + `fetch_ecos_splice()`(첫 공통월 계수로 구값을 신기준에 정렬) + `fetch_one`/`_upsert` 분기. `_ecos`에 desc 인자. 증분갱신은 start≥2021-06이면 신기준표만 fetch. macro.js/템플릿 **미변경**(카테고리 데이터 구동 자동 렌더).
- **스플라이스 연속성**: 서울 아파트 2021-05=99.30 → 2021-06=100.3, 계수≈1.003, 드리프트 <0.1%. 이음새 육안 확인 안 보임.
- 검증: 36종 백필 성공(세종 159행=시 신설 2012, 나머지 267/244행). 로컬 서버 + Playwright — KR뷰 "부동산" 칩 7번째·카드 33개 렌더(값·전월비·스파크라인), 서울 아파트 상세 2003.11~2026.01 연속 곡선·설명 표시, 5년/전체 토글·라이트/다크 정상, 콘솔에러 0. (초기 검증 중 나온 "토글 에러"는 시리즈 fetch 완료 전 클릭 레이스=테스트 타이밍, 실사용 무관.)
- ⚠️ **prod 백필 필요**: 신규 32코드는 prod DB에 없음. beat `refresh()`가 신규코드(last=None) 자동 풀백필하나, 배포 후 `python -m modules.macro_loader --backfill` 수동 1회 권장.
...
- `static/js/examples.js`:
  - `MAX_CMP` 5 → 20.
  - `compareAdd` → `compareToggle`: 이미 담긴 카드를 다시 누르면 목록에서 제거(취소)하고 버튼을 `⚖ 비교`로 되돌림. 안 담긴 경우 기존대로 추가.
  - `setCmpBtn(btn, added)`·`syncCmpBtns()` 헬퍼 추가 — 담김 상태를 카드 인라인/모달 비교 버튼에 일관 반영.
  - 상세 모달 열 때 비교 버튼이 해당 카드의 현재 담김 여부를 반영(모달에서도 취소 가능).
  - 페이지 로드/전체취소(×) 시 `syncCmpBtns()`로 세션에 남은 담김 상태를 버튼에 동기화.
- 검증: 로컬 + Playwright(Pixel 5, is-app). 담기→`✓ 담음`(N=1), 재클릭→`⚖ 비교`(N=0, 취소됨), 8개 담기 성공(리밋>5 확인), 리로드 후 8개 `.added` 유지, 전체취소 후 0·바 숨김, 콘솔에러 0. 라이트/다크 스샷 육안 확인.
...
- 확인: 서버는 이미 실적/배당락 계산에서 `^` 지수, `KRX_GOLD`, `=F` 원자재선물, `=X` 환율, `-` 포함 크립토/비대상 코드를 자동 제외하고 있었다. 문제는 UI가 그 사실을 보여주지 않는 점이었다.
- 수정: `내 알림` 목록에 캘린더 prefs를 가상 알림 행으로 함께 렌더한다. 저장된 증시 캘린더/거시경제지표 알림은 `거시경제지표` pill로 보이며, 08:00 KST 발화 조건과 선택 항목 요약을 보여준다.
- 수정: 해당 행에서 바로 켜기/끄기 저장이 가능하고, `수정` 버튼은 증시 캘린더 알림 설정 카드로 이동한다.
- 수정: 실적·배당락 대상 목록에서 지수·원자재선물·환율·크립토·KRX 금은 disabled + `일정 없음`으로 표시하고, 가격 기반 알림은 위 종목 알림에서 가능하다는 안내를 추가했다.
- 검증: 로컬 Playwright DOM 검사 PASS. `AAPL`, `^GSPC`, `GC=F`, `KRX_GOLD`, `BTC-USD` 테스트 데이터에서 비대상 4개 disabled, AAPL selectable, `내 알림` 캘린더 행 1개 + 수정 버튼 + `08:00` 표시, 콘솔 에러 0. 캘린더 행 켜기/끄기 prefs 저장 PASS. `tests/test_alert_runner.py` 10 PASS, `git diff --check` OK.
- 변경: `templates/alerts.html`, `moneymilestone/wiki/dev/status.md`, `moneymilestone/wiki/dev/bugs.md`, `moneymilestone/wiki/log.md`.

_작성: Codex_

## [2026-07-01] UX/FIX | 증시 캘린더 알림 안내 개선 + 모바일 메뉴 스크롤

오너가 `/alerts` 증시 캘린더 알림의 종류/발화 시간/저장 상태가 비직관적이고, 모바일 햄버거 메뉴가 스크롤되지 않는다고 보고.
- 수정: 증시 캘린더 알림 카드에 “저장 직후 알림 내역은 바로 생기지 않고, 다음 08:00 KST 실행 때 오늘 일정이 있을 때 생성된다”는 안내를 추가.
- 수정: 08:00 KST의 의미를 한국 주식은 장 시작 전, 미국 주식은 그날 밤 정규장 시작 전 알림이라고 명시.
- 수정: 경제지표·통화정책·실적 발표·배당락일에 CPI/FOMC/선택 종목 일정 보조 설명을 붙이고, 저장 후 현재 선택 요약과 알림 대상/발화 조건을 계속 보여주게 했다.
- 수정: 모바일 사이드바 드로어에 `100dvh`, 내부 스크롤, iOS 터치 스크롤, 하단 safe-area 여백을 적용해 긴 메뉴가 드로어 안에서 스크롤되도록 했다.
- 검증: 로컬 Playwright DOM 검사 PASS. 캘린더 안내·4개 설명형 라벨·FOMC 보조문구·경제지표 8/8 선택·저장 POST 200·서버 `econ_ids` 저장·모바일 390x640 드로어 스크롤·콘솔 에러 0. `git diff --check` OK.
- 변경: `templates/alerts.html`, `static/css/style.css`, `templates/base.html`, `moneymilestone/wiki/dev/status.md`, `moneymilestone/wiki/dev/bugs.md`, `moneymilestone/wiki/log.md`.
...
오너 요청 반영.
- `/risk-return` 지표별 심화 비교의 5개 오차막대 그래프(수익률·변동성·MDD·배당·배당성장률)에 표본 길이 선택 버튼 추가: `최대`, `자동`, `5/10/15/20/25/30년`.
- 기본값은 `최대`로 결정. 신생 종목이 포함됐을 때 `자동`이 2~3년 표본으로 전체를 지나치게 얕게 만들 수 있어, 기본 화면은 장기 데이터를 최대한 보존하는 쪽을 택함.
- `자동`은 해당 지표에서 가장 짧은 유효 표본 길이에 맞춰 각 항목의 최근 N개 연도만 사용해 표본 길이 왜곡을 줄인다.
- 카드 상단에 `자동`과 `최대`의 장단점 설명을 별도 안내 블록으로 표시.
- 검증: JS 문법 체크 OK, `git diff --check` OK, 로컬 Playwright DOM 검증 PASS(컨트롤 5개, 기본 `최대` active, `10년` 선택 시 표본수≤10, `자동` 선택 시 수익률 표본 8년으로 전 항목 통일, MDD만 5년 선택 가능, 차트 bar 유지, 콘솔 에러 0).
...
오너 요청 반영.
- `/risk-return`에서 정밀 비교 결과의 리스크-리턴 도표와 종합 비교(레이더/다각형)를 정밀 비교 카드 내부에서 빼서 상관계수 계산기 바로 아래에 표시하도록 재배치.
- 추세 겹쳐보기와 상관계수 계산기의 날짜·1/3/5/10년·전체·사건구간 입력을 `rrOv` 단일 상태로 통합. 어느 한쪽에서 바꾸면 다른 쪽도 같은 날짜와 active 표시로 즉시 따라간다.
- 사건확대 버튼도 3년/5년 버튼처럼 선택 시 파란 active 상태가 유지되도록 `activeEvent`를 추가.
- 검증: 로컬 JS 파싱 OK, `git diff --check` OK, Playwright DOM 검증 PASS(금융위기 active 양쪽 동기화, 3년 버튼 양쪽 동기화, 사건 active 해제, 정밀 비교 후 도표가 상관계수 아래·정밀 비교 입력 위에 표시, 산점도/레이더 렌더, 콘솔 에러 0).
- 변경: `templates/risk_return.html`, `moneymilestone/wiki/dev/status.md`, `moneymilestone/wiki/log.md`.
...
- **버그**(probe 입증): SHY 연수익 **−85.7%~+172.6%**, SCHD 배당성장 **+270.4%** → 차트 Y축 박살. 원인 = 손상 합성백필이 단일점프가 아니라 **구간 전체** 쓰레기(SHY 합성 일변동성 실데이터 20×·하루 +122%, IEF 2.9×). 코덱스 45% 단일일필터는 9121→9102(19행)만 제거. SCHD는 2011 상장 부분 첫해(0.00594)가 저베이스필터(median×0.15) 통과 → 2012 +270%.
- **수정**(`risk_return_logic.py`): ①**종목별 합성손상 게이트**(`_clean_deep_points`) — 합성 일변동성 실데이터 대비 >2.5× OR 단일일 >30%면 그 종목 합성 전부 드롭→real-only 폴백. 정상 합성(SPY/QQQ/SCHD/GLD/TLT ratio 0.9~1.3)은 긴이력 보존 = 코덱스 의도 살림. ②배당성장 저베이스 0.15→0.30·spike캡 3.0→1.0.
- 검증: SHY [−85.7%,+172.6%]→[−3.9%,+4.9%]·IEF→real-only·SCHD 배당 +270%→max+44.8%. Playwright(adhoc 비로그인) 아코디언5 렌더, 차트 max ret 49.2/divg 53.5%(축파괴 0)·콘솔에러 0·라이트 육안 PASS. 신규 `tests/check_rr_deep.js`.
- ⚠️ 다크 육안·실로그인 저장포폴 경로는 미검증(adhoc로 충분 판단). `_drop_corrupt_generated_tail` 코덱스가 2모듈 중복정의(DRY)지만 무해라 미수정.
- 변경=`risk_return_logic.py`(내 수정)+코덱스 `app.py`·`modules/tr_index.py`·`templates/risk_return.html`+`tests/check_rr_deep.js`(신규).
...
- **백엔드(app.py `_portfolio_index_series`)**: ① 종목별 시리즈 일일 캐시 `_ticker_series`(공유종목 TLT·GLD 1회 계산·dedupe) ② **다운샘플** `_downsample_points`(전체기간 ≤1800포인트, syn 보존) → 포폴당 11000→~1637 ③ 결과 캐시 `_TS_RESCACHE`(포폴 시그니처+일자, 재방문 0ms). 일자 바뀌면 캐시 자동 클리어.
- **프런트(risk_return.html)**: 진행률 바(`#rrOvProgBar`/Txt) — 예시 프리로드=포폴별 병렬 POST, 겹쳐보기 로드=키별 동시성4 — 도착하는 대로 N/총 % 채움.
- 검증: 콜드 720ms·다운샘플 1637/포폴·웜(캐시) 0ms / Playwright 5포폴 렌더 maxPts 659·진행바·콘솔0.
- 변경=`app.py`·`templates/risk_return.html`. ⚠️ 다운샘플로 일별 정밀도↓(추세용 충분, 줌도 다운샘플 기준).

## [2026-06-26] FEAT | 심화분석 P1 — 비교탭 겹쳐보기 드래그 확대 + 이벤트 프리셋 (오너)

`포트폴리오_심화분석_plan.md`(분석탭 심화+비교탭 동급화, 설계결정 8개 확정) 중 P1 착수·완료.
- rrOvChart에 `chartjs-plugin-zoom` drag 줌(mode x, 전역 로드 재사용) → 그래프 드래그로 구간 확대(닷컴·서브프라임 등 잘 안 보이던 사건 확대 가능).
- **↺ 초기화** 버튼(resetZoom + 기본 5년 복귀). **이벤트 프리셋**(닷컴 2000-02·금융위기 07-09·코로나 2020·'22 긴축) 원클릭(가용범위 클램프). 드래그 힌트.
- 검증(Playwright): zoom 플러그인 로드·프리셋4·코로나 범위·드래그줌 x스케일 변화(0-252→69-154)·초기화 복귀·콘솔0.
- 변경=`templates/risk_return.html`. 모바일 핀치=후속. 나머지 Phase(P0 롤링엔진·P2 분석심화·P3 비교동급화·P4 심화지표)=대기.

## [2026-06-26] FEAT/FIX | 추세 겹쳐보기 기간 정직화 — 6년캡 제거·합성 점선·중앙값 규격화 (오너 지시)

오너: 10년/전체 버튼이 2020부터만 나옴(=2016/전체 표기 불가). 원인=`_portfolio_index_series` `years=6` 하드캡. 지시=기간 반드시 표기, 합성구간 점선, 없으면 빼고, 시작 다른 포트는 진입시점 **중앙값으로 규격화**.
- **① 6년캡 제거**: `_portfolio_index_series(years=None)` 기본 전체기간(컷오프 '1900-01-01'). 10년→2016, 전체→실제 최이른일(예 60/40 1977)까지. (검증: us-6040 전체 1977~, 오색 1982~.)
- **② 합성 플래그**: 쿼리에 volume 추가, **volume=0/None=합성 백필(상장 전 추정치)**. 포인트=`[date, value, syn]`(syn=1: 그 날 구성종목 중 합성 섞임). 백엔드.
- **③ 점선 렌더**: `rrOvDraw` Chart.js `segment.borderDash`로 syn 구간 점선, 실데이터 실선(norm·raw 둘 다). 범례 힌트 추가.
- **④ 중앙값 규격화**(핵심): 시작 늦은(중간 진입) 시리즈를 100이 아니라 **진입일에 이미 그려지는 다른 시리즈들의 그 날 정규화값 중앙값(median)** 에 앵커링 → 자연 합류. 시작 이른순 정렬 후 순차 앵커. (검증: 전체뷰서 60/40=100[1977], 오색/영구/골든=144.94[1982 peer median], 올웨더=155.21[1986 peer median] 정확 일치.)
- **⑤ 전체 구간 = 최이른 시작**: `rrOvSeriesBounds` mn을 max(공통 최늦)→**min(최이른)** 으로. 이른 포트는 길게, 늦은 포트는 중앙값 합류.
- 검증(Playwright): 5포폴 EX: 프리로드 — 10년=2016·전체=1977·median 앵커 수치일치·합성 점선 시각확인(오색 SGOV 2020 전 점선, 60/40 2003부터 실선)·힌트·콘솔0·기본5년뷰 정상. ⚠️5년초과 정규화 과장 경고는 기존대로 노출.
- 변경=`app.py`(`_portfolio_index_series`)·`templates/risk_return.html`.
...
- **근본 원인**: `price_daily`에 **SPY 2026-06-17 종가=346500**(같은 날 삼성전자 005930 종가와 동일값) 저장 = yfinance 오틱(bad tick)이 SPY 행에 섞임. 하루 +46000%→익일 복귀 = 델타 스파이크. (지난번 삼성전자 스파이크도 같은 부류 — `_drop_isolated_price_spikes` 주석에 005930 2026-06-16 기록, 임계 25→4로 낮춘 그 건.)
- **왜 "전부" 튀나**: 실제 오틱 포폴은 SPY 포함분(영구·60/40)뿐. 올웨더/골든버터플라이/멀티에셋은 SPY 없음→자체 스파이크 아님. 한 차트 겹쳐그릴 때 SPY 스파이크가 **Y축을 ~28000까지 늘려 나머지 라인이 다 찌그러져** 전부 깨져 보인 것.
- **왜 백테는 멀쩡, 겹쳐보기만**: `get_price`(읽기경로, price_loader.py:596)는 `_drop_isolated_price_spikes`로 오틱 제거. 그러나 겹쳐보기 `_portfolio_index_series`(app.py)는 **price_daily를 raw SQL 직접** 읽어 그 필터를 안 거침.
- **픽스①(스파이크)**: `_portfolio_index_series`에 동일 `_drop_isolated_price_spikes` 적용(종목별 시계열 정제 후 지수 합성). 검증: us-permanent·us-6040 giant-spike 0, 06-17 오틱일 드롭(pts 1505→1504).
- **픽스②(스피너)**: 겹쳐보기 차트 위 `.rr-ov-spinner`(공용 `.ds-spinner`) 추가, `rrOvLoad`/`rrOvInit` 페치 중 표시→완료 시 숨김. 검증: 로드 중 노출·완료 후 숨김·차트정상·콘솔0.
- ⚠️ **잔여(근본)**: 쓰기경로(`fetch_from_api`)는 오틱 필터 없음 + `_insert_ignore`(INSERT OR IGNORE)라 잘못 저장된 행이 재페치로 안 고쳐짐 → 읽기시 필터로만 마스킹(저장값 자체는 여전히 틀림). 추후 쓰기시 정제 or 주기적 클린업 검토. 오너 결정사항.
- 변경=`app.py`·`templates/risk_return.html`.
...
- **① 투자대가 탭 = 카드 직접 노출**(별도 "전체보기" 버튼 거쳐야 나오던 것 제거). `examples` 라우트가 `guru_store.list_gurus()` + `get_guru(slug,12)`로 대가 카드 데이터(모노그램·stance·상위보유바 + 정규화 tickers) 동봉. 투자대가 탭에 guru 카드 그리드 바로 렌더. **하단 액션을 examples와 통일**(📈분석·⚖비교·⭐저장, examples.js 핸드오프 그대로 — data-tickers=커버 보유 비중정규화). /gurus 상세 링크는 하단 유지.
- **② 비교 진입 시 추세 겹쳐보기 = 비교 포폴 N개만**. 신규 `POST /api/portfolio/index_series`(ad-hoc 포폴 정규화 지수 시계열, 키=`EX:<i>`, 로그인 불필요, `_portfolio_index_series` 재사용). `risk_return.html`: 프리로드 시 `window._rrExPreload` 세팅 → `rrOvInit`이 기본 항목(SPY/금/코스피) **클리어하고 예시 N개만 주입**. `rrOvLoad`는 `EX:` 시계열 보존(macro/multi엔 없음 → 나머지 키만 조회).
...
## 2026-06-26 — 심화분석 P3a 핫픽스 (비교 아코디언 깨짐 — 오너 지적)

오너 실사용(멀티에셋·영구·골든버터플라이·올웨더·버핏) 시 아코디언 전부 깨짐. 4건 수정 + 데이터 손상 발견.
- **버그1 — 2023~2025만 표시**: annual/배당이 **공통 겹침기간**(common_start~end) 기준이라, 버핏 13F의 짧은 이력 종목 하나가 전 항목 연도를 2-3년으로 깎음. → annual·롤링을 **각 항목 자기 TR 인덱스**에서 산출(`_item_deep`, 공통기간 비종속).
- **버그2 — 수익률 그래프 폭주(y 3000%)**: 영구/골든버터플라이 공통 종목 **SHY의 합성 백필(vol=0, 2008)이 손상**(close 0.00↔190.34 진동) → 2015년 +2932% 가짜값. **IEF 합성도 동일**. → 비교 심화는 **실데이터(syn=0)만** 사용 + |연수익|>300% 글리치 가드. (분석탭 P2는 결정#1로 합성 포함이지만, 비교는 N개 겹침이라 글리치에 취약 → 실데이터.)
- **버그3 — 기간 선택 버튼 없음**: 아코디언에 프리셋(전체/닷컴/금융위기/코로나/'22 긴축) 추가.
- **버그4 — 드래그 확대 없음**: `chartjs-plugin-zoom` drag(x) + 프리셋이 x.min/max 설정. 손실확률 캡션 "추정 포함"→"상장 후 실데이터".
- 검증: 백엔드 스모크(글로벌 max |ret| 0.38, 스파이크 0) + Playwright(각 항목 실기간 SPY1993~/포폴2018~·기간버튼·드래그줌 x변화·툴팁·라이트/다크·콘솔0).
...
## 2026-07-03 — 플랜 확장: F-6 디자인 크리틱 패스 신설 (오너 지시)

오너: "오류 확인만 말고 UI/UX 디자인 개선도 능동적으로" + "모바일앱·데스크탑웹·모바일웹 3가지 다" — 기존 플랜에 없던 항목이라 `출시완성도_개선_plan.md` **F-6** 신설.
- 3폼팩터(데스크탑 1280/1920·모바일웹 390·앱 WebView) × 라이트/다크 = 페이지당 6컷 크리틱.
- 항목: 위계·여백리듬·타이포·컴포넌트 통일·밀도·차트 가독·터치타깃.
- 방식: 페이지 배치별 ①크리틱 ②**오너 승인(스샷 비교)** ③구현 ④재검증. 대규모 시각 변경은 오너 승인 필수(리스킨 금지 원칙 유지).

_작성: Claude_

## 2026-07-03 — E-1 esc() 단일화 (출시완성도 P2 #2)

로컬 복붙 esc 정의 11곳(템플릿 8 + static js 3)을 base.html 전역 `window.mmEsc` 별칭으로 교체. 사용처 무변경(별칭 전략 = 최소 diff).
- **보안 보너스**: 편차 통일 — dividend_target(따옴표 미이스케이프)·alert_widget(작은따옴표 미이스케이프)이 풀 이스케이프로 강화됨.
- 함정 처리: `function`→`const/var` 호이스팅 상실 — 전 사용처가 런타임 함수 내부(파스 후 실행)임을 확인, static js는 IIFE 톱레벨 할당(로드 시 실행)이라 안전. jsdom 테스트는 해당 js 미로드 확인.
- 검증(Playwright, 로그인 시드): 10페이지(홈+알림위젯 종패널·내자산·알림 셀렉트·심볼명·내포폴 목록/상세·비교·배당·예시 카드82·계산기) 렌더+esc 경유 요소 확인, **jsErr 0**. 특수문자 포폴명(`<포폴> & "테스트"`) 텍스트 렌더 확인. 시드 정리 완료.

_작성: Claude_

## 2026-07-03 — E-2 캐시버스팅 자동화 (출시완성도 P2 #3)

수동 `?v=20260701nav2` 방식(갱신 누락 = 유저가 구버전 JS/CSS 재발 사고) 종결.
- `app.py`에 `@app.template_global` **`static_v(filename)`** = url_for + `?v=<파일 mtime>`. 배포(git reset --hard)는 변경 파일만 mtime 갱신 → 바뀐 자산만 캐시 무효화, 나머지는 캐시 유지.
- 템플릿 16개의 수동 패턴 **39곳 전부** 정규식 일괄 치환 → `{{ static_v('js/x.js') }}`. 이후 JS/CSS 수정 시 버전 문자열 갱신 불필요(자동).
- 검증: 렌더 HTML `?v=<mtime숫자>` 확인 + Playwright 7페이지(홈·계산기·백테·은퇴·간편·ISA·거시) — versioned 자산 6~10개/페이지, **static 404 0·JS에러 0**.
- 규칙: 신규 자산 링크는 반드시 `static_v()` 사용 (수동 ?v= 금지).

_작성: Claude_

## 2026-07-03 — F-6 배치1: 홈 디자인 크리틱 (오너 승인 대기)

6컷(1280/1920/390 × 라이트/다크) + 다크 재검 촬영·육안 크리틱. 상세=[[dev/design-critique]].
총평 = 홈 위계·카피·카드 시스템 이미 토스톤에 근접. 제안 D1~D5(모바일 금액 줄바꿈·위젯 그리드 구멍·모바일 도구카드 2열 압축·다크 구분선·다음할일 지연 렌더). 구현은 오너 승인 후.

_작성: Claude_

## 2026-07-03 — F-6 배치1 반영 + BUG-KOSPI-NIGHT-NULL 수정

오너 리뷰: D1·D2 철회(테스트계정 산물)·D3 기각·**D4/D5 승인**·신규 2건(스와이프 모멘텀·코스피 밤 데이터없음).
- **코스피 밤 "데이터 없음"** = index_ohlc NULL봉(자세히=bugs.md BUG-KOSPI-NIGHT-NULL). 쓰기가드+읽기가드 6곳+회귀 2건(home_widgets 20 PASS). prod 정리는 배포 후.
- **스와이프**: `.widget-slide`에 `scroll-snap-stop: always` — 모멘텀이 여러 장 건너뛰는 것 차단.
- **D4**: 다크 위젯 셀 구분선 — 아이템이 인라인 `style="...var(--border)..."`로 그려서 셀렉터 오버라이드 불가 → **그리드 스코프 변수 재정의**(`html[data-theme=dark] .market-grid { --border:#3a3f49 }`). 390/1280 둘 다 rgb(58,63,73) 확인.
- **D5**: 다음 할 일 점진 렌더 — 리밸·은퇴 행 즉시, 배당(콜드 10s+) 도착 시 맨 위 삽입. Playwright로 배당 前 렌더 확인.
- 교훈: 인라인 style 속성은 CSS 룰로 못 이김 → var() 스코프 재정의가 정석 / 크리틱은 오너 실계정 기준.

_작성: Claude_

## 2026-07-03 — D2 개편: 홈 위젯 모바일=리스트/데스크탑=타일 (오너 선택 ②)

빈 칸·홀수 항목 선 엉킴·긴 이름 칸 폭주/잘림 = 셀+보더 구조 문제 → 레이아웃 전환. 상세=[[dev/design-critique]] 배치1.
...
## 2026-07-03 — F-6 배치2: 내자산 디자인 크리틱 (오너 승인 대기)

6컷+모바일 6탭 개별 촬영·재검(6s·DPR1 분리실험). 상세=[[dev/design-critique]] 배치2.
**버그급 2건 발견**: M1 모바일 차트 ~1/6 스케일 렌더(비중 파이=부채꼴 조각·배당 막대=바닥 깔림, 390폭 캔버스 문제 추정) / M2 월배당 ETF 배당일정 행 파괴. + 마감 4건(회색 파이 범례 중복·히어로 콜드 공백·그룹 카드 압사·아바타 폴백) + 카피 1건(조사 "이(가)" 병기). 구현은 오너 승인 후(버그급 2건은 즉시 착수 권장).

_작성: Claude_

## 2026-07-03 — F-6 배치2 구현 완료 (M1 철회·M2~M7 + 오너 신규 2건)

오너 리뷰: 전건 승인 + 신규 2건(종목추가 검색 드롭다운 화면 덮음·배당 월 색상 혼란). **M1은 오진 철회** — Playwright fullPage+canvas 캡처 아티팩트(실브라우저 정상, 오너 지적이 맞았음) → frontend-verification에 "캔버스 페이지 fullPage 캡처 금지" 규칙 추가.
구현 = M2 월배당 압축 / M3 파이 종목명 범례+순환색+합산(설정 추가 안 함) / M4 히어로 스켈레톤 / M5 그룹카드 2행 grid / M6 조사 불요 문형 / M7 아바타 이니셜 폴백 / 신규① 검색 드롭다운 오버레이화(원인=CSS 미정의) / 신규② 배당 실적/예측 스택 바. 상세=[[dev/design-critique]] 배치2 결과·[[dev/bugs]].
...
## 2026-07-03 — F-6 배치4 크리틱 + alert() 일소 + F-4 문구 스타일시트

- **배치4(은퇴/배당)**: 폼 12컷+결과 4컷. 총평 양호 — 마감 3건(R1 고갈값 줄바꿈·R2 분포카드 2+1·R3 모바일 x축 라벨)+문구 1건(R4 "달성 확률" 이중) 승인 대기. 상세=[[dev/design-critique]] 배치4. fullPage 아티팩트 비이슈 2건(빈 캔버스·sticky nav) element 재검으로 판정.
- **alert() 26곳 → mmToast**(오너 즉시 승인, b7786bd): dividend_target 12·myportfolios 9 등. 실클릭 dialog 0 확인.
- **F-4**: [[dev/copy-style]] 신설 — 용어 규칙 성문화(포트폴리오 전체표기·적립/납입 구분·% 자릿수 도메인 규칙·규제선 카피 0 확인). 사용자 노출 "포폴" 문장 2곳 즉시 수정, 미결 3건(내포폴 칩·달성확률 개명·share 월납입) 오너 결정 대기.

_작성: Claude_

## 2026-07-03 — 배치3·4 오너 결정 반영 + F-6 배치5 크리틱

- 오너 결정: 배치3 C1·C2 기각(현행 유지) / R1~R4+문구 3건 승인 → 구현·배포(4bf1c53). R2는 nowrap 잘림 → keep-all 두 줄로 교정.
- 배치5(비교·예시·거시·캘린더·검색·알림·설정): 42컷 — **전 페이지 기준선 충족, 발견 = 긴 페이지 탐색성 2건(취향)만**. 상세=[[dev/design-critique]] 배치5. F-6 전 페이지 1차 패스 완료(홈~설정).

_작성: Claude_

## 2026-07-03 — E-4 Lighthouse 베이스라인 (로컬)

홈 64 / 내자산 58 / 계산기 63 (모바일 프로필). 단 TTFB 4s = 로컬 dev 서버 왜곡 — prod 재측정 전 개선 보류. 유효 시그널 = unused CSS 150KB대·내자산 CLS 0.129·계산기 A11y 82(대비·라벨 미연결). 상세=[[dev/perf-optimization]] E-4 절.

_작성: Claude_

## 2026-07-03 — B5 구현 + G-2 전 시뮬 단계 표시 + F-5 접근성 최소선

- **B5-1**(51351b2): 거시 모바일 sticky 카테고리 점프 칩 + 지표 카드→리스트 행(이름 2줄 클램프·%만 표시·소형 스파크). **B5-2**: 알림 캘린더 종목 체크리스트 포폴별 기본 접힘("n/m 선택" 요약) — 설정 탭 4,508→1,657px.
- **G-2**(4fb747a): 전 시뮬 태스크에 `_mark_preparing`(시작 즉시 PROGRESS/preparing — 데이터 로드가 PENDING에 묻히던 문제 해소) + 백테·은퇴·세금스위치 프론트 phase 매핑. 은퇴 라이브 시퀀스 "준비 중→데이터 준비 중(1%)→계산 중(n%)" 실캡처. 계산기·배당목표는 기존 배선, 비교는 자체 n/m 카운터.
...
## 2026-07-03 — E-3 마무리(base 분리) + A-2 재판정 + 다크 버튼 결정

- **base.html 메인 블록 449줄 → base_app.js**(다크토글·mmToast·벨·nav). 인라인 잔존 = 의도적 4블록(테마 FOUC 방지·앱 감지·MM_LOGGED_IN/mmEsc·JSON-LD, 총 ~38줄). share.html = **인라인 유지 판정**(Jinja 12개 데이터 렌더 + 일회성 조회 페이지라 캐시 실익 0). 검증 = 3페이지(홈·계산기·내자산)에서 테마 토글·토스트·벨 실클릭, jsErr 0.
- **A-2(CSP unsafe-inline 제거) 재판정**: 인라인 `onclick` 핸들러 **~240개**(템플릿 145 + JS 생성 마크업 95)가 실질 선행조건 — 전면 addEventListener 전환은 초대형+회귀 리스크라 **출시 후 착수**로 확정. E-3 산출물(페이지 JS 외부화)은 그 선행을 완료한 상태.
...
- **후속3(오너 "전체화면에도 달력·사건 UI 다 넣고, 가로화면 안내 문구")**: 전체화면 진입 시 `rrOvCtrl`(달력·기간·사건·모드·기준)를 wrap 안으로 DOM 이동(원위치 기억→종료 시 복원). 세로+터치+전체화면에서만 뜨는 회전 안내 `📱↻ 가로로 돌리면 더 편해요`(가로면 `@media(orientation:landscape)`로 자동 숨김).
- **후속4(오너 "그래프가 눌려서 좆병신"=컨트롤이 차트 세로공간 먹어 압축)**: grid 행으로 넣은 게 실수. **접이식 오버레이 패널로 전환** — ctrl-full = `position:absolute`(grid 공간 0)·기본 `display:none`, fullbar `⚙ 도구` 버튼으로 `rr-ov-tools-open` 토글. 차트는 항상 full 높이(캔버스=wrap 93%). 종료 시 tools-open·버튼 on 정리. 재검증 21 PASS(기본 패널 숨김·차트 안눌림 캔버스≥70%·도구 토글 열림/닫힘·컨트롤 이식·회전안내·종료복원·라이트/다크·jsErr 0).

_작성: Claude_

## 2026-07-09 — 물가 상승률(YoY) 파생 시리즈 7종

- 오너 요청: 물가 상승률 그래프를 기준금리와 비교하고 싶다. 방법 검토 결과 외부 API 불필요 — 기존 CPI/PPI 지수에서 전년동월비 계산이 곧 공식 발표치.
- `macro_loader` 파생 타입 `yoy` 신설(부모 지수 → YoY %, DB 내 계산). 미 CPI·근원CPI·PCE·근원PCE·PPI + 한국 CPI·PPI = 7종. 한미 비교쌍 2개 추가. 기준금리와 단일 축 비교 가능. 상세=[[dev/status]] 2026-07-09 절.
- 검증: 공식 발표치 대조(한 2022-07 6.33%↔6.3%) + Playwright 15 PASS(카드·상세·비교·겹쳐보기·다크·jsErr 0).

_작성: Claude_

## 2026-07-09 — 겹쳐보기 지수 동결 픽스 + 백테 차트 y축·줌 리셋

- 오너 리포트: 겹쳐보기 나스닥·S&P500이 2026-04-01에 멈춤. 뿌리 = `SYM:` 경로가 price_daily 직독 + 최초 1회 백필 후 트레일링 갱신 경로 부재 → 최초 사용일 동결. `_portfolio_index_series`에 트레일링 gap-fill 추가(P2-3 dedupe 재사용). 거시 IDX_* 경로는 원래 beat로 최신이었음.
- 백테 가치 추이: y축 0부터(beginAtZero) + `↺ 전체 기간` 버튼(줌 시 노출)·더블클릭 리셋. 상세=[[dev/status]] 2026-07-09 절.

_작성: Claude_

## 2026-07-09 — 포폴 비교에 부동산 지표 검색 가능화

- 오너 요청: 포트폴리오 비교에 한국 부동산 전부. 백엔드는 이미 지원 — 프런트 검색이 이름만 매칭이라 "부동산"으로 못 찾던 것. 벤치마크·겹쳐보기 검색에 카테고리 매칭+캡 40. 상세=[[dev/status]] 2026-07-09 절.

_작성: Claude_

## 2026-07-18 — 정밀비교 대가 10+ 버그 3종 (오너 리포트)

- 오너: "투자대가 10개 넘게 비교하면 포트폴리오 정밀 비교가 안 된다". 재현 → 63s(계산 성공, nginx 30s에 끊김)이 뿌리. 프로파일로 병목 = **요청 중 티커별 야후 갭필 직렬 122콜(56s)** 확정 — `_item_deep` 아님.
- 수정 3종(상세=[[dev/bugs]] 07-18 절): ① rr/비교 경로 `skip_gapfill`(DB-only, 콜드만 페치) + prefix 갭필 하루 1회 가드 ② 결측 티커 포폴 통째 무경고 제외 → 결측만 빼고 재정규화+`partial_portfolios` 표기 ③ `_yf_dl_ticker` 슬래시 처리(BRK/B→BRK-B).
- 결과: 63s→8.9s/4.8s, 대가 10/10 표시(기존 8/10), STKL(야후 상폐)만 부분표기. 검증 = compute_comparison 실측 + test_risk_return·test_portfolio_compare_route 12 PASS + Playwright 라이트/다크 렌더·jsErr 0.
...
## 2026-07-19 — 날짜픽커 기간 하한 1985 → 1900 해제 (오너 지시)

- 오너: "포트폴리오 분석 등 많은 탭 기간 설정 한도가 1985 — 싹 찾아서 1900으로."
- 락은 단일 지점: `static/js/mm_datepicker.js` `bounds()`의 기본 minY 폴백 `1985` → `1900`. (base.html 전역 로드라 모든 탭 공통 적용 — backtest·비교·기여도·myassets 등.) 템플릿/백엔드에 별도 1985 하드코딩 없음(`modules/index_loader_develop.py`의 ^NDX "1985-01-01"은 실제 지수 시작일 메타라 대상 아님).
- 명시적 `min` 속성 있는 입력은 기존대로 그 값이 우선(폴백만 완화).
- 검증: Playwright(chromium) 하네스 5 PASS — 기본 연도 select 1900~내년(128개), 1900 날짜 선택 시 value 반영, `min="2000-01-01"` 입력은 2000 하한 유지.
- 유의: 1900 선택해도 데이터는 각 종목/지수 실데이터 시작일부터(엔진 공통기간 클램프 기존 동작 그대로) — UI 하한만 완화.

_작성: Claude_

## 2026-07-19 — 미국 자산 원화 환산 1964 FX 절벽 수정 (오너 지시)

- 증상(오너 발견): 기간 하한 1900 해제 직후, S&P 백테스트에 1964년 +25,760% 점프.
- 원인: `index_daily` USD/KRW 시리즈가 1964-05-04(255.77원, 평가절하)부터 시작.
  `price_loader.get_price`의 환율 폴백이 첫 관측 이전 날짜에 rate=1.0(달러 그대로) 적용 →
  1964-05-04에 ×255.77 절벽. 1985 하한 시절엔 도달 불가라 잠복. 원시 종가(로컬·prod 모두)는 무결.
- 수정(오너 결정: 첫 환율로 정규화): `_load_usdkrw`가 시리즈를 1900-01-01까지 첫 관측값
  255.77로 플랫 백필(bfill). get_price·get_usdkrw·dividend_history 등 소비처 일괄 적용 —
  모든 미국 자산 공통. backfill_engine의 KR ETF FX절단(07-14, 379780)은 별건이라 유지.
- 검증: 1963~66 ^GSPC 환산 시리즈 최대 일변동 4.17%(이음새 +0.37%), ^GSPC·SPY 1900 시작
  백테 재실행 절벽 소멸(잔여 대변동 = 1932·33 대공황 반등, 1997~98 IMF 환율 — 실제 역사),
  tests/test_priceloader_fx.py 통과.
- 유의: 1964-05 이전 원화 환산은 "당시 실제 환율"이 아니라 255.77 고정 가정(전 시대 환·화폐개혁
...
## 2026-07-20 — 내자산 보유종목 카드에 1주당 현재가 표시 (오너 요청)

- 요청: 보유종목 카드가 평가금액(총액)만 보여줌 → 한 주당 가격도 표시, 칸 여유 확보.
- 구현: 카드 메타행 우측(총액 바로 아래)에 `1주 ₩x` 추가(`.hc-cur`, 모노스페이스·muted).
  카드 패딩 12px→14px 소폭 확대. 새 `fmtPrice()` — **시세는 공개 정보라 금액 숨김(hideAmounts)
  마스킹 제외**(총액·평단은 기존대로 마스킹). 펼침 상세의 현재가도 fmtKRW→fmtPrice로 통일
  (같은 카드에서 접힘=실가·펼침=*** 모순 제거).
- 부수 수정: `.hc-chev`와 `.hc-cur` 둘 다 `margin-left:auto`면 데스크톱서 여백이 분배돼
  1주 가격이 중앙에 뜸 → chev 쪽 auto 제거(가격+chev 우측 정렬 그룹).
- 검증(Playwright, 로컬+mint_session): 모바일 390px(탭 전환)·데스크톱 1280px 다크,
  접힘/펼침, 금액 가리기/표시 토글 양방향 — 가리기 상태서도 1주 가격 표시·총액은 마스킹 확인.
  콘솔에러 0, 스샷 육안 OK.
...
## 2026-07-22 — 내 자산 온보딩 UX + 입력창 시인성 (디자인 개선)

오너 제보 2건 수정. 기능 변경 없음, UI만.

1. **신규 가입자가 "현황"에서 막힘** — 종목 추가 버튼이 "종목" 탭(모바일) / 보유 종목
   카드에만 있어, 첫 진입 화면인 현황에는 진입점이 없었다. 자산 추이 차트 빈 영역
   (`#maHistoryEmpty`)을 안내 문구 한 줄 → **아이콘 + 제목 + 설명 + `+ 첫 종목 추가` CTA**
   빈 상태로 교체. 보유 종목이 이미 있는데 이력만 없는 경우를 구분하려고
   `syncHistoryEmptyMode()`가 `holdings.length`로 `.has-holdings`를 토글 —
   있으면 CTA 대신 "시세 이력이 아직 없습니다" 문구만 보인다.
   보유 종목 목록의 빈 상태(`renderHoldings`)에도 같은 버튼 추가.
2. **모달 입력창이 안 보임** — `.calc-input`은 테두리가 없고 `.input-group` 래퍼가
   테두리·배경·포커스링을 담당하는 구조인데, **종목 검색·매수일·그룹 select·그룹명**
   4개가 래퍼 없이 맨몸으로 놓여 있어 빈 공간처럼 보였다(수량·평단가만 래퍼 있음).
   전부 `.input-group`으로 감싸고 종목 검색엔 돋보기 아이콘 추가(`mi('search')` 신규).
   자동완성 드롭다운은 기존 `position:relative` 컨테이너의 형제로 유지 — 위치 영향 없음.

검증: 로컬 서버 + Playwright(430×900), 라이트/다크 양쪽. 현황 CTA 노출 → 모달 열기 →
"삼성" 검색 → 드롭다운 6건 → 선택 → 수량·평단 입력 → 저장 → 보유 목록·차트 렌더까지
...
## 2026-08-05 적립식 백테스트 수익률·MDD가 납입금에 오염

오너 제보: S&P500 적립식 2006-01-01~2025-12-31 분석에서 **2006년 연간수익률 1000%+,
2007년 80%**, MDD도 이상.

**재현**(로컬 서버 `/api/backtest/run`, 초기 10만 · 월 100만): 2006 +1113%, 2007 +52.8%,
**2008 +11.0%**(실제 폭락 연도). 2008이 양수로 나오는 게 결정적 증거였다.

**뿌리 = 지표를 전부 `portfolio_value`(계좌 잔고) 기준으로 계산**한 것. 적립식은 잔고가
납입 때문에 불어나므로

- `annual_returns` = 연말잔고/연초잔고 − 1 → **수익률이 아니라 납입금 증가율**
- `mdd` = 잔고 cummax 대비 낙폭 → 매달 들어오는 납입금이 고점을 밀어올려 낙폭이 희석
- `sharpe` = `pv.pct_change()` → 납입일마다 점프가 양(+)수익으로 섞여 과대(1.21)

초기금이 작을수록 첫 해 배율이 폭발한다. 10만 + 월 100만이면 1월 잔고 110만 → 12월
1,260만, 즉 **+1045%**. 오너가 본 숫자와 정확히 일치.

**수정** = `backtest_logic.py`에 TWR(시간가중) 지수를 도입하고 수익률성 지표를 전부 여기에 태움.

```
r_t = (PV_t - CF_t) / PV_(t-1) - 1      # 납입금은 당일 종가 매수 → 당일 수익 기여 0
```

- 연간수익률 = TWR 지수 그 해 말 / 전년 말 − 1
- MDD·낙폭 시계열 = TWR 지수 cummax 대비
- Sharpe = TWR 일간수익률
- 총 납입금 = 기록된 `cash_flow` 양수 합계(근사식 `initial + monthly*years*12` 폐기),
  가치추이 차트의 납입금 점선도 실제 누적 계단으로
- CAGR = **금액가중(MWR)** — 월별 순납입 IRR → 연환산. 거치식은 기존 단순 연환산과 동일값

단일계좌·멀티계좌(세금 ON) 두 경로가 **같은 버그를 각자 복사해** 갖고 있어 `_perf_block`
하나로 합쳤다.

**검증**: 적립식과 거치식의 연간수익률이 소수점까지 수렴(TWR은 정의상 납입과 무관해야 함) —
2006 +5.1/+4.9, 2008 −15.2/−15.3, 2022 −12.5/−12.4 … 원화 SPY 기준값과도 부합
(2008 USD −37% × 원달러 +34% ≈ −15%). MDD −29.9%(2008), Sharpe 1.21→0.67,
총 납입금 정확히 2.5억. 거치식 MWR 12.74% = `(109.9M/10M)^(1/20)−1` 일치.
타겟 테스트 12 PASS(gate1·gate2a·d4_fee), Playwright 실브라우저 7 PASS
(`tests/check_bt_dca_returns.js` 신설, jsErr 0).

⚠️ 남은 것: `dividend_mode='withdraw'`는 배당 유출이 `cash_flow`에 없어 TWR이 배당수익률만큼
낮게 잡힌다(수정 전에도 동일). 은퇴탭 `calc_metrics_from_history`의 MDD도 아직 잔고 기준.

_작성: Claude_

---

## 2026-08-05 (2) 같은 뿌리 잔여 2건 — 적립기 MDD · 배당 인출

앞 항목에서 "별건"으로 남겨둔 2건을 오너 지시로 마저 수정.

**① 투자계산기·은퇴 적립기 MDD도 잔고 기준이었다.** 백테스트와 완전히 같은 뿌리.
`AccumulationAnalyzer._calc_metrics`와 `calc_metrics_from_history`가 `pv.cummax()`로
낙폭을 재고 있었다. Sharpe만 "납입일 daily_return 제외"로 반쯤 막아둔 상태였는데,
그건 증상 완화지 원인 제거가 아니다(납입일을 빼도 그 다음날 수익률 분모가 이미 부풀어 있음).

둘 다 TWR 지수 기준으로 바꾸고 Sharpe·Sortino도 TWR 일간수익률로.
**계산기 10년 SPY 롤링 210케이스 MDD: 중앙 -29.6% → -34.2%, 최악 -43.2% → -46.5%.**
적립식이 실제보다 4~5%p 방어적으로 보이고 있었다는 뜻.

**② 배당 인출이 수익이 아니라 손실로 잡혔다.** `dividend_mode='withdraw'`(투자계산기·
배당 목표 역산에서 선택 가능)는 배당을 포트폴리오 밖으로 빼는데, 그 유출이 `cash_flow`에
기록되지 않았다. TWR 입장에선 "잔고가 이유 없이 줄었다" = 손실. 결과적으로 수익률이
배당수익률만큼 통째로 깎였다. `simulation_loop`·`multi_account_loop`에서 인출액을 음수
`cash_flow`로 기록.

평탄가(100 고정) + 분기배당 0.5 결정론 케이스로 검증: 인출 모드 연 **0.0% → +2.0%**,
재투자 모드와 TWR 일치(배당을 재투자했든 빼갔든 *수익률*은 같아야 한다), 낙폭 0.

**③ ②가 ①을 깨뜨릴 뻔했다.** 은퇴탭 IRR이 `cash_flow != 0` 행을 그대로 `enumerate`해
"행 하나 = 한 달"을 가정하고 있었다. 배당 인출은 월중 아무 날에나 생기므로 그 가정이 깨진다.
`monthly_flows()`로 월 버킷팅 후 IRR(백테스트는 처음부터 이 방식). 월중 유출 30건을 주입해도
MWR 불변임을 테스트로 고정.

**공용화**: `modules/perf_metrics.py` 신설 — `twr_index`·`max_drawdown`·`monthly_flows`·`mwr`.
같은 Newton IRR 코드가 3곳에 복사돼 있던 것도 정리(legacy 1곳은 테스트 전용이라 방치).

**의도적 미변경**: 인출기(`withdrawal_analyzer`) MDD는 잔고 기준 유지. 은퇴 인출 국면에서는
"내 잔고가 얼마나 줄었나"가 곧 관심사라, TWR로 바꾸면 지표 의미 자체가 달라진다.

**검증**: 신규 결정론 테스트 9종(`test_accum_metrics_twr.py`·`test_dividend_withdraw_twr.py`).
둘 다 수정을 되돌리면 실제로 FAIL하는 것까지 확인(공허한 테스트 방지).
타겟 회귀 201 PASS — 엔진·적립·인출·배당·세금·롤링 전 경로.
