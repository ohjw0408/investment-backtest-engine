...
**수정**: `_has_intraday_hole()` — 730일 창에서 연속 거래일 간격이
`INTRADAY_MAX_HOLE_DAYS=10`(최장 연휴 8일보다 큼)을 넘으면 결손으로 보고 730d 재조회.
꼬리(마지막 봉 이후)도 같은 기준. 무한 재조회를 막으려고 코드별 6시간 레이트리밋
(`_intraday_fetch_ok(code + ":deep", 21600)`)을 건다. 실행 결과 위 두 구멍 모두
자가복구됐고 남은 5일 이상 공백은 전부 실제 휴장(추석·설·연휴)이었다.
