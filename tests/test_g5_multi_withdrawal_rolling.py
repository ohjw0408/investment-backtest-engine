...
def test_survival_rate_one_when_all_windows_survive():
    ...
    # step 3개월 → 실윈도우 ≥30 → 합성 보충 없음(평탄 결정론 유지)
    res = analyze_household_withdrawal(
        accts, data, dates, "2030-01-01", "2040-12-31",
        withdrawal_years=2, monthly_net=100_000.0, step_months=3,
    )
    assert res["n_synthetic"] == 0
    assert res["survival_rate"] == 1.0
    assert res["n_windows"] >= 30
    # 2년 윈도우 = 23~24개월 인출(첫달 스킵·경계 영업일) × 100k → 9.6M~9.7M
    assert 9_600_000.0 - 1.0 <= res["combined_end_value"]["p50"] <= 9_700_000.0 + 1.0
...
def test_survival_rate_zero_when_all_deplete():
    ...
    res = analyze_household_withdrawal(
        accts, data, dates, "2030-01-01", "2040-12-31",
        withdrawal_years=2, monthly_net=200_000.0, step_months=3,
    )
    assert res["n_synthetic"] == 0
...
def test_per_account_distribution_surfaced():
    ...
    res = analyze_household_withdrawal(
        accts, data, dates, "2030-01-01", "2040-12-31",
        withdrawal_years=2, monthly_net=150_000.0, step_months=3,
    )
    assert res["n_synthetic"] == 0
    assert res["survival_rate"] == 1.0
    ...
    # 위탁 먼저 소진(150k×23~24 = 3.45M~3.6M < 5M) → ISA 전액 5M 잔존(견고)
    assert abs(res["per_account"][1]["end_value"]["p50"] - 5_000_000.0) <= 1.0
    assert 1_400_000.0 - 1.0 <= res["per_account"][0]["end_value"]["p50"] <= 1_550_000.0 + 1.0
    # 합산 = 위탁 + ISA
    assert abs(
