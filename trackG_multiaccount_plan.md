...
**입력:** Phase 2f의 **연도별 종합과세 대상 트래킹**(`comprehensive_years`). 계좌간 금융소득 집계 =
위탁 배당 + KR_FOREIGN 실현차익(ISA/연금 제외) → 가구 단위 연도별 2천만 초과 판정.
...
**의존성:** G1 완료 + Phase 2f(종합과세 트래킹). **난이도:** 매우 높음 (1~2주). 구현보다 검증이 어려움.
