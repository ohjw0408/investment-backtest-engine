...
**의존성:** G1 완료. **난이도:** 매우 높음 (1~2주). 구현보다 검증이 어려움.
...
| 계층 | 내용 | 검증 방식 |
|------|------|----------|
| **L0 회귀** | 1계좌를 MultiAccountLoop로 | 기존 `TaxableSimulationRunner`와 종료값 ±1원 / CAGR·MDD ±0.01% 일치 (골든) |
| **L1 합산** | 2계좌, 이동·세금 OFF | `combined_i == A_i+B_i` 정확. A가 시나리오1 최고/2 최악, B 반대로 구성 → **퍼센타일(합산) ≠ 퍼센타일 덧셈** 증명 |
| **L2 기여금** | 평탄가격, 적립만 | `종료값 == 초기+Σ월납입` 정확. ISA 캡 stop_months 검증(예: 190개월 후 정확히 1억) |
| **L3 세금단위** | 단일배당/계단가격 | 배당세·양도세·ISA 청산세 각각 손계산값과 일치 |
| **L4 한도라우팅(G2)** | 평탄, 이동 ON | ISA 연 정확히 2000만 흡수, 초과분 정책대로, 연금 1800만 도달 시 위탁 캐스케이드 — 월별 손계산 |
| **L5 만기분배(G2)** | 3년 평탄/성장 | 목돈 정책대로 새ISA/연금/위탁 배분, 각 계좌 잔액 정확 |
| **L5b 다중사이클 풍차돌리기(G2)** | 9년(3사이클) 평탄가격 | 매 만기마다 ① 2000만 새 ISA 재가입 ② 비과세 한도 리셋 ③ 나머지 연금/위탁 분배 ④ 3사이클 후 누적 자산이 손계산값과 일치. **풍차돌리기 반복 사이클 핵심 검증** |
| **L6 연금이전공제(G3)** | 이전액 X | 공제 = min(X·0.1, 300만), 연금 연한도 내 검증 |
| **L7 통합** | 실데이터 다계좌 이동ON | **불변식**: 자금보존, 음수잔액 없음, ISA≤1억, 연금+IRP≤1800만 |

### 전 계층 공통 불변식 (매 테스트 어서트)
- **자금 보존:** Σ계좌종료값 = Σ투입 + Σ수익 − Σ세금
- 음수 잔액 없음
- 한도 위반 없음 (ISA ≤ 총 1억, 연금+IRP 연 ≤ 1800만)

### 검증 순서 (구현과 맞물림)
L0 → L1 → L2 → L3 (G1 토대 확정) → L4 → L5 → L5b → L6 (G2/G3) → L7 (통합).
각 계층 통과 전 다음 단계 진행 금지. L0~L3가 G1의 합격 기준, L4~L6이 G2/G3, L7이 최종.
...
### 1. [버그] 다중계좌 시 배당 지표 전부 0
**증상:** 계좌 2개 이상일 때 결과 하단의 총 배당금 / 마지막 연도 배당금 / 배당 CAGR / 배당률 분포가 전부 0으로 나옴. 단일계좌에서는 정상.
**추정 원인:** `MultiAccountAnalyzer` 결과 합산 시 배당 관련 필드(`total_dividend`, `last_year_dividend`, `dividend_cagr`, `dividend_yield_on_cost` 등)가 계좌별 → 합산으로 집계되지 않거나, 프론트가 다중계좌 응답에서 해당 필드를 못 읽음. `MultiAccountSimulationLoop`은 배당을 현금에 반영하지만(L3 PASS), 결과 스키마에 배당 분포 메트릭이 누락됐을 가능성.
**조치:** 계좌별 배당 메트릭 집계 → combined 배당 분포 산출. 단일계좌 결과 스키마(`calculator.js` renderResult가 읽는 필드)와 일치시킴. 우선순위 높음 — 기능 누락.
