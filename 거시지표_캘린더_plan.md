...
작성: 2026-06-15 (Claude, 오너 아이디어).
상태: ✅ **거시지표 탭(PART A, 151종+커브7)·증시 캘린더(PART B)·겹쳐보기 고도화(C1·C2)·포폴비교탭 오버레이 통합(C3) 전부 완료.** (`/macro`·`/calendar`·`/risk-return`). 정책일정 Tier1(FOMC·금통위)도 완료. **거시지표+캘린더 플랜 전체 종결.**

> **구현 메모(2026-06-15):** 초기 지표 ~80 → 시장지수 39 추가로 **총 129종**. KOSIS 불필요(전부 ECOS) 확정. 전체 히스토리(출범부터). UI = 토글 US/KR/글로벌/한미비교/겹쳐보기 + 카드그리드(검색·툴팁) + 상세(기간토글·지수 캔들) + 겹쳐보기(프리셋·구간·다축). 자동갱신 beat. 자세한 경위 = `moneymilestone/wiki/log.md` 2026-06-15 거시경제 지표 탭 항목.
...
## C3. 포트폴리오 비교탭(/risk-return) 오버레이 통합 — ✅ 완료 (2026-06-15)

> **완료 노트(2026-06-15):** `/risk-return`에 "📈 추세 겹쳐보기" 카드 추가(기존 스파이더·산점도·11지표 표 무변경).
> **오너 결정 반영:** ① 재사용 방식 = **독립 오버레이**(기존 `/api/macro/multi` 호출, macro.js 무손상=회귀0). ② 거시지표는 **오버레이 추가 검색에만**(11지표 표·산점도엔 미포함 — 거시는 수익률 지표 산출 불가). ③ **스마트 기본 프리셋**: 저장포폴 0개=일반예시(S&P500·코스피200·금) / 1개=그포폴+S&P500 / 2개=두포폴+S&P500 / **3개↑=앞 2개포폴+S&P500**(과밀 캡). ④ **기간 기본 ≤5년**(정규화 시 작은차이 과장 방지) + 5년초과+정규화 경고문구.
> **백엔드:** `/api/macro/multi` 토큰 `PF:<id>` 추가(`_portfolio_index_series` = 비중고정 정규화 지수, 종목별 종가 시작=100 가중합 → 통화무관, 6년 cap·공통날짜 교집합) + 기존 `SYM:`·거시코드 유지. **프론트(risk_return.html 자체 IIFE):** 빠른추가 버튼(내 포폴⭐+SPY/코스피/금/미국채) + 통합검색(거시 `/api/macro/overview` + 종목 `/api/search`) + 칩 + 구간(날짜·1/3/5/10/전체, 기본5년) + 정규화↔원값(개별축) + 통합 날짜축(category) Chart.js. **검증:** app.py 구문·JS 파싱 OK + 포폴지수 수학(50/50 A+20%·B flat→110) + overview name_ko/unit 키 확인. ⚠️ 서버 재시작(use_reloader=False)+브라우저 실검증 필요.
