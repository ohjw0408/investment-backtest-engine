...
## ▶ 후속 수정 (2026-06-18, 오너 피드백)

- **계산기 "하락 방어" 지표 버그 정정:** 기여(비중×수익)는 큰 비중 종목이 불리(SCHD50>QQQM20 →
  SCHD가 방어 못함처럼 보임). → **비중 무관 포착률(capture)**로 교체 = `analyze_capture`
  (down/up_capture = 종목 하락/상승장 평균수익 ÷ 포폴 평균). 순위는 비중 무관(분모 공통). 용어
  "상승 참여율/하락 방어율(배)" 직관화. 실측: GLD 0.71(최강방어)·SCHD 1.11<QQQM 1.16(SCHD 방어우위)·QQQM 견인1위.
  `/api/attribution/capture`. (analyze_rolling은 사용 안 함, 잔존.)
- 표를 **부채꼴 카드 밑으로** 이동.
- 백테 **모바일 대응:** 드래그 불가 → 날짜 직접선택 입력 + "이 구간 분석" 버튼. **MDD(낙폭) 차트에도 드래그** 활성화.
- 내자산 방어 종목도 포착률 기준(견인=기여 유지). 검증 test_attribution 17 PASS.
