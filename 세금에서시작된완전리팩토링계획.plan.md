...
todos:
  ...
  - id: phase1-tax-profile-api
    content: "Phase 1: tax_settings/domino_tax_settings → 통일 TaxProfile (earned_income, age, income_type, isa_type, harvest) 모든 시뮬 API에 전달. other_financial_income은 세금설정 수동 입력 금지, 계산기별 자동 산출값으로만 런타임 주입 — ✅ 2026-06-13 정리: Phase 2f(4100ecd)로 충족. _ytd_income 주입(account_tax.py:243)·자동산출(recurring_financial_income, backtest/calc/retirement)·수동입력 UI 제거(05-29) 전부 구현 확인"
    status: done
  - id: phase1-gate
  ...
  - id: phase2e-comprehensive-tax-panel
    content: "Phase 2e: 금융소득 종합과세 경고 + 분할매도 절세 계획 패널. ✅ 2026-06-13 정리: 미완 ①②③ 전부 Phase 2f(4100ecd, 2026-05-31)로 해소 — ① 자동산출(recurring_financial_income) ② calculator/retirement 배선(배당탭은 별도엔진 설계상 제외) ③ _ytd_income 주입"
    status: done
  - id: phase2d-withdrawal
    content: "Phase 2d: 은퇴 인출 워커 세금 주입 + Gate 4"
    status: done
  - id: phase2f-comprehensive-accuracy
    content: "Phase 2f(신규, 오너결정 2026-05-31): 금융소득 종합과세 정확도. ① KR_FOREIGN 실현 시세차익(중간+청산)을 그 해 배당과 합산해 종합과세(현재 청산 15.4% 분리) ② _ytd_income=profile.other_financial_income 주입(현재 0) ③ other_financial_income 자동산출(직전 완료년도 gross 배당·이자) ④ 분할매도 슬라이더 패널 전탭 배선(현재 백테스트만) ⑤ 연도별 종합과세 대상(금융소득>2천만) 트래킹·기록 → Track G ISA 자격 입력. US 양도차익은 22% 별도 미합산. ISA/연금 제외."
    status: done
  - id: phase3-cleanup
...
#### Phase 2f — 금융소득 종합과세 정확도 + 대상자 트래킹 (신규, 2026-05-31 오너 결정)

> **실행 순서·의존성 (오너 합의 2026-05-31): Phase 2f 먼저 → 그 다음 Track G2.**
> - 의존 방향 단방향: G2의 ISA 가입불가·풍차 중단 로직은 **2f가 만드는 "연도별 종합과세 대상 트래킹"을 입력으로
>   소비**한다. 2f 없이는 G2 자격 판정 불가.
> - 2f는 transfer 불필요(단일계좌) → 지금 구현 가능 + 손계산 골든값으로 세금 수학 단독 검증. G2는 transfer
>   엔진(현재 `multi_account_loop` `transfers_enabled=True` → NotImplementedError)부터 신규 구축이라 고위험.
> - **설계 제약:** 2f 트래킹·`ytd_financial_income`(배당+KR_FOREIGN 실현차익 통합)을 **세션/가구 레벨 추상으로**
>   설계할 것. 단일계좌에서도 이 구조를 쓰고, G2는 계좌간 집계로 **확장만** 한다(단일계좌 가정 박지 말 것).

**문제(오너):** 10억 배당 ETF 10년 운용, 매년 배당 3천만, 마지막 해 배당 3천만 + 시세차익 1억.
제대로 된 엔진이면 ① 매년 배당 3천만에 종합과세, ② 마지막 해 **배당 3천만 + 시세차익 1억 = 1.3억**
합산 종합과세. 그래야 복리가 정확. 현재는 청산 시세차익을 그 해 배당과 합산 안 하고 15.4% 분리로만 뗌.

**코드 실상(2026-05-31 확인):**
- ✅ 매년 배당 종합과세: `TaxedDividendEngine`가 연도별 `_ytd_income`(gross) 누적 → `after_tax_dividend`로
  2천만 초과 시 종합과세 적용. **단 `_ytd_income=0` 시작**(기존 금융소득 무시, `account_tax.py:230`).
- ❌ KR_FOREIGN 실현 시세차익을 그 해 금융소득 ytd에 **합산 안 함.** 청산=`liquidation.py:66` flat 15.4%,
  중간 실현(리밸/절세매도)도 동일. → 배당+차익 합산 종합과세 미작동.
- ❌ `other_financial_income` 자동산출 없음(`backtest_logic.py:117` 수동값/0).
- ✅ `split_sale_planner.compute_split_sale_plan` 완성(lump/split/optimal/plan_by_year 1~20/after_tax/
  over_threshold) — 슬라이더 백엔드. **단 백테스트 탭만 배선.**

**오너 결정 (디테일):**
- **Q2 소득 범위:** 금융소득 = 이자 + **모든 배당**(KR/US/KR_FOREIGN 배당) + **KR_FOREIGN 시세차익**(세법상
  배당소득). **US_DIRECT 양도차익은 양도소득(22% 별도, 종합 미합산).** ISA/연금 내부 흐름 제외.
- **Q1 end_value/패널:** end_value 헤드라인 = **일괄청산 종합과세 기준**(그 해 실현 배당+차익 합산, 정확).
  결과에 **분할매도 슬라이더**(현 백테스트 구현 방식) — 슬라이더 돌리면 일괄청산세금 / 절세세금 / 세후
  순이익 표시. 소득구간별(2천만↓ / 매년 2천만↑ / 이미 최고세율↑)로 절세효과가 0~중간~0임을 자세히 보여줌.

**작업:**
| 작업 | 파일 |
|------|------|
| 선행: 히스토리/breakdown `dividend_income` gross/net 확인(종합과세 판단=gross) | `taxable_runner.py`, `history_recorder.py` |
| KR_FOREIGN 실현차익(중간+청산)을 `TaxSessionState.ytd_financial_income`에 배당과 통합 누적 | `session.py`(or 상응), `order_executor.py`, `liquidation.py` |
| 청산 시 KR_FOREIGN 차익 + 그 해 ytd 금융소득 합산 → 종합과세 적용(15.4% 분리 → 합산 종합) | `liquidation.py` |
| `TaxedDividendEngine._ytd_income` = `profile.other_financial_income`로 초기화(phase1-api) | `account_tax.py` |
| `other_financial_income` 자동산출: 직전 완료년도 gross 배당·이자(위탁) — 백테스트 먼저, case별(계산기), 배당/연금 순 | `taxable_runner.py` 등 |
| 분할매도 패널 전탭 배선(계산기/배당/연금) + 자동 금융소득 주입 | `calculator_logic.py`/`dividend_logic.py`/`retirement_logic.py` + JS/HTML |
| **종합과세 대상 트래킹:** 연도별 금융소득 합계 > 2천만 여부 기록(`comprehensive_years: set[int]`) → 결과/breakdown 노출. ISA 자격 판정(Track G)의 입력 | `session.py`, `taxable_runner.py` |

**테스트:**
- 소득구간 3종 절세효과(2천만↓=분할이득 / 매년 2천만↑=의미작음 / 최고세율↑=무의미) 골든값.
- 배당3천+청산1억 → 1.3억 합산 종합과세 정확값.
- `_ytd_income` 주입: 기존 금융소득 1,900만 + 당해 배당 경계.
- 종합과세 대상 트래킹: 연도별 flag 정확.
- 회귀: Gate 2a~2d 불변.

> **Track G 연계:** 종합과세 대상 트래킹 결과가 다중계좌 ISA 가입불가 판정(직전 3년)·풍차 중단의 입력.
> 상세는 `trackG_multiaccount_plan.md` § 2-4.
...
| Phase 1 TaxProfile API 통일 | ✅ 완료 (2026-06-13 정리 — Phase 2f로 충족) | _ytd_income 주입·자동산출·수동입력 제거 코드 확인 |
| Phase 2c: 배당 역산 Runner | ✅ 구현 + Gate 재검증 완료 | 2026-05-30: DJUSDIV_PROXY ^GSPC 제거(2003 시작) 후 Gate 2c PASSED 3/3, 투자계산기·배당금계산기 양쪽 SCHD≈458730 수렴(역산 4x→1.2x). 커밋 e6707bd |
| Phase 2d: 은퇴 인출 세금 주입 | ✅ 완료 | Gate 2d 5/5 통과 |
| Phase 2e: 금종세 경고 + 분할매도 절세 패널 | ✅ 완료 (Phase 2f 4100ecd로 잔여 해소) | 자동산출·전탭배선·ytd주입 전부 구현(배당탭만 별도엔진 제외) |
...
> ✅ **2026-06-13 정정:** 아래 갭 목록의 [2e] 3건은 전부 Phase 2f(4100ecd, 2026-05-31)로 해소 확인 —
> ① 자동산출 = `recurring_financial_income`(backtest_logic:309·calculator_logic:476,780·retirement_logic:317,654)
> ② 전탭 배선 = 동일 호출부 ③ `_ytd_income` 주입 = `account_tax.py:243` + 공유 `TaxSessionState`.
> 인출(decum) 경로 종합과세도 기배선 확인(GAP-DECUM-COMP 해소, `test_decum_comprehensive.py` 4 PASS).
> 아래 목록은 당시 기록 보존용.
...
> **2026-05-31 갱신:** Phase 2c 재검증 ✅ 완료(Gate 2c 3/3). 배당 백필 Stage A + Stage B(채권/MMF·
> 환헤지·US 채권 자동분류) 서버검증 완료 → **종합과세 데이터 토대(배당·이자 흐름) 완성, 블로커 없음.**

> ✅ **2026-06-12 정리: 아래 우선순위 1은 완료됨** — Phase 2f 커밋 `4100ecd`(2026-05-31)로 ①~⑤ 전부 구현
> (④ 배당탭은 별도엔진이라 설계상 제외). 검증 = `test_phase2f_comprehensive` 7/7 + `tax_truth` 64/64 +
> Gate 2a/2b/2c. 멀티계좌 경로는 Track G + E2E 16/16(2026-06-11)으로 추가 검증. 잔여 = Phase 3 정리·문서(낮음).

**우선순위 1 — 금융소득 종합과세 완전 구현 = Phase 2f + Phase 2e 배선 + phase1-tax-profile-api.**
종합과세 **엔진 수학은 완료**(`base_tax`, 2천만 임계, `tax_truth_test`). 오너 결정(2026-05-31) 반영한
**상세 작업·디테일은 위 `#### Phase 2f` 섹션** 참조. 요약:
- ① **KR_FOREIGN 실현 시세차익(중간+청산)을 그 해 배당과 합산 종합과세** — 현재 청산 15.4% 분리(핵심 갭).
- ② **`_ytd_income` = `profile.other_financial_income` 주입**(현재 0 고정).
- ③ **`other_financial_income` 자동산출**(직전 완료년도 gross 배당·이자, 백테스트 먼저).
- ④ **분할매도 슬라이더 패널 전탭 배선**(현재 백테스트만; 슬라이더→일괄/절세/세후순이익, 소득구간별).
- ⑤ **연도별 종합과세 대상 트래킹** → Track G ISA 가입불가/풍차 중단 판정의 입력(`trackG § 2-4`).
- 자산 분류: US 양도차익 22% 별도(미합산), ISA/연금 제외. 선행=`dividend_income` gross/net 확인.

실행 지시: 「금융소득 종합과세 구현해줘」.
