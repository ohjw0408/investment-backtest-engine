...
| Phase 2c: 배당 역산 Runner | ✅ 구현 / 🔴 Gate 재검증 필요 | 진짜 원인 = 배당 데이터 0 버그 (ETF_BACKFILL Phase 6.0). 가격 수렴만 검증됐었음 |
...
- **🔴 [선행] 종합과세 YTD 입력 = 배당인데 배당 데이터가 0** (ETF_BACKFILL Phase 6.0 버그). 배당 정상화 전엔 종합과세가 실전에서 발동 안 하므로 2e 완성·검증 불가. **Phase 6.0 Stage A 선행 필수.**
